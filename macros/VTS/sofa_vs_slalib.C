...
map< int, dset* > read_tree( int runnumber, string iFile, string iname = "" )
{
    ...
    if( t->GetBranchStatus("XCore")) t->SetBranchAddress( "XCore", &xc );
    if( t->GetBranchStatus("YCore")) t->SetBranchAddress( "YCore", &yc );
    if( t->GetBranchStatus("MSCW" )) t->SetBranchAddress( "MSCW", &mscw );
    if( t->GetBranchStatus("MSCL" )) t->SetBranchAddress( "MSCL", &mscl );
    if( t->GetBranchStatus("IsGamma" ) ) t->SetBranchAddress( "IsGamma", &gh );
        ...
        bool bPrintIsGamma )
{
    ...
    {
        if( bPrintIsGamma && it->second->IsGamma == 0 ) continue;

        if( map2.find( it->first ) == map2.end() )
        {
            if( bPrint ) it->second->Print(it->first, "\t event missing: " );
            z++;
        }
        ...
        bool b_derot )
{
    ...
    for( map< int, dset* >::iterator it = map1.begin(); it != map1.end(); ++it )
    {
        if( map2.find( it->first ) != map2.end() )
        {
            ...
            if( d_diff < d_diff_min ) d_diff_min = d_diff;
        }
    }
    if( b_derot ) cout << "X/Yderot diff: mean ";
    else cout << "X/Yoff diff: mean ";
