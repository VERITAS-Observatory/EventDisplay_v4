...
VMinimizer::VMinimizer(const VMinimizerFn& fn, const vector<double>& param, const vector<double>& error): fFn(fn), fHasLoBound(param.size()), fLoBound(param.size()), fHasHiBound(param.size()), fHiBound(param.size()), fParam(param), fError(error), fCov(param.size(),param.size()), fFnVal(), fConverged(), fNFit(fn.getNParam()), fFit(fn.getNParam(),true)
{
}
...
VLevenbergMarquardt::VLevenbergMarquardt(const VMinimizerFn& fn, const vector<double>& param, const vector<double>& err, double tol, unsigned max_iterations, bool verbose): VMinimizer(fn,param,err), fDa(), fAlpha(), fBeta(), fCov(fn.getNParam(),fn.getNParam()), fTol(tol), fMaxIterations(max_iterations), fVerbose(verbose)
{
}
...
void VLevenbergMarquardt::minimize()
{
  ...
  while(1) {
    ...
    if(fNiter >= fMaxIterations) break;
    fnval = fFnVal;
      
    iterate();
      
    dfnval = (fFnVal-fnval);	
    if(fVerbose) cout<< "VMinimizer: iter "<< fNiter << " min " << setprecision(8) << fnval << " difmin " << dfnval << " lamba " << fLambda <<endl; 
  ...
  setFnVal(fFnVal);
  if ( fVerbose && !getConverged() ) cout<<"VLevenbergMarquardt::minimize(): warning not converged"<<endl;
}
...
void VLevenbergMarquardt::iterate()
{
  ...
  for(unsigned ifit=0; ifit<nf; ifit++) {
    for(unsigned jfit=0; jfit<nf; jfit++) alpha[ifit][jfit] = fAlpha[ifit][jfit];
  ...
  for(unsigned int iparm=0, jparm=0; iparm<getNParam; iparm++) {
    if(fFit[iparm]) {
      ...
      if(getHasLoBound(iparm) && atry[iparm] <= getLoBound(iparm)) atry[iparm] = getLoBound(iparm);
      if(getHasHiBound(iparm) && atry[iparm] >= getHiBound(iparm)) atry[iparm] = getHiBound(iparm);
    }
    else atry[iparm] = VMinimizer::getParam(iparm);
  }
...
bool VLevenbergMarquardt::mrqcof(const vector<double>& param, vector< vector< double > >& alpha, vector<double>& beta, double& fnval)
{
  ...
  for(unsigned iparm=0, ifit=0; iparm < NParam; iparm++) {
	...
	if(fFit[jparm]) alpha[ifit][jfit++] = alpha2[iparm][jparm];
...
{
  fV.resize(fNCol);
  for (int i = 0; i < fNCol; i++) fV[i].resize(fNCol);
...
{
  if(b.size() != unsigned(fNRow) || x.size() != unsigned(fNCol)) return; 
  ...
  for(int jcol=0;jcol<fNCol;jcol++) {
    double s = 0.0;
    if(fW[jcol] > fThresh) {
      for(int irow=0; irow<fNRow; irow++) s += fU[irow][jcol] * b[irow];
      s /= fW[jcol];
    }
    temp[jcol] = s;
  }
  ...
  for(int jcol=0;jcol<fNCol;jcol++) {
    double s = 0.0;
    for(int jjcol = 0;jjcol<fNCol;jjcol++) s += fV[jcol][jjcol] * temp[jjcol];
    x[jcol] = s;
  }
...
vector< vector< double > > VSVD::inverse() const
{
  ...
  ut.resize( fU.size() ); 
  for (unsigned int i = 0; i < fU.size(); i++) ut[i].resize( fU[i].size() );
  ...
  w.resize( fNCol ); 
  for (int i = 0; i < fNCol; i++) w[i].resize( fNCol );
  
  for(int icol = 0; icol < fNCol; icol++)
    if(fabs( fW[icol] ) > thresh) w[icol][icol] = 1 / fW[icol];
    else w[icol][icol] = 0;
...
void VSVD::decompose()
{
  ...
  for(i=0; i<fNCol; i++) {
    ...
    if(i < fNRow) {
      for(k=i; k<fNRow; k++) scale += fabs( fU[k][i] );
      if(scale != 0.0) {
	for(k=i; k<fNRow; k++) {
	  fU[k][i] /= scale;
	  s += fU[k][i] * fU[k][i];
	...
	for(j=l-1; j<fNCol; j++) {
	  for(s=0.0, k=i; k<fNRow; k++) s += fU[k][i] * fU[k][j];
	  f=s/h;
	  for(k=i; k<fNRow; k++) fU[k][j] += f * fU[k][i];
	}
	for(k=i; k<fNRow; k++) fU[k][i] *= scale;
      }
    }
    fW[i]=scale *g;
    g = s = scale = 0.0;
    if(i+1 <= fNRow && i+1 != fNCol) {
      for(k=l-1; k<fNCol; k++) scale += fabs( fU[i][k] );
      if(scale != 0.0) {
	for(k=l-1; k<fNCol; k++) {
	  fU[i][k] /= scale;
	  s += fU[i][k] * fU[i][k];
	...
	h = f*g-s;
	fU[i][l-1] = f-g;
	for(k=l-1; k<fNCol; k++) rv1[k] = fU[i][k]/h;
	for(j=l-1; j<fNRow; j++) {
	  for(s=0.0, k=l-1; k<fNCol; k++) s += fU[j][k] * fU[i][k];
	  for(k=l-1; k<fNCol; k++) fU[j][k] += s * rv1[k];
	}
	for(k=l-1; k<fNCol; k++) fU[i][k] *= scale;
      }
    }
  ...
  for(i=fNCol-1; i>=0; i--) { 
    if(i < fNCol-1) {
      if (g != 0.0) {
	for(j=l; j<fNCol; j++) fV[j][i] = ( fU[i][j]/ fU[i][l] )/g;
	for(j=l; j<fNCol; j++) {
	  for(s=0.0,k=l; k<fNCol; k++) s += fU[i][k] * fV[k][j];
	  for(k=l; k<fNCol; k++) fV[k][j] += s*fV[k][i];
	}
      }
      for(j=l;j<fNCol;j++) fV[i][j] = fV[j][i] = 0.0;
    }
    fV[i][i] = 1.0;
    g=rv1[i];
    l=i;
  ...
  for(i=min(fNRow,fNCol)-1; i>=0; i--) {
    l=i+1;
    g=fW[i];
    for(j=l;j<fNCol;j++) fU[i][j] = 0.0;
    if (g != 0.0) {
      g=1.0/g;
      for(j=l; j<fNCol; j++) {
	for (s=0.0,k=l; k<fNRow; k++) s += fU[k][i] * fU[k][j];
	f=( s / fU[i][i] )*g;
	for(k=i; k<fNRow; k++) fU[k][j] += f * fU[k][i];
      }
      for(j=i; j<fNRow; j++) fU[j][i] *= g;
    } 
    else for(j=i; j<fNRow; j++) fU[j][i] = 0.0;
    ++fU[i][i];
  }
  ...
  for(k=fNCol-1; k>=0; k--) {
	...
	if(fabs(fW[nm]) <= fEps*anorm) break;
      }
      if(flag) {
	c=0.0;
	s=1.0;
	...
	if(z < 0.0) {
	  fW[k] = -z;
	  for(j=0; j<fNCol; j++) fV[j][k] = -fV[j][k];
	}
	break;
      }
      if (its == 29) {
...
void VSVD::reorder() 
{
  ...
  do { inc *= 3; inc++; } while (inc <= fNCol);
  do {
    inc /= 3;
    for(i=inc; i<fNCol; i++) {
      sw = fW[i];
      for(k=0; k<fNRow; k++) su[k] = fU[k][i];
      for(k=0; k<fNCol; k++) sv[k] = fV[k][i];
      j = i;
      while(fW[j-inc] < sw) {
	fW[j] = fW[j-inc];
	for(k=0; k<fNRow; k++) fU[k][j] = fU[k][j-inc];
	for(k=0; k<fNCol; k++) fV[k][j] = fV[k][j-inc];
	j -= inc;
	if(j < inc) break;
      }
      fW[j] = sw;
      for(k=0; k<fNRow; k++) fU[k][j] = su[k];
      for(k=0; k<fNCol; k++) fV[k][j] = sv[k];
    }
  } while (inc > 1);
  for(k=0; k<fNCol; k++) {
    s=0;
    for(i=0; i<fNRow; i++) if( fU[i][k] < 0.) s++;
    for(j=0; j<fNCol; j++) if( fV[j][k] < 0.) s++;
    if(s > (fNRow + fNCol)/2) {
      for(i=0; i<fNRow; i++) fU[i][k] = -fU[i][k];
      for(j=0; j<fNCol; j++) fV[j][k] = -fV[j][k];
    }
  }
}
...
{
  if(thresh>=0.0) fThresh = thresh;
  else fThresh = 0.5*sqrt(double(fNRow+fNCol)+1.0)* fW[0] * fEps;
}
...
{
  if(!fInstance.get())fInstance.reset(new VMinimizerFactory);
...
VMinimizer* VMinimizerFactory::getMinimizer(const VMinimizerFn& fn, const vector<double>& param, unsigned int max_iterations, double tolerance) const
{
  ...
  for(unsigned int i = 0; i<param.size(); i++) err[i] = 0.01 * param[i];
