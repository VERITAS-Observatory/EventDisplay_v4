...
// get mean of all bins around this bin
                    ...
                    for( int ii = i - iwidth; ii <= i+iwidth; ii++ )
                    {
                        for( int jj = j - iwidth; jj <= j+iwidth; jj++ )
                        {
                            ...
                            {
                                if( !bError ) imean += htemp->GetBinContent( ii, jj );
                                else          imean += htemp->GetBinContent( ii, jj ) * htemp->GetBinContent( ii, jj );
                    ...
                    if( z > 0. )
                    {
                        if( !bError ) hs->SetBinContent( i, j, imean / z );
                        else          hs->SetBinContent( i, j, sqrt( imean ) / z );
