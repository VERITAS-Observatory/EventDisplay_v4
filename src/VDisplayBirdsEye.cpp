...
{
    fDebug = false;
    if( fDebug ) cout << "VDisplayBirdsEye::VDisplayBirdsEye" << endl;
    fPlotPaper = false;
...
{
    if( fParameters.size() == fData->getTeltoAna().size() ) fParameters.clear();
...
{
    if( fDebug ) cout << "VDisplayBirdsEye::draw( TPad *iPad )" << endl;
...
// get min/max size or nphotons
    ...
    for( unsigned int i = 0; i < fData->getTeltoAna().size(); i++ )
    {
       ...
       if( iS > 0 && log10( iS ) < iMin ) iMin = log10( iS );
       if( iS > 0 && log10( iS ) > iMax ) iMax = log10( iS );
    }
...
// plot telescopes 
        ...
        if( fData->getReader()->hasLocalTrigger( fData->getTeltoAna()[i] ) )
	{
	    ...
	    if( fData->getImageParameters()->size > 0. ) iLT->SetMarkerColor( 2 );
	    iLT->Draw();
	} 
...
{
    if( fDebug ) cout << "VDisplayBirdsEye::drawTelescopes()" << endl;
...
// telescopes (circles)
        ...
        fElTel[i]->SetR1( scaleX( fTelRad[i]) );
        fElTel[i]->SetR2( scaleY( fTelRad[i]) );
        if( i+1 < 10 )          fElTel[i]->SetFillColor( i+1 );
	else                    fElTel[i]->SetFillColor( 1 );
...
// draw the cores only for succesfull reconstruction (Chi2>=0.)
    ...
    if( fData->getShowerParameters()->fShower_Chi2[iM] >= 0. ) fMarkerCore.back()->Draw();
...
{
    if( fDebug ) cout << " VDisplayBirdsEye::setData( VEvndispData *iData )" << endl;
    fData = iData;
    if( fData->getReader()->isGrisuMC() && fData->getDetectorGeo()->getGrIsuVersion() < 412 ) fMCSign =  -1.;
    else                             fMCSign = -1.;
}
...
{
    if( fDebug ) cout << "VDisplayBirdsEye::setGeometry()" << endl;
...
// get telescopes in shower coordinates
    ...
    for( unsigned int i = 0; i < fData->getTeltoAna().size(); i++ )
    {
        if( fTelPosX[i] > fFieldX - fFieldCentreX ) fFieldX = fTelPosX[i] - fFieldCentreX;
        if( fTelPosY[i] > fFieldY - fFieldCentreY ) fFieldY = fTelPosY[i] - fFieldCentreY;
    }
