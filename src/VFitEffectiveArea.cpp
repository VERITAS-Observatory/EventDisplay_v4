...
void VFitEffectiveArea::makeCanvases()
{
    ...
    hnullLog->SetStats( 0 );
    if( bAMC ) hnullLog->SetXTitle( "log_{10} energy_{MC} [TeV]" );
    else       hnullLog->SetXTitle( "log_{10} energy_{Rec} [TeV]" );
    ...
    hnullLin->SetStats( 0 );
    if( bAMC ) hnullLin->SetXTitle( "log_{10} energy_{MC} [TeV]" );
    else       hnullLin->SetXTitle( "log_{10} energy_{Rec} [TeV]" );
...
void VFitEffectiveArea::getEffectiveArea( double iZe, int az, double index, bool iAMC )
{
    ...
    for( int i = 0; i < nentries; i++ )
    {
        ...
        {
            if( bAMC ) gEff = (TGraphAsymmErrors*)c.gEffAreaMC->Clone();
            else gEff = (TGraphAsymmErrors*)c.gEffAreaRec->Clone();
            iDir->Append( gEff );
...
// combined power law at low energies
    if( x < p[6] ) f = p[0]*TMath::Power(10, (x-p[3])*p[1]) / ( 1 + TMath::Power( 10, (x-p[3])*(p[1]-p[2]) ) );
    else
    {
...
TGraphAsymmErrors* VFitEffectiveArea::getLogGraph()
{
    ...
    if( gEffLog ) gEffLog->Set( gEff->GetN() );
    else          gEffLog = new TGraphAsymmErrors( gEff->GetN() );

    double x, y;
    for( int i = 0; i < gEff->GetN(); i++ )
    {
        gEff->GetPoint( i, x, y );
        if( y > 0. ) gEffLog->SetPoint( i, x, log10( y ) );
        else         gEffLog->SetPoint( i, x, log10( 1. ) );
        x = gEff->GetErrorYhigh( i );
        if( y > 0. ) gEffLog->SetPointEYhigh( i, x/y );
        else         gEffLog->SetPointEYhigh( i, 0. );
        x = gEff->GetErrorYlow( i );
        if( y > 0. ) gEffLog->SetPointEYlow( i, x/y );
        else         gEffLog->SetPointEYlow( i, 0. );
    }
...
void VFitEffectiveArea::write( string ofile )
{
    ...
    if( !f.IsZombie() && fTree ) fTree->Write();
    f.Close();
}
...
void VFitEffectiveArea::smoothGraph( double iOut, int iIter )
{
    ...
    for( int t = 0; t < iIter; t++ )
    {
        for( int i = 1; i < gEff->GetN() - 1; i++ )
        {
            ...
            if( y2 / y1 < (1.-iOut) && y2 / y3 < (1.-iOut ) ) y2 = 0.5 * (y1 + y3);
