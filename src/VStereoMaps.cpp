...
VStereoMaps::VStereoMaps( double i_r0, bool iuc, int iRandomSeed )
{
    fData = 0;
...
bool VStereoMaps::fill( bool is_on, double x_sky, double y_sky, double ze, double erec, int irun, bool ishapecuts )
{
    if( is_on ) return fillOn( x_sky, y_sky, ze, erec, irun, ishapecuts );
    else        return fillOff( x_sky, y_sky, ze, erec, irun, ishapecuts );

    return false;
}


bool VStereoMaps::fillOn( double x, double y, double ze, double erec, int irun, bool ishapecut )
{
    double i_weight = 1.;
...
// model dependent initialisations
//////////////////////////////////
// REFLECTED REGION MODEL
//////////////////////////////////
...
// check if is shower in fiducal area (source region)
...
bool VStereoMaps::fillOff( double x, double y, double ze, double erec, int irun, bool ishapecuts )
{
    if( irun != fInitRun )
    {
        ...
        if( fRunList.fBackgroundModel == eONOFF ) fInitRun = irun;
    }
///////////////////////////////////////
// ON/OFF analysis
    ...
    {
        return fillOn( x, y, ze, erec, irun, ishapecuts );
    }
...
// RINGMODEL
    ...
    {
        return fill_RingBackgroundModel( x, y, ze, erec, irun, ishapecuts );
    }
...
//  REFLECTED REGION MODEL
    ...
    {
        return fill_ReflectedRegionModel( x, y, irun, ishapecuts );
    }
...
// FOVMODEL
    ...
    {
        return fill_FOVBackgroundModel( x, y, ze, erec, irun, ishapecuts );
    }
...
// apply new shape cuts
        ishapecuts = true;
        if( fData->MSCW < fRunList.fTE_mscw_min || fData->MSCW > fRunList.fTE_mscw_max ) ishapecuts = false;
        if( fData->MSCL < fRunList.fTE_mscl_min || fData->MSCL > fRunList.fTE_mscl_max ) ishapecuts = false;
        return fillOn( x, y, ze, erec, irun, ishapecuts );
    }
...
{
    makeTwoDStereo_BoxSmooth( i_xderot, i_yderot, i_weight, f_r0 );
}


void VStereoMaps::makeTwoDStereo_BoxSmooth( double i_xderot, double i_yderot, double i_weight, double ir0 )
{
...
//! i_r0 is the radius of the smoothing box

// bin numbers for current event
    int i_x = hmap_stereo->GetXaxis()->FindBin( i_xderot );
    int i_y = hmap_stereo->GetYaxis()->FindBin( i_yderot );

// get upper and lower limit for loop (add 2 to be save)
    int fn_r0X = int(ir0 / hmap_stereo->GetXaxis()->GetBinWidth( 2 )) + 2;
    int fn_r0Y = int(ir0 / hmap_stereo->GetYaxis()->GetBinWidth( 2 )) + 2;

    int ix_start = i_x - fn_r0X;
    if( ix_start < 0 ) ix_start = 0;
    int ix_stopp = i_x + fn_r0X;
    if( ix_stopp > hmap_stereo->GetNbinsX() ) ix_stopp = hmap_stereo->GetNbinsX();

    int iy_start = i_y - fn_r0Y;
    if( iy_start < 0 ) iy_start = 0;
    int iy_stopp = i_y + fn_r0Y;
    if( iy_stopp > hmap_stereo->GetNbinsY() ) iy_stopp = hmap_stereo->GetNbinsY();
 ...
 *
 *   following Funke 2005
 *
 */
void VStereoMaps::RM_getAlpha( bool iIsOn )
{
    cout << "\t finalizing ring model alpha plots ";
    if( bUncorrelatedSkyMaps ) cout << "(uncorrelated maps)";
    else                       cout << "(correlated maps)";
    cout << endl;
...
// define area which has to be test for this test source (x)
        ...
        if( ix_stopp > i_nbinsX ) ix_stopp = i_nbinsX;
...
// define area which has to be test for this test source (y)
            ...
            if( iy_stopp > i_nbinsY ) iy_stopp = i_nbinsY;
...
// check if event is in bin
                            if( ci == i && cj == j ) i_acc += fAcceptance->getAcceptance( cx, cy );
                        }
                        else
                        {
...
// check if event is inside ring
                        if( cr > i_rL * i_rL && cr < i_rU * i_rU ) i_acc += fAcceptance->getAcceptance( cx, cy );
                    }
                }
            }
// fill the alpha map
...
void VStereoMaps::RE_getAlpha( bool iIsOn )
{
    for( int i = 1; i <= hmap_alpha->GetNbinsX(); i++ )
    {
        for( int j = 1; j <= hmap_alpha->GetNbinsY(); j++ )
        {
            if( iIsOn ) hmap_alpha->SetBinContent( i, j, 1. );
            else
            {
                ...
                else hmap_alpha->SetBinContent( i, j, 0. );
            }
        }
    }
}
...
/*!
  this function might be obsolet (???)
*/
void VStereoMaps::TE_fillAlpha( int irun, double x, double y, bool ishapecuts )
{
    int i_x = hmap_alpha->GetXaxis()->FindBin( x );
    int i_y = hmap_alpha->GetYaxis()->FindBin( y );

    int fn_r0X = int(f_r0 / hmap_stereo->GetXaxis()->GetBinWidth( 2 )) + 2;
    int fn_r0Y = int(f_r0 / hmap_stereo->GetYaxis()->GetBinWidth( 2 )) + 2;

// assume hmap_stereo and hmap_alpha has same binning definition
    int ix_start = i_x - fn_r0X;
    if( ix_start < 0 ) ix_start = 0;
    int ix_stopp = i_x + fn_r0X;
    if( ix_stopp >= hmap_alpha->GetNbinsX() ) ix_stopp = hmap_alpha->GetNbinsX();

    int iy_start = i_y - fn_r0Y;
    if( iy_start < 0 ) iy_start = 0;
    int iy_stopp = i_y + fn_r0Y;
    if( iy_stopp >= hmap_alpha->GetNbinsY() ) iy_stopp = hmap_alpha->GetNbinsY();
    ...
    if( ishapecuts )
    {
        for( int i = ix_start; i < ix_stopp; i++ )
        {
            xbin = hmap_alpha->GetXaxis()->GetBinCenter( i );
            for( int j = ix_start; j < iy_stopp; j++ )
            {
                ybin = hmap_alpha->GetYaxis()->GetBinCenter( j );

                if( (x-xbin)*(x-xbin) + (y-ybin)*(y-ybin) < fRunList.fSourceRadius )
                {
                    hmap_alpha->Fill( xbin, ybin );
                }
            }
        }
    }
...
bool VStereoMaps::fill_ReflectedRegionModel(  double x, double y, int irun, bool ishapecuts )
{
...
//
    ...
    if( i_evDist > fRunList.fmaxradius ) return false;
...
// (GM)  if( hRE_regions ) delete hRE_regions;
        ...
        if( hAuxHisList ) hAuxHisList->Add( hRE_regions );
        else cout << "VStereoMaps::initialize_ReflectedRegionModel() warning: aux list does not exist"  << endl;
    }

    n_ex = (int)vXTOEXCLUDE.size();
...
// test if there are enough off source regions
                        if( (int)x_off.size() >= fRunList.fRE_nMaxoffsource || ( x_off.size() > 10 && (int)x_off.size() >= n_max_RE - 5 ) ) break;
                    }
                    n_r = (int)x_off.size();
...
// number of off source regions
                    ...
                    if( hRE_NRegions ) hRE_NRegions->SetBinContent( i, j, n_r );
                }
            }
...
// get available phase space for theta2 plots
    ...
    {
        char hname[200];
// (GM)      if( hAux_theta2On ) delete hAux_theta2On;
        ...
        if( hAuxHisList ) hAuxHisList->Add( hAux_theta2On );
...
// (GM)      if( hAux_theta2Off ) delete hAux_theta2Off;
        ...
        if( hAuxHisList ) hAuxHisList->Add( hAux_theta2Off );
...
// (GM)      if( hAux_theta2Ratio ) delete hAux_theta2Ratio;
        ...
        if( hAuxHisList ) hAuxHisList->Add( hAux_theta2Ratio );
...
// (take only every second bin)
        ...
        for( int i = 0; i < nxybin; i += 2 )
        {
            ...
            for( int j = 0; j < nxybin; j += 2 )
            {
                ...
                for( unsigned int t = 0; t < getTheta2_length(); t++ ) hAux_theta2On->Fill( getTheta2()[t], getTheta2_weigthREonly()[t] * i_tAcc );

                calculateTheta2( false, x, y );
                for( unsigned int t = 0; t < getTheta2_length(); t++ ) hAux_theta2Off->Fill( getTheta2()[t], getTheta2_weigthREonly()[t] * i_tAcc );
            }
        }
...
bool VStereoMaps::fill_RingBackgroundModel( double x, double y, double ze, double erec, int irun, bool ishapecuts )
{
// at the beginning of each run, get acceptance curve
    if( irun != fInitRun )
    {
...
// check if event is inside an excluded region
    if( fAcceptance->isExcludedfromBackground( x, y ) ) return false;
...
// check if bin is inside fiducial area
                if( sqrt( i_cx*i_cx + i_cy*i_cy ) > fRunList.fmaxradius ) continue;
...
// determine if event is in off region of source region, i.e. inside the ring
    ...
    if( i_cr < i_rU*i_rU && i_cr > i_rL*i_rL ) return true;
...
bool VStereoMaps::initialize_RingBackgroundModel( bool iIsOn )
{
    ...
    if( !iIsOn )
    {
        ...
        if( !bUncorrelatedSkyMaps ) initialize_theta2();
    }
...
// ON
    if( isOn )
    {
        ...
        {
            if( fAcceptance ) iWeight = fAcceptance->getAcceptance( x, y );
            fTheta2[0] = theta2;
            fTheta2_weight[0] = iWeight;
            fTheta2_weightREonly[0] = iWeight;
            fTheta2_length = 1;
        }
        else fTheta2_length = 0;
    }
...
// (if this region is too small, theta2 plots will be contaminated by missreconstructed gammas for strong sources)
            ...
            if( hAux_theta2Ratio ) i_phaseSpaceCorrection = hAux_theta2Ratio->GetBinContent( hAux_theta2Ratio->FindBin( theta2 ) );
            if( i_phaseSpaceCorrection > 0. ) i_phaseSpaceCorrection = 1./i_phaseSpaceCorrection;
...
void VStereoMaps::initialize_ReflectedRegionHistograms()
{
    ...
    else
    {
        ...
        if( hAuxHisList ) hAuxHisList->Add( hRE_NRegions );
    }
    defineAcceptance();
    initialize_theta2();
}
...
{
// (GM) Why?    if( !bUncorrelatedSkyMaps )
    {
        if( hAuxHisList ) hAuxHisList->Delete();
        else              hAuxHisList = new TList();
    }
...
// clean sky plots
// - removing bins which are not completly in fiducial are circle
...
{
    if( !defineAcceptance() ) return false;
...
bool VStereoMaps::fill_FOVBackgroundModel( double x, double y, double ze, double erec, int irun, bool ishapecuts )
{
