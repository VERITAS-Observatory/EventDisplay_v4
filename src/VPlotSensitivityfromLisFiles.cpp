...
// read file and fill data vector
    ...
    while( getline( is, is_line ) )
    {
	 ...
	 {
	    if( is_line.find( "<lg E>" ) < is_line.size() ) bMean = true;
	    continue;
         }
...
// get ID index
   ...
   for( fVarIter = fData[iID]->fVar.begin(); fVarIter != fData[iID]->fVar.end(); fVarIter++ )
   {
      ...
      for( unsigned int i = 0; i < (*fVarIter).second.size(); i++ ) cout << "\t" << (*fVarIter).second[i];
      cout << endl;
   }
   return true;
}
...
TCanvas* VPlotSensitivityfromLisFiles::plot_AllIDs( string iVName, Style_t iLineStyle, Style_t iMarkerStyle, TCanvas *c )
{
   ...
   if( d )
   {
      for( unsigned int i = 1; i < fData.size(); i++ ) plot( iVName, fData[i]->fID, d, iLineStyle, iMarkerStyle );
   }
   return d;
}
...
{
   for( unsigned int i = 0; i < fVarName.size(); i++ ) cout << fVarName[i] << endl;
}
...
void VPlotSensitivityfromLisFiles::compare_cuts()
{
   ...
   {
      cout << fData[i]->fID << "\t";
      if( fData[i]->fVar["ImageTh"].size() > 0 ) cout << fData[i]->fVar["ImageTh"][0];
      cout << "\t";
      if( fData[i]->fVar["BorderTh"].size() > 0 ) cout << fData[i]->fVar["BorderTh"][0];
      cout << "\t";
      if( fData[i]->fVar["Amp"].size() > 0 ) cout << fData[i]->fVar["Amp"][0];
      cout << "\t";
      if( fData[i]->fVar["NPix"].size() > 0 ) cout << fData[i]->fVar["NPix"][0];
      cout << "\t";
      if( fData[i]->fVar["NTel"].size() > 0 ) cout << fData[i]->fVar["NTel"][0];
      cout << "\t";
      cout << endl;
   }
}
