...
{
    if( fDebug ) cout << "VBFDataReader::~VBFDataReader()" << endl;
    if (pack!=NULL)
    {
        delete pack;
    }
...
{
    if( fDebug ) cout << "VBFDataReader::getNextEvent() " << endl;
    bool bSimulations = false;

    try
    {
        if( fDebug ) cout << "bool VBFRawDataReader::getNextEvent() " << index << endl;
        for (;;)
        {
            ...
            delete old_pack;
            if( fDebug ) cout << "\t VBFRawDataReader::getNextEvent(): index " << index << endl;
...
// get eventnumber from simulation package
            ...
            if (pack->hasArrayEvent())
            {
                ...
                if( fDebug )
                {
                    ...
                    if( ae->hasEventNumber() ) cout << ", array package eventnumber " << ae->getEventNumber();
...
// check if event for requested telescopes are available
                ...
                for( unsigned int i = 0; i < getTeltoAna().size(); i++ )
                {
                    if( getTeltoAna()[i] < ae->getPresentTelescopes().size() )
                    {
                        ...
                        {
                            if( getTeltoAna()[i] < ib.size() ) ib.set( getTeltoAna()[i], 1 );
                        }
                    }
                    setEventStatus( (unsigned int)ib.to_ulong() );
                }
                if( fDebug ) cout << "\t VBFRawDataReader::getNextEvent(): hasArrayEvent, complete? " << ib.to_ulong() << " " << bComplete << endl;
                ...
                {
                    for( unsigned int i = 0; i < fEvent.size(); i++ ) fEvent[i] = 0;
                    return false;
                }
                ...
                {
                    fArrayTrigger = true;
                    if( fDebug ) cout << "\t VBFRawDataReader::getNextEvent(): hasTrigger" << endl;
                    at=ae->getTrigger();
                }
                else
                {
            ...
            if( fDebug )
            {
                ...
                if( ae ) cout << ae->getNumEvents();
                cout << "\t" << gotOneEv;
                cout << endl;
            }

            if ( !gotOneEv && !bSimulations ) continue;

            if( fDebug ) cout << "bool VBFRawDataReader::getNextEvent() (end)" << endl;
...
{
    if( fDebug ) cout << "VBFDataReader::hasLocalTrigger" << endl;
