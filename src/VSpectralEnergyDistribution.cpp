...
// read ascii file
    ...
    while(  getline( is, is_line ) )
    {
        ...
        if( is_temp == "MJD" )
        {
            ...
            else i_pF_temp.Color = 1 + z;
            if( !is_stream.eof() )
            {
                is_stream >> is_temp;
                i_pF_temp.Marker = atoi( is_temp.c_str() );
            }
            else i_pF_temp.Marker = 20 + z;
            if( !is_stream.eof() )
            {
        ...
        else
        {
            ...
            is_stream >> is_temp;
            i_pF_temp.flux_error_down_ergscms.push_back( atof( is_temp.c_str() ) );
            if( TMath::Abs( i_pF_temp.flux_error_down_ergscms.back() ) < 1.e-16 ) i_pF_temp.flux_error_down_ergscms.back() = 0.;
            if( is_stream.eof() )
            {
                i_pF_temp.flux_error_up_ergscms.push_back( atof( is_temp.c_str() ) );
                if( TMath::Abs( i_pF_temp.flux_error_up_ergscms.back() ) < 1.e-16 ) i_pF_temp.flux_error_up_ergscms.back() = 0.;
            }
            else
            {
                is_stream >> is_temp;
...
// no upper limits for < X-ray frequencies (work around)
            ...
            {
                double iMult = 1.;
                if( i_pF_temp.energy_eV_max.back() - i_pF_temp.energy_eV_min.back() > 0. ) iMult = i_pF_temp.energy_eV.back() / ( i_pF_temp.energy_eV_max.back() - i_pF_temp.energy_eV_min.back() );
...
// use data from one instrument only
        if( is_line.substr( 0, 2 ) == "NO" ) break;

        if( z == 0 || is_line.substr( 0, 2 ) == "NO" )
        {
            if( z != 0 ) i_photonFlux.push_back( i_pF_temp );
            i_pF_temp.name = name;
            i_pF_temp.MJD_min = MJD_min;
            i_pF_temp.MJD_max = MJD_max;
            i_pF_temp.Marker = imarker;
...
// energy bin width (not used)
        if( iFormat == 0 ) is_stream >> is_temp;
// fluxes
        if( iFormat == 0 )
        {
            is_stream >> is_temp;
...
// set up the data structure
    ...
    while(  getline( is, is_line ) )
    {
        ...
        is_stream >> is_temp;
        if( i_pF_temp.MJD_min > 2400000. ) i_pF_temp.MJD_min -= 2400000.5;
        double MJD_min = atof( is_temp.c_str() );
        if( MJD_min > 2400000. ) MJD_min -= 2400000.5;
        is_stream >> is_temp;
...
bool VSpectralEnergyDistribution::readPhotoMetricBands( string ifile, bool iPrint )
{
    ...
    if( iPrint ) cout << "VSpectralEnergyDistribution: reading photometric bands from " << ifile << endl;
    ...
    while(  getline( is, is_line ) )
    {
        ...
        if( TMath::Abs( iTemp.fF0_UKIRT_Jy ) < 1.e-8 ) iTemp.fF0_UKIRT_Jy = 0.;
...
// micrometer to meter
    ...
    if( fDebug ) cout << "getEffectiveWavelength: Wavelength: [m] " << iWL << " " << iband << " " << iUnit << endl;
    ...
    if( iUnit == "eV" )
    {
        ...
        if( fDebug ) cout << "\t eV " << iWL << endl;
        return iWL;
    }
    else if( iUnit == "Hz" )
    {
        iWL = TMath::C() / iWL;
        if( fDebug ) cout << "\t Hz " << iWL << endl;
        return iWL;
    }
    else
    {
...
double VSpectralEnergyDistribution::getFluxfromMagnitude( double magnitude, string iband, string system )
{
    ...
    for( unsigned int i = 0; i < fPhotoMetricBand.size(); i++ )
    {
        if( fPhotoMetricBand[i].fBand == iband )
        {
            if( system == "CIT" )   f_0 = fPhotoMetricBand[i].fF0_CIT_Jy;
            if( system == "UKIRT" ) f_0 = fPhotoMetricBand[i].fF0_UKIRT_Jy;
            break;
        }
    }
    if( f_0 <= 0. ) return 0.;
...
TCanvas* VSpectralEnergyDistribution::plot( TCanvas *c, int bLegend, int canvas_x, int canvas_y, bool bPlotErrorX, bool bPlotName )
{
    ...
    else if( bLegend == 2 ) iL = new TLegend( 0.31, 0.77, 0.5, 0.85 );
    else if( bLegend == 3 ) iL = new TLegend( 0.5, 0.73, 0.95, 0.95 );

// set up graphs and draw them
    for( unsigned int i = 0; i < fSpectralFlux.size(); i++ )
    {
        if(  fSpectralFlux[i].size() == 0 ) continue;
...
// (terrible coding...)
    ...
    for( unsigned int i = 0; i < itemp.size(); i++ ) cout << itemp[i] << endl;
