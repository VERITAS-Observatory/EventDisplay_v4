...
// define tree
    ...
    if( fTreeDet->GetBranchStatus( "FOV" ) ) fTreeDet->SetBranchAddress( "FOV", &fFOV );
    ...
    if( fTreeDet->GetBranchStatus( "NGains" ) ) fTreeDet->SetBranchAddress( "NGains", &nGains );
    if( fTreeDet->GetBranchStatus( "HiLoScale" ) ) fTreeDet->SetBranchAddress( "HiLoScale", &fHiLoScale );
    if( fTreeDet->GetBranchStatus( "HiLoThreshold" ) ) fTreeDet->SetBranchAddress( "HiLoThreshold", &fHiLoThreshold );
    if( fTreeDet->GetBranchStatus( "HiLoOffset" ) ) fTreeDet->SetBranchAddress( "HiLoOffset", &fHiLoOffset );
    ...
    if( fTreeDet->GetBranchStatus( "AreaTube_m2" ) ) fTreeDet->SetBranchAddress( "AreaTube_m2", fATube_m2 );
    if( fTreeDet->GetBranchStatus( "TubeOFF" ) )    fTreeDet->SetBranchAddress( "TubeOFF", fTubeOFF );
    if( fTreeDet->GetBranchStatus( "NMirrors" ) )   fTreeDet->SetBranchAddress( "NMirrors", &fNMirrors );
    if( fTreeDet->GetBranchStatus( "MirrorArea" ) ) fTreeDet->SetBranchAddress( "MirrorArea", &fMirrorArea );
...
// FITS record: telescope array
    ...
    for( unsigned int i = 0; i < fTreeDet->GetEntries(); i++ )
    {
       ...
       fieldofview      = fFOV;
       npix    		= nPixel;
       if( fRTubeDeg[0] > 0. ) pix_size = 2.*fRTubeDeg[0];
