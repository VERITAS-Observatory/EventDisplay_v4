...
// same zenith angle, don't interpolate
    if( TMath::Abs( ze1 - ze2 ) < 1.e-3 ) return (w1+w2)/2.;
...
// check for valid size
    if( iSize > 0. ) iSize = log10( iSize );
    else             return -99.;

// check for valid length
    float iWidthOLenght = -99.;
    if( iLength > 0. ) iWidthOLenght = iWidth / iLength;
...
// require at least 5 events per bin
		  if( fData->h2D_DispTableN->GetBinContent( x_bin, y_bin ) > 5. )
		  {
               ...
               else cout << "VDispTableAnalyzer::evaluate: error no data histograms found" << endl;
            }
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
// require at least 5 events per bin
    ...
    else cout << "VDispTableAnalyzer::evaluate: error: no data  tree for disp analysis" << endl;
...
// (number of possible combinations is 2^NTOT_MAX )
     ...
     if( x.size() > NTOT_MAX ) iNTel_max = NTOT_MAX;
