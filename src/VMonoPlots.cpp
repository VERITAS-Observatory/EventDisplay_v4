...
VMonoPlots::VMonoPlots( bool ison, CData* tree, string i_hsuffix, VAnaSumRunParameter* ipara, unsigned int itelid )
{
	...
	if( !fIsOn )
	{
		i_off = fRunPar->fRunList[0].fPairOffset / 60. / 24.*360.*M_PI / 180.0;
	}
...
double VMonoPlots::fillHistograms( unsigned int itelid, int irun )
{
	if( fDebug )
	{
		cout << "VMonoPlots::fillHistograms telescope " << itelid << endl;
	}
	if( fData->fChain == 0 )
	{
		return -1.;
	...
	if( fIsOn )
	{
		cout << "\t on run,  total number of entries in data chain " << nentries << "\t" << irun << endl;
	}
	else
	{
		cout << "\t off run, total number of entries in data chain " << nentries << "\t" << irun << endl;
	}
	...
	if( fDebug )
	{
		cout << "VMonoPlots::fillHistograms number of events passing cuts: " << i_count << endl;
	}
	return i_count;
}
