 ...
 *
 */
TCanvas* VPlotAnasumHistograms::plot_skyPlots( string iPlotMode, bool iSingleCanvases, double excess_min, double excess_max, double sig_min, double sig_max )
{
    char hname[200];
    char htitle[200];
...
TCanvas* VPlotAnasumHistograms::plot_significanceDistributions( double rmax, double rSource, double xmin, double xmax, TCanvas* cCanvas, bool regioncodeflag )
{
    ...
    TH1D* hsig_1DExcluded = get_Bin_Distribution( hmap_stereo_sig, fRunNumber, rmax, rSource, false, hmap_stereo_on, iN, v_x, v_y, v_r1, v_r2, v_theta );
...
void VPlotAnasumHistograms::plot_skyPlots_perRun( string iHistoName, double rmax, double zmin, double zmax, double rSource, int nentries, unsigned int nstart )
{
    ...
    if(( unsigned int )( nentries + nstart ) > getRunList().size() && getRunList().size() > nstart )
    {
    ...
    for( unsigned int i = nstart; i < ( unsigned int )nentries + nstart; i++ )
    {
        ...
        if( hsig_1DAll->GetMaximum() > hsig_1D->GetMaximum() )
        {
            hsig_1D->SetMaximum( hsig_1DAll->GetMaximum() * 1.2 );
        }
