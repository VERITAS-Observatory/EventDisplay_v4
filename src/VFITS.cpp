...
bool VFITS::readAnasumFile(bool iPrint )
{
   ...
   if (fWriteOneFile == true)
   {
     ...
     if( fits_create_file(&fptr, fFileName.c_str(), &status) ) return printerror( status );
     if ( fits_create_img(fptr,  DOUBLE_IMG, 0, 0, &status) ) return printerror( status ); 
...
bool VFITS::writeCumSignificance(bool iPrint)
{
  cout<<" Write cumulative significance to fits file"<<endl;
    ...
    if (iPrint) cout<<" Calculated the cumulative significance"<<endl;

    writeTGraphFits(gCumSig,"cumulativeSignificance", "time", "cumSig", "min", "sigma", iPrint);
    if (iPrint) cout<<" transformed cumulative significance into FITS-table"<<endl;
...
// get all histograms
    ...
    if (iPrint) cout<<" Got necessary histograms from file"<<endl;
...
bool VFITS::writeLightCurve(bool iPrint)
{
  ...
  if (iPrint) cout<<" Got the rate TGraphErrors"<<endl;
...
bool VFITS::writeEnergySpectrum(bool iPrint)
{
  ...
  if (iPrint) cout<<" define VEnergySpectrum variable"<<endl;
  //combine runs
  fEspec.combineRuns();
  if (iPrint) cout<<" combine all runs"<<endl;

  //calculate energy Spectrum
  fEspec.calculateDifferentialFluxes();
  TGraphErrors* gEspec = (TGraphErrors*)fEspec.getEnergySpectrumGraph(); 
   if (iPrint) cout<<" Got the energy spectrum TGraphErrors"<<endl;  

   if (gEspec ==0) return false ;
  ...
  if (iPrint) cout<<" transformed EnergySpectrum into FITS-table"<<endl;
  return true;
}
...
bool VFITS::writeSignificanceSkyMap(bool iPrint)
{
  ...
  if (iPrint) cout<<" Got the SignificanceSkyMap TH2D"<<endl;  

  //Transform to Fits
  createImageFitsFile(hSkyMap , "SignificanceMap", iPrint);
  if (iPrint) cout<<" transformed SkyMap image into FITS-table"<<endl;
  return true;
}
...
bool VFITS::writeExcessSkyMap(bool iPrint)
{
  ...
  if (iPrint) cout<<" Got the ExcessSkyMap TH2D"<<endl;  
...
{ 
  ...
  */
   ...
   if (iPrint) cout<<" Write Vector of TH1Ds to fits file ..." << endl;  
    ...
    for( int i = 0; i < vhist[0].first->GetNbinsX(); i++ ) {
	 ...
	 if (vhist[0].first->GetNbinsX() != vhist[k].first->GetNbinsX()) {
         ...
         if (vhist[k].second.find("x",0) < 10) runValues.push_back(vhist[k].first->GetBinCenter(i)); //1.row = X values
         if (vhist[k].second.find("w",0) < 10) runValues.push_back(vhist[k].first->GetBinWidth(i)); //2.row = bin width
         if (vhist[k].second.find("y",0) < 10) runValues.push_back(vhist[k].first->GetBinContent(i)); //3.row = Y values
         if (vhist[k].second.find("e",0) < 10) runValues.push_back(vhist[k].first->GetBinError(i)); //4.row = Y Error   
				     
      }
      if (i == 0) size = int(runValues.size());
      table.push_back(runValues);
    }

    //change column names if they are equal
    if (iPrint) cout<<"   Size of table columns = "<<size<<endl;
...
int VFITS::writeTH1DFits(TH1D* h,string DiagName, string x_name, string y_name, string x_unit, string y_unit, bool iPrint)
{
    ...
    if (iPrint) cout<<"   Got X and Y values from histogram and stored them in a table"<<endl;
...
int VFITS::writeTGraphFits(TGraph* g,string DiagName, string x_name, string y_name, string x_unit, string y_unit, bool iPrint)
{
    ...
    if (iPrint) cout<<"   Got X and Y values from TGraph and stored them in a table"<<endl;
...
int VFITS::createTableFitsFile(vector< vector<double> > Table , char *ttype[] ,char *tunit[], char *tform[], string DiagName, bool iPrint)
{
    ...
    if (fWriteOneFile == false)
    {
      ...
      if ( fits_create_img(fptr,  DOUBLE_IMG, 0, 0, &status) ) return printerror( status ); 
      writeFITSInfo( iPrint);
      if( iPrint ) cout << "       Created FITS file: "<<fFileName<< endl;
      //for (int j = 0; j<nCol; j++) { cout<<"tForm["<<j<<"] = "<<tform[j]<<endl;}
    }

    if( fits_create_tbl(fptr, BINARY_TBL, nRows, nCol, ttype, tform, tunit,DiagName.c_str() , &status) ) return printerror( status );
    ffghdn(fptr, &hdunum);
    if( iPrint ) cout << "      Created BinaryTable ..."<< endl;

    for (int k = 0; k < nCol; k++){
       for( int i = 0; i < nRows; i++ ) {
         if(fits_write_col(fptr, TDOUBLE, k+1, i+1, 1, 1, &Table[i][k] , &status)) return printerror( status );
         if( iPrint ) cout << "        Write Table["<<i<<"]["<<k<<"] = "<<Table[i][k]<<endl;
       }
       if( iPrint ) cout << "      Write "<<k<<".Column in BinaryTable ..."<< endl;
    }
    if (fWriteOneFile == false)
    {
      if(fits_close_file(fptr, &status))return printerror( status ) ; 
      if( iPrint ) cout << "      Close FITS file: "<<fFileName<< endl;
    }
    return hdunum;
}
...
int VFITS::createImageFitsFile(TH2D *hSkyMap , string DiagName, bool iPrint)
{
    ...
    for( int ii=1; ii < naxes[1]; ii++ ) array[ii] = array[ii-1] + naxes[0];
    ...
    if ( fits_create_img(fptr,  bitpix, naxis, naxes, &status) ) return printerror( status );
    ffghdn(fptr, &hdunum);
    if( iPrint ) cout << "      Created image ..."<< endl;
    ...
    for( int a = 0; a < naxes[0];a++)
    {
        for( int b = 0; b < naxes[1];b++)
        {
            ...
            if (array[a][b] < -9000) array[a][b] = NAN;
        }
    }
    ...
    if ( fits_write_img(fptr, TDOUBLE, fpixel, nelements, array[0], &status) ) return printerror( status );
...
bool VFITS::writeFITSInfo( bool iPrint)
{
    ...
    if( iPrint ) cout << "--> current date is: " << DD << MM << YYYY << "   time:" << HH << ":" << MN <<":" <<SS << endl;
...
bool VFITS::writeFITSFile(bool iPrint)
{
  if (fWriteOneFile == true)
  {
    int status = 0;
    if(fits_close_file(fptr, &status))return printerror( status ) ; 
    if( iPrint ) cout << "Close FITS file "<< endl;
  }
...
bool VFITS::mergeColumns(fitsfile *fPtr, vector<int> hdunums, vector<vector <int> > columns, int nRows, bool iPrint)
{
 ...
 char *tForm[10];
  ...
  for ( unsigned int i = 0; i < hdunums.size(); i++){
     if (fits_movabs_hdu(fPtr, hdunums[i]-t, &hdutype, &status)) return printerror( status ) ;
     if (iPrint) cout<<"  At hdunums["<<i<<"] - t = "<< hdunums[i]-t<<endl;
     for (unsigned int j = 0; j < columns[i].size(); j++){
       if (iPrint) cout<<"   At columns["<<i<<"]["<<j<<"] = "<< columns[i][j]<<endl;
       sprintf(tname,"TUNIT%d",columns[i][j]);
       if (fits_read_key(fPtr,TSTRING,tname,value+d, comment, &status)) return printerror( status ) ;
       tUnit[p] = value+d;
       d=d+30;
       ...
       for ( int k = 0; k< nRows; k++){
         double test = 0.;
         if (fits_read_col(fPtr, TDOUBLE, columns[i][j] , k+1, 1, 1, 0, &test, &anynul, &status) ) return printerror( status );
         if (iPrint) cout<<"     OutPut of table at Row "<<k<<" and column "<<j<<" = "<<test<<endl;
         rows.push_back(test);	     
       }
       table.push_back(rows);
       p++;
     }
     if (fits_delete_hdu(fPtr, &hdutype, &status)) return printerror( status );
     t++;
  }
  ...
  for (int g = 0; g < p; g++) {
    ...
    for (int h = g; h < p; h++){
      if ( h == g) continue;
      string type2(tType[h]);
      if ( type1 == type2)  sprintf(tType[h], "%s_%d",tType[h],f++); 
    }
  }
    ...
    for (unsigned int k = 0; k< table[1].size(); k++){
	 ...
	 if (iPrint) cout<<"     OutPut of Table2 at Row "<<k<<" and column "<<j<<" = "<<table[j][k]<<endl;
