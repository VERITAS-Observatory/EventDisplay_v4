...
// define the time vector
    ...
    if( fDebug ) cout << "END: VPedestalCalculator::initialize " << endl;
...
// fill the tree
    if( telID < fTree.size() && fTree[telID] ) fTree[telID]->Fill();
...
// temporary vectors
    ...
    {
        if( fTimeVec[telID] == 0 ) fTimeVec[telID] = t;
///////////////////////////////////////////////////////
///////////////////////////////////////////////////////
// end of a time slice
...
// loop over all summation windows
                                ...
                                if( chanID < fpedcal_n[telID].size() && w < fpedcal_n[telID][chanID].size() )
				{
				   ...
				   fpedcal_mean2[telID][chanID][w] += i_tr_sum * i_tr_sum;
                                ...
                                {
                                    cout << "ERROR : " << telID << " " << fpedcal_n.size() << endl;
                ...
                catch(...)
                {
                    if( getDebugLevel() == 0 )
                    {
                        cout << "VPedestalCalculator::doAnalysis(), index out of range (fReader->getHitID) " << i << "(Telescope " << getTelID()+1 << ", event " << getEventNumber() << ")" << endl;
                        setDebugLevel( 1 );
                    }
                    continue;
                }
