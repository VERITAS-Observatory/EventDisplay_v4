...
// define run parameter, load list of runs
    ...
    if( fAnalysisRunMode != 1 )
    {
        ...
        else                                            fRunPara->getWobbleOffsets( fDatadir );
    }
    fNumTels = fRunPara->getMaxNumberofTelescopes();
    fTelOffset = 0;
    if( fNumTels == 1 ) fTelOffset = i_singletel;
...
// (rate plot times are relevant for ON runs only)
    ...
    if( fAnalysisType == 0 || fAnalysisType == 1 || fAnalysisType == 5 )
    {
        for( int i = 0; i < fNumTels; i++ ) fMonoRatePlots.push_back( new VRatePlots( fRunPara, fStereoOn->getRunMJD() ) );
...
// print table with all results
    if( fRunSummary ) fRunSummary->print();
}
...
// fill on and off histograms and sky maps
    if( onrun  == -1 ) fStereoOn->fillHistograms( icounter, onrun, -1.e3, 1.e3, -1. );
    else               fStereoOn->fillHistograms( icounter, onrun, fRunAzMinOn[onrun], fRunAzMaxOn[onrun], fRunPedVarsOn[onrun] );

    if( offrun == -1 ) fStereoOff->fillHistograms( icounter, offrun, -1.e3, 1.e3, -1. );
    else               fStereoOff->fillHistograms( icounter, offrun, fRunAzMinOff[offrun], fRunAzMaxOff[offrun], fRunPedVarsOff[offrun] );
...
// calculate and print mean elevation and raw rate
    ...
    else
    {
        ...
        cout << "\t mean azimuth: " << fMeanAzimuthOn << " (ON), " << fMeanAzimuthOff << " (OFF)" << endl;
...
// normalization
    ...
    if( i_sig != 0. ) cout << i_rate/i_sig << " gammas/min" << endl;
    else            cout << 0. << " gammas/min" << endl;
...
// energy differential spectra
    makeEnergySpectrum( onrun, fstereo_onoff->getEnergyHistograms(), iexp_on );
    makeEnergySpectrum( onrun, fStereoOn->getEnergyHistograms(), iexp_on );
    makeEnergySpectrum( onrun, fStereoOff->getEnergyHistograms(), iexp_off );
...
// finalize and write everything to disk
    ...
    if( onrun != -1 ) fStereoOff->writeDebugHistograms();
...
// fill all histograms and maps
        ...
        double i_rate = 0.;
        if( iexp_on > 0. ) i_rate = ( i_nevts_on - i_norm * i_nevts_off ) * 60. / iexp_on;
        ...
        if( irunon != -1 )
        {
            if( i < (int)fMonoRatePlots.size() ) fMonoRatePlots[i]->fill( irunon, i_sig, hMoSig->GetMaximum(), i_nevts_on, i_nevts_off*i_norm, i_rate );
        }
...
void VAnaSum::makeEnergySpectrum( int ionrun, TList* l, double it )
{
    ...
    {
// skip counting and weight histograms
        string itemp = h->GetName();
        if( itemp.find( "herecWeights" ) < itemp.size() ) continue;
        if( itemp.find( "hLinerecWeights" ) < itemp.size() ) continue;
        if( itemp.find( "Counts" ) < itemp.size() ) continue;
        if( itemp.find( "EffectiveArea" ) < itemp.size() ) continue;

//////////////////////
// for combined runs scale flux histograms by number of runs
        if( ionrun == -1 )
        {
            double inorm = (double)fRunPara->fRunList.size();
...
// scale histograms from m^2 to cm^2
        h->Scale( 1./100./100. );
...
// fill results tree
    ...
    if( onrun != -1 ) fRunSummary->MJDOn = fStereoOn->getMJD( onrun );
    ...
    if( onrun != -1 ) fRunSummary->fWobbleWest = fStereoOn->getWobbleWest();
    else              fRunSummary->fWobbleWest = 0.;
    if( onrun != -1 ) fRunSummary->fNTel = fRunPara->fMapRunList[onrun].fNTel;
    else              fRunSummary->fNTel = 0;
    fRunSummary->tOn = iexp_on;
    fRunSummary->tOff = iexp_off;
    if( onrun != -1 ) fRunSummary->elevationOn = fStereoOn->getMeanElevation();
    else              fRunSummary->elevationOn = fMeanElevationOn;
    if( onrun != -1 ) fRunSummary->elevationOff = fStereoOff->getMeanElevation();
    else              fRunSummary->elevationOff = fMeanElevationOff;
    if( onrun != -1 ) fRunSummary->azimuthOn = fRunAzMeanOn[onrun];
    else              fRunSummary->azimuthOn = fMeanAzimuthOn;
    if( onrun != -1 ) fRunSummary->azimuthOff = fRunAzMeanOff[offrun];
    else              fRunSummary->azimuthOff = fMeanAzimuthOff;
    if( onrun != -1 )
    {
        if( iexp_on > 0. )  fRunSummary->RawRateOn = fStereoOn->getRawRate()/iexp_on;
        else                fRunSummary->RawRateOn = 0.;
        if( iexp_off > 0. ) fRunSummary->RawRateOff = fStereoOff->getRawRate()/iexp_off;
        else                fRunSummary->RawRateOff = 0.;
    }
    else
    {
    ...
    if( onrun != -1 ) fRunSummary->pedvarsOn = fRunPedVarsOn[onrun];
    else              fRunSummary->pedvarsOn = fMeanPedVarsOn;
    if( onrun != -1 ) fRunSummary->pedvarsOff = fRunPedVarsOff[offrun];
    else              fRunSummary->pedvarsOff = fMeanPedVarsOff;
    ...
    double i_tnorm = 1.;
    if( fRunSummary->tOff > 0. ) i_tnorm = fRunSummary->tOn / fRunSummary->tOff;
    if( fRunSummary->tOn > 0. ) fRunSummary->RateE = sqrt( i_nevts_on + i_tnorm * i_tnorm * i_norm_alpha * i_norm_alpha * i_nevts_off ) / fRunSummary->tOn * 60.;
    else                        fRunSummary->RateE = 0.;
    fRunSummary->RateOff = i_rateOFF;
    if( fRunSummary->tOff > 0. ) fRunSummary->RateOffE = sqrt( i_tnorm * i_tnorm * i_norm_alpha * i_norm_alpha * i_nevts_off ) / fRunSummary->tOff * 60.;
    else                         fRunSummary->RateOffE = 0.;
    if( onrun != -1 )
    {
    ...
    {
        if( iexp_on > 0. ) fRunSummary->DeadTimeFracOn = fMeanDeadTimeOn / iexp_on;
        if( iexp_off > 0. ) fRunSummary->DeadTimeFracOff = fMeanDeadTimeOff / iexp_off;
    }
    if( fstereo_onoff )
    {
...
double VAnaSum::getAzRange(int i_run, string i_treename, double &azmin, double &azmax )
{
    ...
    int i_v = 0;
    if( fRunPara )   i_v = fRunPara->getEVNDISP_TREE_VERSION( i_tree );
    else             i_v = VGlobalRunParameter::getEVNDISP_TREE_VERSION( i_tree );
    double telaz[4];
    unsigned int ImgSel = 0;
    ULong64_t ImgSel_64 = 0;
    if( i_v < 6 ) i_tree->SetBranchAddress( "ImgSel", &ImgSel );
    else          i_tree->SetBranchAddress( "ImgSel", &ImgSel_64 );
    ...
    {
        i_tree->GetEntry(i);
        if( i_v < 6 ) ImgSel_64 = (ULong64_t)ImgSel;
    ...
    if( azmin > 180. ) azmin -= 360.;
    if( azmax > 180. ) azmax -= 360.;
