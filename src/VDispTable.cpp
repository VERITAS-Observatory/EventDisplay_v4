/*! \file VDispTable.cpp
    ...
    Revision $Id: VDispTable.cpp,v 1.1.2.6.2.7.4.2 2011/02/07 11:05:25 gmaier Exp $
...
bool VDispTable::terminate()
{
    ...
    if( fTableFile ) fTableFile->Close();

    return true;
}


void VDispTable::addAzBin( float iMin, float iMax, bool iPe )
{
...
bool VDispTable::isGoodEvent( Ctpars *c )
{
   ...
   if( c->ntubes <= fQC_Ntubes_min ) return false;
   if( c->size <= fQC_Size_min )     return false;
   if( c->length <= fQC_Length_min ) return false;
   if( c->loss >= fQC_Loss_max )     return false;
...
// loop over all events for this telescope
	if( iNentries < 0 ) iNentries = c->fChain->GetEntries();
	if( iNentries > c->fChain->GetEntries() ) iNentries = c->fChain->GetEntries();
...
// dispPhi is always between 0 and 90 deg -> estimation of error of major axis
                ...
                if( dispPhi > TMath::PiOver2() * 3. ) dispPhi  = 2.*TMath::Pi() - dispPhi;
                else if( dispPhi > TMath::Pi() )      dispPhi -= TMath::Pi();
                else if( dispPhi > TMath::PiOver2() ) dispPhi  = TMath::Pi() - dispPhi;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    if( i_meanPedvarsN > 0. ) i_meanPedvars /= i_meanPedvarsN;
