...
VLombScargle::VLombScargle()
{
   ...
   vector< double > iL;          vector< int > iN;
   iL.push_back( 0.95 );         iN.push_back( 2 );
   iL.push_back( 0.99 );         iN.push_back( 2 );  
   iL.push_back( 0.9999 );       iN.push_back( 4 );
   iL.push_back( 0.999999 );     iN.push_back( 6 );
...
// shuffle light curve for toy MC
	   if( iShuffle && fRandom ) k = fRandom->Integer( fLightCurveData.size() );
...
{
   if( !fPeriodigramCanvas ) return;
   if( !fPeriodigramHisto  ) return;
...
// trials = number of frequencies
   ...
   for( unsigned int i = 0; i < fProbabilityLevels.size(); i++ )
   {
	   ...
	   i_L->SetLineStyle( 2 );
	   if( iPlotinColor ) i_L->SetLineColor( i+1 );
	   i_L->Draw();
...
// 2D histogram for counting
   double y_max = 1000.;
   if( fPeriodigramHisto->GetMaximum() > 0. ) y_max = fPeriodigramHisto->GetMaximum();
...
// shuffle light curves and fill histogram
   ...
   {
	if( i % 500 == 0 ) cout << "filling MC cycle " << i << endl;
...
// calculate probability levels
   ...
   for( unsigned int i = 0; i < fProbabilityLevels.size(); i++ )
   {
      ...
      if( iPlotinColor ) setGraphPlottingStyle( iG, i+1, 1., 1, 1., 0, 3 );
      else               setGraphPlottingStyle( iG );
      unsigned int z = 0;
...
void VLombScargle::plotPeriodigram( string iXTitle, string iYTitle, bool bLogX )
{
    ...
    if( !fPeriodigramCanvas )
    {
       ...
       if( bLogX ) fPeriodigramCanvas->SetLogx( 1 );
    ...
    if( hLS )
    {
       if( iXTitle.size() > 0 ) hLS->SetXTitle( iXTitle.c_str() );
       if( iYTitle.size() > 0 ) hLS->SetYTitle( iYTitle.c_str() );
