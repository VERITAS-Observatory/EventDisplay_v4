...
void VModel3DFn::setModel3DFn( const vector< vector< double > >& locTel3D, const vector<double>& param )
{
  ...
  // cout<<"SET: "<<fSel<<" "<<fSaz<<" "<<fXcore<<" "<<fYcore<<" "<<fSmax<<" "<<fsigmaL<<" "<<fsigmaT<<" "<<fNc <<endl; //TEST

  /// azimuth from -180 to 180 ///
  if( fSaz < -180. ) fSaz += 360.;
  if( fSaz >  180. ) fSaz -= 360.;
  ...
  for (unsigned int i = 0; i < fNdim3D; i++) {
    for (unsigned int j = 0; j < fNTel3D; j++) {
      fxB[i][j] = fxBo[i] - locTel3D[i][j]; //CHECK THIS !!
    }
  }
...
//// Lemoine-Goumard 2006, equation 5 //////////////////////////
  ...
  double u = dot( fs[0], fs[1], fs[2], ipX, ipY, ipZ ); //CHECK
  ...
  /// double C = 1. - freq( Cx ); // doesn't seem to make much difference (very close to 1.0)?
  /// double Eq5t1 = (Nc * C) / (2.*TMath::Pi() * sigmaU * fsigmaT); //Original

  double Eq5t1 = (Nc) / (2.*TMath::Pi() * sigmaU * fsigmaT); //TEST without "C"
  ...
  /// replace ((90. - fSel)*TMath::Pi())/180. with tSzen
  /// replace (fSaz*TMath::Pi())/180. with tSaz
...
// void VModel3DFn::calcTESTFn( unsigned int iTel, unsigned int iPix, const double ipX, const double ipY, const double ipZ, double& val) const
// {
  /// if(iPix == 100) cout<<"TEST flocTel3D[1][iTel] = " << flocTel3D[1][iTel] <<endl;

  ///  val = exp(fNc - 0.5*(-(((pow(fsigmaL,2) - pow(fsigmaT,2))* pow(-fSmax + flocTel3D[2][iTel]*cos(tSzen) + (flocTel3D[0][iTel] - fXcore)*cos(tSaz)*sin(tSzen) + flocTel3D[1][iTel]*sin(tSzen)*sin(tSaz) - fYcore*sin(tSzen)*sin(tSaz) + (ipZ*cos(tSzen) + ipX*cos(tSaz)*sin(tSzen) + ipY*sin(tSzen)*sin(tSaz))* (ipZ*(-flocTel3D[2][iTel] + fSmax*cos(tSzen)) + ipX*(-flocTel3D[0][iTel] + fXcore + fSmax*cos(tSaz)*sin(tSzen)) + ipY*(-flocTel3D[1][iTel] + fYcore + fSmax*sin(tSzen)*sin(tSaz))),2))/ (pow(fsigmaT,2)*(pow(fsigmaT,2)* pow(ipZ*cos(tSzen) + ipX*cos(tSaz)*sin(tSzen) + ipY*sin(tSzen)*sin(tSaz),2) + pow(fsigmaL,2)*(1 - pow(ipZ*cos(tSzen) + ipX*cos(tSaz)*sin(tSzen) + ipY*sin(tSzen)*sin(tSaz),2))))) + (pow(-flocTel3D[2][iTel] + fSmax*cos(tSzen),2) + pow(-flocTel3D[0][iTel] + fXcore + fSmax*cos(tSaz)*sin(tSzen),2) + pow(-flocTel3D[1][iTel] + fYcore + fSmax*sin(tSzen)*sin(tSaz),2) - pow(ipZ*(-flocTel3D[2][iTel] + fSmax*cos(tSzen)) + ipX*(-flocTel3D[0][iTel] + fXcore + fSmax*cos(tSaz)*sin(tSzen)) + ipY*(-flocTel3D[1][iTel] + fYcore + fSmax*sin(tSzen)*sin(tSaz)),2))/pow(fsigmaT,2)))/(2.*fsigmaT*TMath::Pi()*sqrt(pow(fsigmaT,2)* pow(ipZ*cos(tSzen) + ipX*cos(tSaz)*sin(tSzen) + ipY*sin(tSzen)*sin(tSaz),2) + pow(fsigmaL,2)*(1 - pow(ipZ*cos(tSzen) + ipX*cos(tSaz)*sin(tSzen) + ipY*sin(tSzen)*sin(tSaz),2)))) ;
  //
  //}
