...
// read or write the tables
    ...
    if( freadwrite == 'W' || freadwrite == 'w' ) fwrite = true;
    else fwrite = false;

    cout << endl;
    cout << "-------------------------------------------------------" << endl;
    if( fwrite ) cout << "making lookup tables" << endl;
    else         cout << "reading lookup tables" << endl;
    cout << "-------------------------------------------------------" << endl;
    cout << endl;
...
// bins in azimuth (lookup tables are created for each bin)
    fTableAzLowEdge.push_back(  135. );  fTableAzUpEdge.push_back( -135. );
    fTableAzLowEdge.push_back(  -135. ); fTableAzUpEdge.push_back( -45. );
    fTableAzLowEdge.push_back(  -45. );  fTableAzUpEdge.push_back( 45. );
    fTableAzLowEdge.push_back(  45. );   fTableAzUpEdge.push_back( 135 );
...
{
    if( fDebug ) cout << "void VTableLookup::setMCTableFiles" << endl;
    if( !fwrite )
    {
...
// prepare output file and create all directories
    if( fDebug ) cout << "VTableLookup::setMCTableFiles() prepare output file and create all directories" << endl;
    char hname[800];
    char htitle[800];
// NOISE LEVEL
    sprintf( hname, "NOISE_%05d", noise );
    if( fLookupTableFile->Get( hname ) ) fLookupTableFile->cd( hname );
    else fLookupTableFile->mkdir( hname )->cd();
// ZENITH ANGLE
    sprintf( hname, "ze_%03d", (int)(ize*10.+0.5) );
    if( gDirectory->Get( hname ) ) gDirectory->cd( hname );
    else gDirectory->mkdir( hname )->cd();
// DIRECTION OFFSET
    sprintf( hname, "woff_%04d", woff );
    if( gDirectory->Get( hname ) ) gDirectory->cd( hname );
    else gDirectory->mkdir( hname )->cd();
...
// AZIMUTH ANGLE
    ...
    for( unsigned int i = 0; i < fTableAzLowEdge.size(); i++ )
    {
        ...
        if( gDirectory->Get( hname ) )   gDirectory->cd( hname );
        else                             gDirectory->mkdir( hname, htitle )->cd();
...
{
    if( fDebug ) cout << "void VTableLookup::setMCTableFiles( string itablefile, string isuff )" << endl;
...
// NOISE LEVEL
 ...
 //       cout << iDName[n] << "\t" << fTableNoiseLevel.back() << "\t" << iDName[n].substr( 6, 5 ) << "\t" << n << endl;
        ...
        if( fDebug == 2 ) cout << "XXX DIR " << " " << gDirectory->GetPath() << endl;
// ZENITH ANGLE
...
bool VTableLookup::sanityCheckLookupTableFile( bool iPrint)
{
  ...
  { 
    ...
    if (iPrint==true) cout << " Noise level index  " << i << " has " << fTableZe[i].size() << " zenith angles"<< endl;
    if (iPrint==false)
    ...
    {
      if (iPrint==true) cout << "Zenith index " << j <<  " has " << fTableDirectionOffset[i][j].size() << " wobble offsets"<< endl;
      if (iPrint==false)
      {
...
// print progress
            if( (fevent%1000000) == 0 && fevent != 0 ) cout << "\t now at event " << fevent << endl;
	    if( fDebug ) cout << "now at event " << fevent << "\t" << fData->getEventStatus() << endl;
...
// first event
        bool bFirst = true;
        if( !fTLRunParameter->isMC ) bFirst = false;
...
// print progress
            fevent = fData->getEventCounter();
            if( (fevent%1000000)==0 ) cout << "\t now at event " << fevent << endl;
...
// get zenith angle for first valid MC event from MC files
            ...
            {
                if( fNTel > 0 ) fTLRunParameter->ze = TMath::Floor( (90.-fData->getTelElevation()) + 0.5 );
                else            fTLRunParameter->ze = TMath::Floor( fData->getMCZe() + 0.5 );
                if( fTLRunParameter->ze < 1.5 ) fTLRunParameter->ze = 0.;
...
// get noise level for this event
                ...
                if( fDebug == 2 ) cout << endl << endl << "XXX NEW EVENT " << fData->getEventCounter() << endl;
/////////////////////////////
// NOISE (low) ZENITH (low)
                if( fDebug == 2 ) cout << "XXX NOISE LOW ZENITH LOW" << endl;
                for( int t = 0; t < fNTel; t++ )
                {
                    if( fDebug == 2 )
                    {
                        cout << "XXX TELESCOPE " << t << " (T" << t+1 << ")" << endl;
                        cout << "XXX     zenith " << ze << ", noise " << fNoiseLevel[t] << ", woff " << woff << ", az " << fData->getAz() << ", az bin " << i_az << endl;
                    }
// noise (low)
...
// get zenith angle
                    ...
                    if( fDebug == 2 )
                    {
                        cout << "XXX WOFF " << t << " " << inoise_low << " " << inoise_up << " " << fNoiseLevel[t] << "\t" << fTableNoiseLevel.size();
                        for( unsigned int ii = 0; ii < fTableNoiseLevel.size(); ii++ ) cout << "    " << fTableNoiseLevel[ii];
                        cout << endl;
                    }
...
// get direction offset index
                ...
                if( fDebug == 2 ) cout << "XXX WOFF INTER 1 " << woff << " " << fTableDirectionOffset[inoise_low][ize_low][iwoff_low] << " " << fTableDirectionOffset[inoise_low][ize_low][iwoff_up] << " " << inoise_low << " " << inoise_up << " " << ize_low << " " << s_NlowZlowWlow->mscl << " " << s_NlowZlowWup->mscl << " " << s_NlowZlow->mscl << endl;
...
// get direction offset index
                ...
                if( fDebug == 2 ) cout << "XXX ZE INTER 1 " << ze << " " << fTableZe[inoise_low][ize_low] << " " << fTableZe[inoise_low][ize_up] << " " << inoise_low << " " << inoise_up << " " << s_NlowZlow->mscl << " " << s_NlowZup->mscl << " " << s_Nlow->mscl << endl;

///////////////////////////
// NOISE (up) ZENITH (low)
                if( fDebug == 2 ) cout << "XXX HIGH NOISE" << endl;
                for( int t = 0; t < fNTel; t++ )
                {
// noise (up)
                    getIndexBoundary( &inoise_up, &inoise_low, fTableNoiseLevel, fNoiseLevel[t] );
// get zenith angle
                    ...
                    if( fDebug == 2 ) cout << "XXX WOFF " << t << " " << inoise_low << " " << inoise_up << " " << fNoiseLevel[t] << endl;
...
// get direction offset index
                ...
                if( fDebug == 2 ) cout << "XXX WOFF INTER 1 " << woff << " " << fTableDirectionOffset[inoise_up][ize_low][iwoff_low] << " " << fTableDirectionOffset[inoise_up][ize_low][iwoff_up] <<  " " << inoise_low << " " << inoise_up << " " << s_NupZlowWlow->mscl << " " << s_NupZlowWup->mscl << " " << s_NupZlow->mscl << endl;
...
// get direction offset index
                ...
                if( fDebug == 2 ) cout << "XXX WOFF INTER 2 " << woff << " " << fTableDirectionOffset[inoise_up][ize_up][iwoff_low] << " " << fTableDirectionOffset[inoise_up][ize_up][iwoff_up] << " " << inoise_low << " " << inoise_up << " " << s_NupZupWlow->mscl << " " << s_NupZupWup->mscl << " " << s_NupZup->mscl << endl;
///////////////////////////

                interpolate( s_NupZlow, fTableZe[inoise_up][ize_low], s_NupZup, fTableZe[inoise_up][ize_up], s_Nup, ze, true );
                if( fDebug == 2 ) cout << "XXX ZE INTER 2 " << ze << " " << inoise_low << " " << inoise_up << " " << s_NupZlow->mscl << " " << s_NupZup->mscl << " " << s_Nup->mscl << endl;
                interpolate( s_Nlow, fTableNoiseLevel[inoise_low], s_Nup, fTableNoiseLevel[inoise_up], s_N, fMeanNoiseLevel, false );
                if( fDebug == 2 ) cout << "XXX NOISE INTER " << fMeanNoiseLevel << " " << fTableNoiseLevel[inoise_low] << " " << fTableNoiseLevel[inoise_up] << " " << inoise_low << " " << inoise_up << " " << s_Nlow->mscl << " " << s_Nup->mscl << " " << s_N->mscl << endl;

// determine number of telescopes with MSCW values
                for( unsigned int j = 0; j < s_N->fNTel; j++ )
                {
                    if( s_N->mscw_T[j] > -90. ) fnmscw++;
                }
...
// calculate mean width ratio (mean scaled variables)
                ...
                if( inr > 0. ) fData->setMWR( imr / inr );
                else           fData->setMWR( -99. );
// calculate mean length ratio (mean scaled variables)
                ...
                if( inr > 0. ) fData->setMLR( imr / inr );
                else           fData->setMLR( -99. );
...
// otherwise like [-45,45]
    ...
    {
        if( i == 0 && ( az > fTableAzLowEdge[0] || az < fTableAzUpEdge[0] ) ) return 0;
        else if( az > fTableAzLowEdge[i] && az < fTableAzUpEdge[i] ) return i;
    }
    return 0;
}
...
vector< string > VTableLookup::getSortedListOfDirectories( TDirectory *iDir )
{
    ...
    if( bWoffAltered )
    {
        ...
        {
            if( iDName[i].substr( 0, 6 ) == "woff_0" ) iDName[i] = "woff_" + iDName[i].substr( 6, iDName[i].size() );
        }
    }
...
void VTableLookup::readNoiseLevel( bool bWriteToRunPara )
{
    ...
    if( bWriteToRunPara )
    {
        ...
        for( unsigned int i = 0; i < fNoiseLevel.size(); i++ ) cout << " " << fNoiseLevel[i];
        cout << endl;
    }
// check noise levels
...
// PE simulations with no noise values (preliminary, should somehow be included into the MC)
	...
	{
	    if( fData->getNtubes()[i] > 0 ) fTelToAnalyze[i] = true;
	    else                            fTelToAnalyze[i] = false;
        }
// normal data or MC run
	else
	{
	   ...
	   else if( TMath::Abs( fNoiseLevel[i] ) < 1.e-2 && fData->getNtubes()[i] < 1 ) fTelToAnalyze[i] = false;
	   else fTelToAnalyze[i] = true;
       }
    }
...
void VTableLookup::getIndexBoundary( unsigned int *iup, unsigned int *ilow, vector< double >& iV, double x )
{
    ...
    if( x <= iV[0] ) *iup = *ilow = 0;
    else if( x >= iV[iV.size()-1] ) *iup = *ilow = iV.size()-1;
    else
    {
...
void VTableLookup::getTables( unsigned int inoise, unsigned int ize, unsigned int iwoff, unsigned int iaz, unsigned int tel, VTablesToRead *s )
{
    ...
    if( fDebug == 2 )
    {
        cout << "XXX getTables() "  << inoise << " " << ize << " " << iwoff << " " << iaz << " " << telX << endl;
        cout << "XXX " << fmscw[inoise][ize][iwoff][iaz][telX] << endl;
        cout << "XXX MEDIAN (MSCW) " << inoise << " " << ize << " " << iwoff << " " << iaz << " " << telX << " " << fmscw[inoise][ize][iwoff][iaz][telX]->getHistoMedian()->GetEntries() << "\t" << fmscw[inoise][ize][iwoff][iaz][telX]->getHistoMedian()->GetTitle() << "\t" << fmscw[inoise][ize][iwoff][iaz][telX]->getHistoMedian()->GetDirectory()->GetPath() << endl;
        cout << "XXX MEDIAN (MSCW,2) " << fmscw[inoise].size() << endl;
        cout << "XXX MEDIAN (MSCL) " << inoise << " " << ize << " " << iwoff << " " << iaz << " " << telX << " " << fmscl[inoise][ize][iwoff][iaz][telX]->getHistoMedian()->GetEntries() << "\t" << fmscl[inoise][ize][iwoff][iaz][telX]->getHistoMedian()->GetTitle() << "\t" << fmscl[inoise][ize][iwoff][iaz][telX]->getHistoMedian()->GetDirectory()->GetPath() << endl;
        cout << "XXX MEDIAN (MSCL,2) " << fmscl[inoise].size() << endl;
    }
...
// weight event to this spectral index
    if( fTLRunParameter->isMC ) setSpectralIndex( fTLRunParameter->fSpectralIndex );
...
// set min/max distance to camera center
        if( fData ) fData->setMCDistanceToCameraCenter( fTLRunParameter->fMC_distance_to_cameracenter_min, fTLRunParameter->fMC_distance_to_cameracenter_max );
    }
// read MC tables
    else
    {
