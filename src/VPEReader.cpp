...
VPEReader::VPEReader(  string isourcefile, vector< unsigned int > iTeltoAna, VDetectorGeometry *iD, bool iDebug )
{
    fDebug = iDebug;
    setEventStatus( 1 );
    if( fDebug ) cout << "VPEReader::VPEReader" << endl;
...
bool VPEReader::init()
{
    if( fDebug ) cout << " VPEReader::init() " << endl;
...
// check if this telescope should be analyzed
        ...
        if( bAna )
        {
            ...
            if( iTelGrisu.find( i ) != iTelGrisu.end() ) iTelID = iTelGrisu[i];
...
{
    if( fDebug ) cout << "VPEReader::getNextEvent()" << endl;

    for( unsigned int i = 0; i < fSums.size(); i++ )
    {
        fSums[i] = 0.;
	for( unsigned int j = 0; j < fTracePulseTiming[i].size(); j++ ) fTracePulseTiming[i][j] = 0.;
...
// get MC info from first tree
        ...
        if( fPE_Tree[i]->isFull() )
        {
            ...
            if( fPE_ze < 0. ) fPE_ze = 0.;
            fPE_ze = acos( sqrt( fPE_ze ) );
            if( TMath::Abs( fPE_ze ) < 0.1 ) fPE_az += TMath::Pi();
...
// add wobble offset
        ...
        if( fDebug )
        {
            if( fPE_Tree[i]->isFull() )
            {
                ...
                {
                    if( fSums[i][j] > 0. ) cout << "\t" << i << "\t" << j << "\t" << fSums[i][j] << endl;
                }
                cout << endl;
            }
        }
...
// fill data vectors
    for( unsigned int i = 0; i < fNTelescopes; i++ )
    {
        ...
        {
	    for( unsigned int k = 0; k < fTracePulseTiming[i].size(); k++ ) fTracePulseTiming[i][j][k] = 0.;
            fHiLo[i][j] = (bool)0;
        }
    }
...
// array trigger requires simply 2 telescope with non-zero size
    if( fNLocalTrigger > 1 ) fArrayTrigger = true;
    else                     fArrayTrigger = false;
// single telescope analysis
    if( fNTelescopes == 1 && fNLocalTrigger == 1 ) fArrayTrigger = true;
