...
VEventLoop::VEventLoop( VEvndispRunParameter* irunparameter )
{
	...
	else
	{
		if( fDebug )
		{
			cout << "setting fit tracehandler" << endl;
		}
...
int VEventLoop::analyzeEvent()
{
	...
	for( unsigned int i = 0; i < fRunPar->fTelToAnalyze.size(); i++ )
	{
		...
		if( fLowGainMultiplierWarning < 3 && fRunPar->fIsMC == 0 && ( fRunPar->frunmode == 0 || fRunPar->frunmode == 3 || fRunPar->frunmode == 4 ) )
		{
			...
			{
				if( getLowGainDefaultSumWindows()[j] == fRunPar->fsumwindow_1[fRunPar->fTelToAnalyze[i]] )
				{
					found = true;
				}
			}
		...
		if( fLowGainMultiplierWarning < 3 && fRunPar->fIsMC == 0 && ( fRunPar->frunmode == 0 || fRunPar->frunmode == 3 || fRunPar->frunmode == 4 ) )
		{
			...
			for( unsigned int j = 0; j < getLowGainDefaultSumWindows().size() ; j++ )
			{
				if( getLowGainDefaultSumWindows()[j] == fRunPar->fsumwindow_2[fRunPar->fTelToAnalyze[i]] )
				{
					found = true;
				}
			}
		...
		if( fLowGainMultiplierWarning < 3 && fRunPar->fIsMC == 0 && ( fRunPar->frunmode == 0 || fRunPar->frunmode == 3 || fRunPar->frunmode == 4 ) )
		{
			...
			for( unsigned int j = 0; j < getLowGainDefaultSumWindows().size() ; j++ )
			{
				if( getLowGainDefaultSumWindows()[j] == fRunPar->fsumwindow_pass1[fRunPar->fTelToAnalyze[i]] )
				{
					found = true;
				}
			}
