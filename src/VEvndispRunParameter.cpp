...
void VEvndispRunParameter::print( int iEv )
{
	...
	if( frunmode == 0 || frunmode == 4 )
	{
		...
		if( fDoublePass )
		{
			...
			if( fDynamicIntegrationWindow )
			{
				cout << " (dynamical integration window) ";
			}
			cout << " (uncertainties: ";
			if( fDoublePassErrorWeighting2005 )
			{
				cout << "2005) ";
			}
			else
		...
		{
			cout << "no double pass cleaning";
		}
		cout << endl;
		if( fFixWindowStart )
		{
			cout << "using fixed window start" << endl;
		}
		...
		if( ftracefit > -1. )
		{
			cout << "trace fitting: " << ftracefit << " with " << ftracefitfunction << endl;
		}
		if( fSmoothDead )
		{
