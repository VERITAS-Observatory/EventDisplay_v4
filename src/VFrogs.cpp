...
struct frogs_imgtmplt_in VFrogs::frogs_convert_from_ed(int eventNumber, int adc_type, double inEnergy) {
  ...
  for(int tel=0;tel<rtn.ntel;tel++) 
  {
    ...
    rtn.scope[tel].xfield = transformTelescopePosition( tel, 90 - fData->getShowerParameters()->fTelElevation[0], fData->getShowerParameters()->fTelAzimuth[0], 0 );
    rtn.scope[tel].yfield = transformTelescopePosition( tel, 90 - fData->getShowerParameters()->fTelElevation[0], fData->getShowerParameters()->fTelAzimuth[0], 1 );
    ...
    //float foclen=1000*fData->getDetectorGeo()->getFocalLength()[tel]; //Focal length in mm
...
//        *FROGS_DEG_PER_RAD/foclen;
      ...
      float tmppixarea=fData->getDetectorGeo()->getTubeRadius_MM(tel)[pix];
//        FROGS_DEG_PER_RAD/foclen;*/
...
// GH UNITS HERE!!
  float cosz = sin(fData->getShowerParameters()->fTelElevation[0]*FROGS_PI/180.0);
  if(rtn.ntel!=4) 
    frogs_showxerror("Estimation of starting energy value requires 4 telescopes");
...
//    float corrected_sum = taevnt->img[tel][adc_type].sum/tel_correct[tel];
    float corrected_sum = fData->getImageParameters()->size/tel_correct[tel];
    // small image correction in datareader
//    if(taevnt->img[tel][adc_type].ntub<15) 
    if(fData->getImageParameters()->ntubes<15) 
//      corrected_sum *= 1+(15-taevnt->img[tel][adc_type].ntub)*0.015;  
      corrected_sum *= 1+(15-fData->getImageParameters()->ntubes)*0.015;  
  ...
  if(Etelsum > 0. && nEtel > 0 )
    rtn.startpt.log10e=log10( (Etelsum/nEtel)/1000.0 );
  else
    rtn.startpt.log10e=FROGS_BAD_NUMBER;

  rtn.startpt.log10e = inEnergy;
...
#define OUTUNIT stdout
  ...
  fprintf(OUTUNIT,".................................................\n");
  fprintf(OUTUNIT,"            STARTING POINT %d:\n",d.event_id);
...
struct frogs_imgtemplate frogs_read_template_elev(float elevation) {
  ...
  if(elevation>=65.0) {
    ...
    strcpy(template_file_name,"/afs/ifh.de/user/g/gahughes/scratch/VERITAS/EVNDISP/EVNDISP-400/Templates/SimulatedModels/elev70X0.0-4.0s0.5r085.tmplt"); 
  ...
  if(fabs(elevation-60.0)<=5.0) {
    char template_file_name[FROGS_FILE_NAME_MAX_LENGTH];
    strcpy(template_file_name,"/afs/ifh.de/user/g/gahughes/scratch/VERITAS/EVNDISP/EVNDISP-400/Templates/SimulatedModels/elev60X0.0-4.0s0.5r085.tmplt"); 
    rtn=frogs_read_template_file(template_file_name);
    //The template file elevation is not stored in the file and we set it here.
    rtn.elevation=60.0;
    return rtn;
  ...
  if(fabs(elevation-50.0)<=5.0) {
    char template_file_name[FROGS_FILE_NAME_MAX_LENGTH];
    strcpy(template_file_name,"/afs/ifh.de/user/g/gahughes/scratch/VERITAS/EVNDISP/EVNDISP-400/Templates/SimulatedModels/elev50X0.0-4.0s0.5r085.tmplt"); 
    rtn=frogs_read_template_file(template_file_name);
    //The template file elevation is not stored in the file and we set it here.
    rtn.elevation=50.0;
    return rtn;
