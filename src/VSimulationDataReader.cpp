...
// (very dependent on structure of string -> will soon be replace by a VGrisuSimulationRunHeader)
	...
	while( !is_stream.eof() )
	{
	   ...
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->shower_date;
           }
	   ...
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->primary_id;
           }
	   ...
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->injection_height;
           }
	   else if( iTemp == "TSTART" )
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->fixed_int_depth;
           }
	   else if( iTemp == "E_SLOPE" )
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->spectral_index;
           }
	   else if( iTemp == "E_MIN" )
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->E_range[0];
           }
	   else if( iTemp == "E_MAX" )
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->E_range[1];
           }
	   else if( iTemp == "ZENITH_MIN" )
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->alt_range[0];
	      iMCRunHeader->alt_range[0] = (90.-iMCRunHeader->alt_range[0])/(45. / atan( 1. ));
           }
	   else if( iTemp == "ZENITH_MAX" )
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->alt_range[1];
	      iMCRunHeader->alt_range[1] = (90.-iMCRunHeader->alt_range[1])/(45. / atan( 1. ));
           }
	   else if( iTemp == "AZIMUTH_MIN" )
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->az_range[0];
	      iMCRunHeader->az_range[0] /= (45. / atan( 1. ));
	   }
	   else if( iTemp == "AZIMUTH_MAX" )
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->az_range[1];
	      iMCRunHeader->az_range[1] /= (45. / atan( 1. ));
	   }
	   else if( iTemp == "VIEWCONE_MIN" )
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->viewcone[0];
	      iMCRunHeader->viewcone[0] /= (45. / atan( 1. ));
	   }
	   else if( iTemp == "VIEWCONE_MAX" )
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->viewcone[1];
	      iMCRunHeader->viewcone[1] /= (45. / atan( 1. ));
	   }
	   else if( iTemp == "NSCATT" )
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->num_use;
           }
	   else if( iTemp == "XSCATT" )
	   {
	     if( !is_stream.eof() ) is_stream >> iMCRunHeader->core_range[0];
	   }
	   else if( iTemp == "YSCATT" )
	   {
	     if( !is_stream.eof() ) is_stream >> iMCRunHeader->core_range[1];
	   }
	   else if( iTemp == "CBUNCH" )
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->corsika_bunchsize;
           }
	   else if( iTemp == "C_WMIN" )
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->corsika_wlen_min;
           }
	   else if( iTemp == "C_WMAX" )
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->corsika_wlen_max;
           }
	   else if( iTemp == "GEO_X" )
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->B_inclination;   // entry will be corrected later
           }
	   else if( iTemp == "GEO_Z" )
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->B_total;  // entry will be corrected later
           }
	   else if( iTemp == "GEO_A" )
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->B_declination;
	      iMCRunHeader->B_declination /= (45. / atan( 1. ));
           }
	   else if( iTemp == "HAD_LOW" )
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->corsika_low_E_model;
           }
	   else if( iTemp == "HAD_HIGH" )
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->corsika_high_E_model;
           }
	   else if( iTemp == "HAD_TRANS" )
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->corsika_low_high_E;
           }
	   else if( iTemp == "CFLAG" )
	   {
	      if( !is_stream.eof() ) is_stream >> iMCRunHeader->corsika_iact_options;
           }
	   else if( iTemp == "ATM" )
	   {
	      int iATM = 0;
	      if( !is_stream.eof() ) is_stream >> iTemp;
	      if( !is_stream.eof() ) is_stream >> iATM;
// (GM) simulation header in MC vbf file is not filled properly (yet)
	      if( iATM != 0 ) iMCRunHeader->atmosphere = iATM;
...
// scatter mode (y=0)
    if( TMath::Abs( iMCRunHeader->core_range[1] ) < 1.e-5 ) iMCRunHeader->core_pos_mode = 1;
// geomagnetic field
    ...
    if( iMCRunHeader->B_total > 0. ) iMCRunHeader->B_inclination = acos( iMCRunHeader->B_inclination / iMCRunHeader->B_total );
    else                             iMCRunHeader->B_inclination = 0.;
...
{
    if( fDebug ) cout << "\t\t VSimulationDataReader::setSimulationData" << endl;
    bool iReturn = false;

    if( !packet ) return iReturn;
...
// this vector has size 255
        ...
        if( i_ae->hasEventNumber() ) eventNumber = i_ae->getEventNumber();
        else                         eventNumber = 0;
    }
    else
    {
    ...
    {
        if( fDebug ) cout << "\t\t\t VSimulationDataReader::setSimulationData(): hasSimulationData(), eventnumber " << eventNumber << endl;
...
// (this might be different to what VGrisuReader reads from the MC file
        ...
        fMCXcos = sin( (fMCZe) / degrad ) * cos( (fMCAz) / degrad );
                                                  // rounding error
        if( fabs( fMCXcos ) < 1.e-8 ) fMCXcos = 0.;
        fMCYcos = sin( (fMCZe) / degrad ) * sin( (fMCAz) / degrad );
                                                  // rounding error
        if( fabs( fMCYcos ) < 1.e-8 ) fMCYcos = 0.;
        iReturn = true;
        if( fDebug )
        {
    ...
    {
        if( fDebug ) cout << "\t\t\t VSimulationDataReader::setSimulationData(): no hasSimulationData()" << endl;
        iReturn = false;
    }
