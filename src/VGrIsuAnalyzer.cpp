...
// (GM) small number check
    for( unsigned int i = 0; i < 3; i++ ) if( TMath::Abs(c[i]) < 1.e-5 ) c[i] = 0.;
...
// calculate weighted variance
    ...
    if( wsum > 0. ) *std /= ((float)xcore.size() * wsum );
    if( xcore.size() > 0. ) *std /= (float)xcore.size() ;
    if( *std < 1.e-5 ) *std = 0.;
...
// accept currently no zero direction vectors
    if( mx1 == 0. || my1 == 0. ) return false;
    else if( mx2 == 0. || my2 == 0. ) return false;
    else if( mx2/mx1 - my2/my1 == 0. || mx1/mx2 - my1/my2 == 0. ) return false;
    else
    {
