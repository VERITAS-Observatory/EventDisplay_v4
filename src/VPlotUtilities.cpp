...
// y-axis
        if( bLogY )
        {
            ...
            TGaxis *y2 = new TGaxis( iPad->GetUxmax(), iPad->GetUymin(), iPad->GetUxmax(), iPad->GetUymax(), h->GetMinimum(), h->GetMaximum(), 510, "+UG" );
            y2->Draw();
        }
        else
        {
...
void VPlotUtilities::setFunctionPlottingStyle( TF1 *fun, int icolor, double iwidth, double isize, int imarker, int iFillStyle )
{
    ...
    if( imarker !=0 ) fun->SetMarkerStyle( imarker );
    if( iFillStyle >= 0 )
    {
  ...
  if( iminF > -10. )
    {
	  ...
	  for( int j = 1; j <= his->GetNbinsY(); j++ )
            {
	      if( his->GetBinContent(i,j)>-9999. &&  his->GetBinContent(i,j) < idiffmin ) idiffmin = his->GetBinContent(i,j);
            }
        }
...
// plot axis on the right side
    ...
    for( int i = 0; i < fColorAxis_ncont; i++ )
    {
        ...
        if (w1 < fColorAxis_vmin) w1 = fColorAxis_vmin;
        w2 = fColorAxis_vmax;
        if (i < fColorAxis_ncont-1) w2 = fColorAxis_vmin + wls / fColorAxis_ncont * ( i + 1 );
        if (w2 <= fColorAxis_vmin) continue;
...
void VPlotUtilities::setPadMargins( TCanvas *c, int nPads, double lM, double rM )
{
  ...
  if( nPads > 1 )
  {
	 ...
	 if( c->GetPad( i+1 ) )
	 {
	     if( lM > -999. ) c->GetPad( i+1 )->SetLeftMargin( lM );
	     if( rM > -999. ) c->GetPad( i+1 )->SetRightMargin( rM );
	 }
	 else
	 {
  ...
  {
     if( lM > -999. ) c->SetLeftMargin( lM );
     if( rM > -999. ) c->SetRightMargin( rM );
  }
}
  ...
  for( int i = 1; i <= h->GetNbinsX(); i++ )
    {
	  ...
	  if( x*x + y*y > r ) h->SetBinContent( i, j, ivalue );
...
{
   if( fPlottingAxisData.find( iName ) != fPlottingAxisData.end() ) return fPlottingAxisData[iName];
