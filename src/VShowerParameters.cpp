...
void VShowerParameters::initTree( string iName, string iTitle, bool iMC )
{
    ...
    if( fShortTree < 1 ) fTreeSC->Branch("dataFormat", &fsourcetype, "dataFormat/I" );
    fTreeSC->Branch( "NTel", &fNTelescopes, "NTel/i" );
    if( fShortTree < 1 ) fTreeSC->Branch( "traceFit", &fTraceFit, "traceFit/I" );
...
// added C.Duke 21dec06
        ...
        fTreeSC->Branch( "TargetRA", &fTargetRA, "TargetRA/F" );
    }
...
void VShowerParameters::resetDispVectors()
{
    ...
    {
        for( unsigned int j = 0; j < fShower_Yoff_DISP[i].size(); j++ ) fShower_Yoff_DISP[i][j] = -99.;
    }
    for( unsigned int i = 0; i< fShower_Xoff_DISP.size(); i++ )
    {
        for( unsigned int j = 0; j < fShower_Xoff_DISP[i].size(); j++ ) fShower_Xoff_DISP[i][j] = -99.;
    }
    for( unsigned int i = 0; i< fShower_Weight_DISP.size(); i++ )
    {
        for( unsigned int j = 0; j < fShower_Weight_DISP[i].size(); j++ ) fShower_Weight_DISP[i][j] = -99.;
    }
}
