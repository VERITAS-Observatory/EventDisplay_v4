...
VTableEnergyCalculator::VTableEnergyCalculator( const char* hname_add, char m, TDirectory* iDir, bool iMedian, string iInterpolate )
{
	...
	if( ( Omode == 'w' ) || ( Omode == 'W' ) )
	{
		...
		if( fUseMedianEnergy )
		{
			sprintf( htitle, "energy vs. distance vs. log10 size (median), %s", hname_add );
		}
		else
		{
...
void VTableEnergyCalculator::terminate( TDirectory* iOutDir, char* xtitle )
{
	...
	if( ( Omode == 'w' ) || ( Omode == 'W' ) )
	{
		...
		if( fUseMedianEnergy )
		{
			cout << "\t (using median of size distributions)";
		}
		else
		{
			cout << "\t (using mean of size distributions)";
		}
		cout << endl;
		TDirectory* iDir1D = 0;
		...
		for( tel = 0; tel < ntel; tel++ )
		{
			...
			if( s[tel] > fMinSize && r[tel] >= 0. && r[tel] < fMaxDistance )
			{
				...
				}
				{
					...
					if( fDebug )
					{
						cout << "\t\t  VTableEnergyCalculator::calc(): energy estimator: " << med << "\t" << sigma << endl;
					}
...
{
	TH2F* hM = hMedian;
	if( hVMedian.size() > 0 && tel < hVMedian.size() )
	{
		hM = hVMedian[tel];
	}
	if( !hM )
	...
	{
		if( hM->GetBinContent( j, ir ) > 0. && hM->GetBinContent( j, ir ) < logSize )
		{
			break;
		}
	}
...
void VTableEnergyCalculator::get_logEnergy2D( double logSize, double r, double& med, double& sigma, unsigned int itel )
{
	...
	if( ir1 < 1 || ir1 >= hM->GetNbinsY() )
	{
		...
		if( fDebug )
		{
			cout << "\t\t void VTableEnergyCalculator::get_logEnergy2D overflow bin in r " << r << "\t" << ir1 << "\t" <<  hM->GetNbinsY() << endl;
		}
		return;
	}
