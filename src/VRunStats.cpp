...
bool VRunStats::readFromDB()
{
    ...
    if( !readDBWeatherInfo( f_db ) ) return false; 

    if( !readDB_default_HVEvndispData( f_db ) ) return false;

    if( fReadfullHVEvndispData ) 
    {
       if( !readDBHVEvndispData( f_db ) ) return false;
    }
...
bool VRunStats::readDBHVEvndispData( TSQLServer *f_db )
{
   ...
   for( unsigned int i = 0; i < 4; i++ )
   {
	 ...
	 if( iChannel < 500 )
	 {
	    ...
	    else fHVEvndispData.back()->startMJD = 0.;
...
//      for( unsigned int f = 0; f < db_res->GetFieldCount(); f++ ) cout << f << "\t" << db_res->GetFieldName( f ) << endl;
	 ...
	 if( iChannel < 500 )
	 {
	    ...
	    else fdefaultHVEvndispData.back()->startMJD = 0.;
	    if( db_row->GetField( 1 ) )
	    {
	       itemp = db_row->GetField( 1 );
	       getDBMJDTime( itemp, iMJD, iTime, true );
	    ...
	    else fdefaultHVEvndispData.back()->stopMJD = 0.;
	    if( db_row->GetField( 3 ) )
	    {
	       itemp = db_row->GetField( 3 );
	       fdefaultHVEvndispData.back()->default_HV = atof( itemp.c_str() );
            }
	    else fdefaultHVEvndispData.back()->default_HV = 0.;
         }
      }
   }
   return true;
...
// read run data 
   ...
   for( unsigned int i = 0; i < 8; i++ ) cout << i << "\t" << db_res->GetFieldName( i ) << endl;
...
// read run data 
   ...
   for( unsigned int i = 0; i < 4; i++ ) cout << i << "\t" << db_res->GetFieldName( i ) << endl;
...
// read run data
    ...
    for( unsigned int i = 0; i < 9; i++ ) cout << i << "\t" << db_res->GetFieldName( i ) << endl;
...
// read run data
    ...
    int fNRows = db_res->GetRowCount();
    for( unsigned int i = 0; i < 3; i++ ) cout << i << "\t" << db_res->GetFieldName( i ) << endl;
...
// get run number
        itemp = db_row->GetField( 0 );
	if( i == 0 ) fMin_run_id = atoi( itemp.c_str() );
	if( i == fNRows - 1 ) fMax_run_id = atoi( itemp.c_str() );
    }
...
// read run data
    ...
    int fNRows = db_res->GetRowCount();
    for( unsigned int i = 0; i < 19; i++ ) cout << i << "\t" << db_res->GetFieldName( i ) << endl;
...
// loop over all runs
    for( unsigned int i = 0; i < fRunData.size(); i++ )
    {
        ...
        if( fRunData[i]->SourceID.size() < 300 ) sprintf( iRunSourceID, "%s", fRunData[i]->SourceID.c_str() );
        else                               sprintf( iRunSourceID, "%s", fRunData[i]->SourceID.substr( 0, 299 ).c_str() );
...
// analysis comments
        ...
        for( unsigned int j = 0; j < fAnalysisComments.size(); j++ )
	{
	   if( iRun == fAnalysisComments[j]->runNumber )
	   {
	      ...
	      if( fAnalysisComments[j]->comment.size() < 300 ) sprintf( iComment, "%s",fAnalysisComments[j]->comment.c_str() );
	      else                                             sprintf( iComment, "%s",fAnalysisComments[j]->comment.substr( 0, 299 ).c_str() );
	      break;
           }
	   else
	   {
...
// camera status
	...
	if( nCameraStatusPoints > nCSPoints_MAX ) continue;
...
// FIR data
	...
	if( FIR_NPoints > nFIRPoints_MAX ) continue;
...
// weather data
        ...
        if( nWeatherPoints > nWeatherPoints_MAX ) continue;
...
// 40
        ...
        if( fDQMData.back()->fieldVersion < 1 ) continue;
...
// 100
        ...
        if( fDQMData.back()->fieldVersion < 2 ) continue;
...
// 120
        ...
        for( unsigned int i = 0; i < 4; i++ )
        {
            if( TMath::Abs( fDQMData.back()->FIR_mean[i] ) < 1.e-5 )  fDQMData.back()->FIR_mean[i] = 0.;
            if( TMath::Abs( fDQMData.back()->FIR_RMS[i] ) < 1.e-5 )  fDQMData.back()->FIR_RMS[i] = 0.;
        }
