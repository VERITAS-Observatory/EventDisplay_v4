...
bool VTrackingCorrections::readTrackingCorrectionsFromDB( string iSQLDate )
{
    ...
    TSQLRow *db_row = 0;
    for( int i = 0; i < fNRows; i++ ) db_row = db_res->Next();
    if( db_row )
    {
        string itemp;
        cout << "0: " << db_row->GetField( 0 ) << ", ";
        if( db_row->GetField( 1 ) ) cout << "1: " << db_row->GetField( 1 ) << ", ";
        for( int i = 2; i < 12; i++ ) cout << i << ": " << db_row->GetField( i ) << ", ";
        cout << endl;
        for( int i = 12; i < 24; i++ ) cout << i << ": " << db_row->GetField( i ) << ", ";
        cout << endl;
        correctionParameters = new CorrectionParameters();
        itemp = db_row->GetField( 2 );
        if( itemp == "enabled" ) correctionParameters->enable_offsets = true;
        else                     correctionParameters->enable_offsets = false;
        itemp = db_row->GetField( 3 );
        if( itemp == "enabled" ) correctionParameters->enable_corrections = true;
        else                     correctionParameters->enable_corrections = false;
        itemp = db_row->GetField( 14 );
        if( itemp == "enabled" ) correctionParameters->enable_vff = true;
        else                     correctionParameters->enable_vff = false;
