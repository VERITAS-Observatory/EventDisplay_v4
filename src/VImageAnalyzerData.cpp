...
// fit value results
    if( iTraceFit )
    {
        ...
        fFallTime.resize( iChannels, 0. );
        fRiseTimePar.resize( iChannels, 0. );
        fFallTimePar.resize( iChannels, 0. );
        fChi2.resize( iChannels, 0. );
        fTraceNorm.resize( iChannels, 0. );
    }
...
// expect that trace sum is set before!!!
        if( fSums[iChannel] > 0. && getTZeros(true)[iChannel] > 0 )
        {
            ...
            {
                if( iHiLo ) hMeanPulseLow[iChannel]->Fill( log10( fSums[iChannel] ), (double)(t) - getTZeros(true)[iChannel], fT[t] - iPeds );
                else        hMeanPulseHigh[iChannel]->Fill( log10( fSums[iChannel] ), (double)(t) - getTZeros(true)[iChannel], fT[t] - iPeds );
            }
        }
    }
}
...
void VImageAnalyzerData::fillPulseSum( unsigned int iChannel, double iS, bool iHiLo )
{
    if( iS > 0. )
    {
        if( !iHiLo && iChannel < hPulseSumHigh.size() ) hPulseSumHigh[iChannel]->Fill( log10( iS ) );
        if(  iHiLo && iChannel < hPulseSumLow.size()  ) hPulseSumLow[iChannel]->Fill( log10( iS ) );
    }
}

vector<unsigned int>& VImageAnalyzerData::getFADCstopTrigChannelID()
{
   if( fSpecialChannel ) return fSpecialChannel->getFADCstopTrigChannelID();
...
{
   if( fSpecialChannel ) fSpecialChannel->reset();
   else
   {
...
{
   if( fSpecialChannel ) return fSpecialChannel->getHIGHQE_gainfactor( iChannel );
