...
bool VAtmosphereSoundings::readRootFile()
{
	...
	for( unsigned int i = 0; i < fDataTree->GetEntries(); i++ )
	{
		...
		for( unsigned int j = 0; j < nPoints; j++ )
		{
			...
			{
				fData[i]->fRelativeHumidity.back() *= 1.e2;
			}
...
bool VAtmosphereSoundings::write_2C1( unsigned int iIndexAverageData, string filename, double max_height = 150e3 )
{
	...
	for( unsigned int i = 0; i < Data->fHeight_m.size(); i++ )
	{
		...
		if( Data->fHeight_m.at( i ) > max_height )
		{
	...
	{
		if( fModtranHeights.at( i ) <= max_height )
		{
			continue;
		}
...
void VAtmosphereSoundings::plot_season( double mjd_start, double mjd_end, TString season_name, char* value, TString outfileprefix = "" )
{
	...
	for( int i = 0; i < Nmonth; i++ )
	{
		TGraph* temp = getResidualGraph( v[i]->getGraph( value ), winter->getGraph( value ), col[i] );
		if( temp )
		{
			temp->Draw( "lp same" );
		}
	...
	for( int i = 0; i < Nmonth; i++ )
	{
		TGraph* temp = getResidualGraph( v[i]->getGraph( value ), summer->getGraph( value ), col[i] );
		if( temp )
		{
			temp->Draw( "lp same" );
		}
