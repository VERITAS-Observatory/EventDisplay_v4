...
// --------------------------------------------------------------------------
    ...
    az2 = az1-Angle::sc_twoPi;
                                                  ...
                                                  // never happen
    if((az1>sc_lim_az_cw)&&(az2<sc_lim_az_cc))return false;
    else if(az1>sc_lim_az_cw)az=az2;
    else if(az2<sc_lim_az_cc)az=az1;
    else if(fabs(az1-tel_az_driveangle)<=fabs(az2-tel_az_driveangle))az=az1;
    else az=az2;
    ...
    if(el>Angle::sc_halfPi)      -- THIS IS WRONG WE JUST HAVE TO ALLOW THE
    {
        -- ELEVATION TO GO GREATER THAN 90 AND LESS
            el = Angle::sc_Pi - el;  -- THAN ZERO OR PREFERABLY USE THE INBUILT
            az = az + Angle::sc_Pi;  -- OFFSETS
    }
#endif
