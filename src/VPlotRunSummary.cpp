...
{
    if( !fIn->IsZombie() ) cout << "printing daily rates for " << fIn->GetName() << endl;
...
// only do something if there are runs in this phase
        if(  fPhaseRunlist[l].size() == 0 ) continue;
...
bool VPlotRunSummary::openfile( string file, int iTot )
{
    ...
    char hfile[200];
    if( iTot > 0 ) sprintf( hfile, "total_%d/stereo", iTot );
    else if( iTot < 0 ) sprintf( hfile, "total_%d/stereo", -1*iTot );
    else           sprintf( hfile, "total/stereo" );
    if( !fIn->cd( hfile ) ) return false;
...
// mean vs wobble direction (in steps of 0.05)
    ...
    for( int i = 0; i < i_nWobble; i++ ) i_ratevsWobbleDirectionX[i] = 0.;
    double i_ratevsWobbleDirectionXX[i_nWobble];
    for( int i = 0; i < i_nWobble; i++ ) i_ratevsWobbleDirectionXX[i] = 0.;
    double i_rateOffvsWobbleDirectionX[i_nWobble];
    for( int i = 0; i < i_nWobble; i++ ) i_rateOffvsWobbleDirectionX[i] = 0.;
    double i_rateOffvsWobbleDirectionXX[i_nWobble];
    for( int i = 0; i < i_nWobble; i++ ) i_rateOffvsWobbleDirectionXX[i] = 0.;
    double i_ratevsWobbleDirectionN[i_nWobble];
    for( int i = 0; i < i_nWobble; i++ ) i_ratevsWobbleDirectionN[i] = 0.;

    double i_SignificancevsWobbleDirectionX[i_nWobble];
    for( int i = 0; i < i_nWobble; i++ ) i_SignificancevsWobbleDirectionX[i] = 0.;
    double i_SignificancevsWobbleDirectionXX[i_nWobble];
    for( int i = 0; i < i_nWobble; i++ ) i_SignificancevsWobbleDirectionXX[i] = 0.;
    double i_SignificancevsWobbleDirectionN[i_nWobble];
    for( int i = 0; i < i_nWobble; i++ ) i_SignificancevsWobbleDirectionN[i] = 0.;

    double i_ElevationvsWobbleDirectionX[i_nWobble];
    for( int i = 0; i < i_nWobble; i++ ) i_ElevationvsWobbleDirectionX[i] = 0.;
    double i_ElevationvsWobbleDirectionXX[i_nWobble];
    for( int i = 0; i < i_nWobble; i++ ) i_ElevationvsWobbleDirectionXX[i] = 0.;
    double i_ElevationvsWobbleDirectionN[i_nWobble];
    for( int i = 0; i < i_nWobble; i++ ) i_ElevationvsWobbleDirectionN[i] = 0.;
...
// need additional counter because of skipped runs
    ...
    if( fMinRun < 0 ) fSetRunRange = true;
...
///// test run range
        if( !fSetRunRange && ( ( fMinRun > 0 && c->runOn < fMinRun ) || ( fMaxRun > 0 && c->runOn > fMaxRun ) ) ) continue;

        if( fSetRunRange && c->runOn > 0 && ( c->runOn < fMinRun || fMinRun < 0 ) ) fMinRun = c->runOn;
        if( fSetRunRange && c->runOn > fMaxRun ) fMaxRun = c->runOn;

// loop over list of runs to be excluded
        bBreak = false;
        for( unsigned int r = 0; r < fListofExcludedRuns.size(); r++ )
        {
            if( fListofExcludedRuns[r] == c->runOn ) bBreak = true;
        }
        if( bBreak ) continue;

        if( c->elevationOn < fMinElevation || c->elevationOn > fMaxElevation ) continue;
...
// 1D histgrams
            ...
            if( c->tOn > 0. ) gRatevsTime->SetPointError( t, 0., sqrt( c->NOn + c->OffNorm * c->OffNorm * c->NOff ) / c->tOn * 60.);

            gRateOffvsTime->SetPoint( t, c->MJDOn, c->RateOff );
            if( c->tOn > 0. ) gRateOffvsTime->SetPointError( t, 0., c->OffNorm * sqrt( c->NOff ) / c->tOn * 60.);
...
// elevation plots
            if( c->elevationOn > 0. ) gRatevsElevation->SetPoint( t, c->elevationOn, c->Rate );
            else                      gRatevsElevation->SetPoint( t, c->elevationOff, c->Rate );
            ...
            if( c->tOn > 0. ) gSignificancevsElevation->SetPoint( t, c->elevationOn, c->Signi/sqrt(c->tOn/3600.) );
            ...
            if( iEl < 1. ) iEl = c->elevationOff;
            if( c->elevationOff < 60. ) iElevationBin = 0;
            else if( c->elevationOff < 65. ) iElevationBin = 1;
            else if( c->elevationOff < 70. ) iElevationBin = 2;
            else if( c->elevationOff < 75. ) iElevationBin = 3;
            else iElevationBin = 4;
...
// rate vs elevation
    ...
    {
        if( i == 0 ) elevation = 55.;
        else if( i == 1 ) elevation = 62.5;
        else if( i == 2 ) elevation = 67.5;
        else if( i == 3 ) elevation = 72.5;
        else elevation = 80.;
...
{
    if( fNPhaseBins <= 0 || fPhaseOrbit == 0. ) return 0.;
...
// get date from mjd
        ...
        if( iDate.size() == 1 ) iNewDay = true;
        else if( iDate.size() > 1 && iDate[i-1] != iDate.back() ) iNewDay = true;
        else                                                      iNewDay = false;
...
// scale previous days
            if( fDayAlpha.size() > 0. )
            {
                if( fDayDuration.back() > 0. ) fDayAlpha.back() /= fDayDuration.back();
            }
// make new day
        ...
        else
        {
            ...
            {
                if( c->DeadTimeFracOn > 0. ) fDayNOn.back() += c->NOn * (1.+c->DeadTimeFracOn);
                if( c->DeadTimeFracOff > 0. ) fDayNOff.back() += c->NOff * (1.+c->DeadTimeFracOff);
            }
...
// day wise rates and significances
    ...
    for( unsigned int i = 0; i < fDayDate.size(); i++ )
    {
        ...
        if( TMath::Abs( ii_rate ) < 1.e-8 ) ii_rate = 0.;
...
// loop over list of runs to be excluded
        ...
        if( c->runOn == -1 ) cout << endl << " ALL RUNS (" << c->fChain->GetEntries()-1 << ")";
        else cout << " " << c->runOn;
        if( c->elevationOn > 0.5 ) cout << " " << setw(2) << (int)(c->elevationOn+0.5) << " deg";
        else                       cout << " " << setw(2) << (int)(c->elevationOff+0.5) << " deg";
...
// get date from mjd
        ...
        if( iDate.size() == 1 ) ifirstRun.push_back( 1 );
        else if( iDate.size() > 1 && iDate[i-1] != iDate.back() ) ifirstRun.push_back( 1 );
        else                                                      ifirstRun.push_back( 0 );
...
//      is << "    &      & &  phase   & duration & vation & offset (N,W) & & ($N_{\\mathrm{Off}}*\\alpha$)   & & [$\\gamma$/min] & ficance & [$\\gamma$/min] & sign. \\\\" << endl;
    ...
    for( unsigned int i = 0; i < c->fChain->GetEntries(); i++ )
    {
        ...
        if( ifirstRun[i] == 1 ) is << "\\hline" << endl;

        is << c->runOn << " & ";
        is << (int)iDate[i] << " & ";
//       is << iMJD[i] << " & ";
        cout << "now at date " << (int)iDate[i] << endl;
        if( iPhases ) is << setprecision( 2 ) << iPhase[i] << " & ";
        is << (int)(c->tOn/60.+0.5) << " min & ";
        if( c->elevationOn > 2. ) is << (int)(c->elevationOn+0.5) << "$^\\mathrm{o}$ & ";
        else                      is << (int)(c->elevationOff+0.5) << "$^\\mathrm{o}$ & ";
        is << "(";
        if( fabs( c->WobbleNorth ) < 1.e-5 ) is << 0;
        else  is << setprecision(1) << c->WobbleNorth;
        is << "$^\\mathrm{o}$, ";
        if( fabs( c->WobbleWest ) < 1.e-5 ) is << 0;
        else is << setprecision( 1 ) << c->WobbleWest;
        is << "$^\\mathrm{o}$";
        is << ") &";
        ...
        else                                                 is << setprecision( 2 ) << c->Rate << "$\\pm$" << setprecision( 2 ) << c->RateE << " & ";
...
// mean Crab rates for September 2006 (2-tel)
                    if( c->runOn < 31600 ) ig = 3.9;
// mean Crab rates for October 2006 (2-tel)
                    else if( c->runOn < 32100 ) ig = 4.4;
// mean Crab rates for November 2006 (2-tel)
                    else if( c->runOn < 32600 ) ig = 4.1;
// mean Crab rates for January 2007 (3-tel)
                    else if( c->runOn < 33500 ) ig = 7.4;
// mean Crab rates for February 2007 (3-tel)
                    else ig = 7.7;
//		  is << fDayRate[t]/ig << "$\\pm$";
// don't print daily rate in Crab units
//		  is << fDayRateE[t]/ig << " & ";
                    if( !iSignificancePerHour ) is << fDaySigni[t];
                    else
                    {
                        if( fDayDuration[t] > 0. ) is << fDaySigni[t]/sqrt(fDayDuration[t]/60.);
                        else                       is << 0 << endl;
                    }
                    is << " \\\\" << endl;
...
TCanvas* VPlotRunSummary::plot_rates( bool iOff )
{
    ...
    if( iOff ) g->GetHistogram()->SetYTitle( "background rate [1/min]" );
    else       g->GetHistogram()->SetYTitle( "#gamma-ray rate [1/min]" );
...
TCanvas* VPlotRunSummary::plot_ratevsElevation( bool iOff )
{
    ...
    TGraphErrors *g = 0;
    if( !iOff ) g = gRatevsElevation;
    else        g = gRateOffvsElevation;
    if( !g ) return 0;
...
{
    if( iList.size() == 0 ) fListofExcludedRuns.clear();
