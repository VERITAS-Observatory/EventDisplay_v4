...
{
	// parse argument
	if ( argc != 2 ) { cout << "Needs runnumber, exiting..." << endl; exit(1) ; }
	int runnumber = atoi( argv[1] ) ;
	if ( runnumber < 9999 or runnumber > 99999 ) { cout << "Needs valid runnumber (9999-99999)" << endl; exit(1); }
	...
	char query[1000] ;
	sprintf( query, "select run_id, data_start_time, data_end_time, config_mask from VERITAS.tblRun_Info where run_id = %d", runnumber ) ;
	//printf( "%s\n", query ) ;
	TSQLResult *db_res = f_db->Query( query );
    if( !db_res ) { cout << "TSQLResult empty, exiting..." << endl ; exit(1) ; }
	...
	{
		if ( ! a.test( i_tel ) ) continue ;
		sprintf( query, "select mjd, ra, decl from VOFFLINE.tblPointing_Monitor_Telescope%d_Calibrated_Pointing where mjd > %f and mjd < %f ;", i_tel, MJDStart, MJDEnd ) ;
		//printf( "%s\n", query) ;
