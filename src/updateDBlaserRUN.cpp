...
//--- indicate if the function has succeeded
    ...
    if(freANA_ALL_RUN)std::cout<<"WARNING: all RUN will be analysed event if the calibration already exist !!!!!!!!!!!!!!!!!!!!!!!!!"<<std::endl;
    if(ftestEVNDcalib)std::cout<<"REMINDER: Calibration checking, HARD coded name TTree read are: "<<fcalib_gain_tree_name <<" and "<<fcalib_toff_tree_name <<std::endl;
  ...
  //------------ getting the list of new laser run (present in the veritas DB but not in the VOFFLINE)
	...
	if(finput_list_run) create_list_for_download(new_laser_run_list_name);
...
//--- Create file for downloading the laser run
    ...
    if(fdownload_new_laser_run){ NEW_laser_list_File_for_download = fopen (NEW_laser_list_for_download.Data(),"w");}
...
//--- compare the list of laser run from VERITAS DB with the one from VOFFLINE
    ...
    if(fdownload_new_laser_run){ fclose(NEW_laser_list_File_for_download); }
...
void prepare_CalibVOFF_writing(TString new_laser_run_list_name){
	...
	if ((int) VERITAS_DB_LaserRunNumber[i] == (int) forget_this_run )continue;

	TString calib_status = "BAD";
	TString writing_status = "BAD";
	if(Summary_tel_list[i].compare(Summary_calib_status[i]) == 0) calib_status = "GOOD";
	if(Summary_tel_list[i].compare(Summary_writing_status[i]) == 0) writing_status = "GOOD";
...
bool test_gain_toff(TString file_root_name_gain,TString file_root_name_toff ,FILE* &pbFile){
	...
	if(fcurrent_run <63416 && fold_calib) tree_name = fcalib_gain_tree_name_old;
	...
	if( h_gz_channel->GetEntries() >=499){
		...
		if(h_gz_channel->GetBinContent(ii)>0)V_pixel_zero_gain.push_back(TMath::Nint(h_gz_channel->GetBinCenter(ii)-0.1));
	    }
	}
	...
	if(fcurrent_run<63416 && fold_calib)tree_name = fcalib_toff_tree_name_old;
	tree_name+=fcurrent_tel;
	...
	if( h_tz_channel->GetEntries() >=499){
	    ...
	    //-- loop on the pixel	   
	    for(int ii = 1 ; ii<= 500; ii++){
		if(h_tz_channel->GetBinContent(ii)>0)V_pixel_zero_toff.push_back(TMath::Nint(h_tz_channel->GetBinCenter(ii)-0.1));
	    }
...
//		    std::cout<<"VOFFLINE == VERITAS "<<std::endl;
	    ...
	    if(!run_in_voff_db){ std::cout<<"RUN "<< fcurrent_run <<": miss tel:" <<missing_tel.set(tel_indice,1) <<" in VOFFLINE"<<std::endl ;  } // construction of the mask for the telescopes missing from VOFFLINE DB
	}
...
void read_laserRUN_fromVOFFLINE_DB(vector < vector < unsigned int > > &VOFFLINE_DB_LaserRunNumber_Telnum,vector < vector < string > > &VOFFLINE_DB_LaserDate_Telnum, vector < vector < string > > &VOFFLINE_DB_LaserVersion_Telnum,unsigned int Tel_num){
	...
	while( TSQLRow *db_row = db_res->Next() ){
	    ...
	    if(atoi( db_row->GetField( 0 ) ) == forget_this_run) continue;
...
bool read_calib_file(TString file_root_name, vector < double > &Vchannel, vector < double > &Vmean, vector < double > &Vvar, bool bgain){
    ...
    TString want_mean = "";
    if(bgain){
	if(fcurrent_run <63416 && fold_calib){tree_name +=  "Gains_";}
	else{tree_name +=  "gain_";}
	tree_name += fcurrent_tel;
	want_mean = "gain";
    }else{
	if(fcurrent_run <63416 && fold_calib){ tree_name +=  "Toffs_";}
	else{tree_name +=  "toff_";}
	tree_name += fcurrent_tel;
	want_mean = "toff";
    }
...
vector<int> get_list_of_telescope(unsigned int config_mask,unsigned int excluded_tel){
	...
	int Tel = i+1;
	if(Tel_is_in_mask(Tel,config_mask) && !Tel_is_excluded(Tel,config_mask,excluded_tel)){ list_of_telescope.push_back(Tel); }
    }
...
vector<int> get_list_of_telescope(unsigned int mask){
    ...
    for(int i=0 ; i<fmax_number_tel ; i++){
	int Tel = i+1;
	if(Tel_is_in_mask(Tel,mask)){ list_of_telescope.push_back(Tel); }
    }
...
//----------------------------------------------
bool Tel_is_excluded(Int_t Tel,Int_t config_mask, Int_t excluded_tel){
    if( Tel_code_not_valid(config_mask) || Tel_code_not_valid(excluded_tel) ){ std::cout<<"WARNING.... "<<std::endl; return true;}
