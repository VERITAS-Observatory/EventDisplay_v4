...
#########################################################################################################################################
Revision $Id: VPlotArrayReconstruction.cpp,v 1.1.2.4.10.2 2010/05/04 07:32:01 gmaier Exp $
\author Gernot Maier
#########################################################################################################################################
*/
...
VPlotArrayReconstruction::VPlotArrayReconstruction( string iname, string ifile )
{
    ...
    if( t )  read_corsikaIOreader_file();
    else     read_eventdisplay_file();
}
...
{
    if( getEvent( iEventNumber ) < 0 ) return;

    if( fsize.size() != telPos_x.size() ) return;
    if( fsize.size() != telPos_y.size() ) return;
    if( fsize.size() != fltrig.size() ) return;
    if( fsize.size() != fwidth.size() ) return;
    if( fsize.size() != flength.size() ) return;
    if( fsize.size() != fdist.size() ) return;
    if( fsize.size() != floss.size() ) return;
    if( fsize.size() != fntubes.size() ) return;
...
int VPlotArrayReconstruction::getEvent( int iEventNumber )
{
    ...
    if( ftcors ) return get_corsikaIOreader_event( iEventNumber );
...
int VPlotArrayReconstruction::get_corsikaIOreader_event( int iEventNumber )
{
    ...
    else fEventNumber = -1*iEventNumber;
...
int VPlotArrayReconstruction::get_eventdisplay_event( int iEventNumber )
{
    if( fshowerpars )
    {
        ...
        else if( iEventNumber < fshowerpars->GetEntries() ) fshowerpars->GetEntry( iEventNumber );
        else
        {
...
// get min/max size or nphotons
    ...
    for( unsigned int i = 0; i < fsize.size(); i++ )
    {
        if( fsize[i] > 0 && log10( fsize[i] ) < iMin ) iMin = log10( fsize[i] );
        if( fsize[i] > 0 && log10( fsize[i] ) > iMax ) iMax = log10( fsize[i] );
    }
    for( unsigned int i = 0; i < fphotons.size(); i++ )
    {
        if( fphotons[i] > 0 && log10( fphotons[i] ) < iMin ) iMin = log10( fphotons[i] );
        if( fphotons[i] > 0 && log10( fphotons[i] ) > iMax ) iMax = log10( fphotons[i] );
    }
...
// distance between image centre and reconstruction direction
                ...
                if( fnimages[0] > 0 ) fPlotTelescopeScale = max( iMC_dist, iRE_dist )*1.1;
                else                  fPlotTelescopeScale = iMC_dist * 1.1;
