...
// define plot colors and styles
    ...
    fColorEstimated = 6;
    fColorDead = 13;
    if( fPlotPaper ) fColorDead = 0;
    fColorFADCTrig = 28;
    if( fPlotPaper ) fColorFADCTrig = 0;
    fColorSum = 4;
    fColorTrigger = 3;
    fColorHit = 2;
    fColorTiming = 4;
    ...
    fFillStyleDead = 1001;
    if( fPlotPaper ) fFillStyleDead = 0;
...
{
    if( fDebug ) cout << "VCamera::draw event " << iEventNumber << "\t" << fData->getTelID() << endl;
    if( fCanvas == 0 ) return;
    fScaleMax = i_max;
    ...
    if( iEventNumber != 0  )                      /* first time, there is no content in the tubes */
    {
        switch( fcameraModus )
        {
            ...
            case C_HIT:
                ...
                break;
            case C_SUMWINDOW:
                setPMTColorScheme( fData->getCurrentSumWindow(), false, 100., 0., "summation window", false, false );
                break;
            case C_SUMWINDOWSTART:
                setPMTColorScheme( fData->getTCorrectedSumFirst(), false, 100., 0., "summation window start", false, false );
                break;
            case C_PEDMEAN:
                if( !bFixScale ) setPMTColorScheme( fData->getPeds(), false, 100., 0., "charge", false );
                else             setPMTColorScheme( fData->getPeds(), false, 10., 25., "charge", false );
                break;
            case C_PEDVAR:
            {
        if( fData->getRunParameter()->fsourcetype != 7 || fData->getReader()->hasFADCTrace() )
{
   ...
   if( !bFixScale ) setPMTColorScheme( i_pedvars, false, 100., 0., "charge", false );
            ...
            break;
            case C_GAINS:
                if( !bFixScale ) setPMTColorScheme( fData->getGains(), false, 100., 0., "gain", false );
                else             setPMTColorScheme( fData->getGains(), false, 0.8, 1.2, "gain", false );
                break;
            case C_GAINVARS:
                if( !bFixScale ) setPMTColorScheme( fData->getGainvars(), false, 100., 0., "gain", false );
                else             setPMTColorScheme( fData->getGainvars(), false, 0.8, 1.2, "gain", false );
                break;
            case C_TOFF:
            ...
            case C_TZERO:
                ...
                break;
            case C_PEDMEANLOW:
                if( !bFixScale ) setPMTColorScheme( fData->getPeds(true), false, 100., 0., "charge", false );
            ...
            case C_GAINSLOW:
                ...
                break;
            case C_GAINVARSLOW:
                if( !bFixScale ) setPMTColorScheme( fData->getGainvars(true), false, 100., 0., "gain", false );
                else             setPMTColorScheme( fData->getGainvars(true), false, 0.8, 1.2, "gain", false );
                break;
            case C_TOFFLOW:
...
{
    if( fDebug ) cout << "VCamera::setPMTColorForChargeTiming() " << fData->getTelID() << "\t"<< fData->getReader()->getTelescopeID() << endl;

    if( fPMTData.size() != fData->getSums().size() ) fPMTData.resize( fData->getSums().size(), 0. );
// copy data (will be rescaled to fit into plotted tubes)
    for( unsigned int i = 0; i < fPMTData.size(); i++ ) fPMTData[i] = fData->getSums()[i];
...
// set tube radii
            ...
            if( fPMTData[i] >= 0. ) fgraphTubesEntry[i]->SetFillStyle( fFillStylePos );
            else fgraphTubesEntry[i]->SetFillStyle( fFillStyleNeg );
// image pixels
            if( fData->getImage()[i] )
            {
...
{
    if( fDebug ) cout << "VCamera::drawEventText()" << endl;
    char iText[600];

// big T1/T2... in lower right corner
//   red means telescope has triggered
    if( fData->getReader()->hasLocalTrigger( fTelescope ) ) fTextTelescopeN->SetTextColor( 2 );
    else                                                    fTextTelescopeN->SetTextColor( 1 );
    sprintf( iText, "T%d", fTelescope+1 );
    fTextTelescopeN->SetTitle( iText );
    fTextTelescopeN->SetNDC( true );
    if( !fBoolAllinOne ) fTextTelescopeN->DrawLatex( 0.85, 0.1, fTextTelescopeN->GetTitle() );
    else
    {
        sprintf( iText, "All" );
        fTextTelescopeN->SetTitle( iText );
...
// show only event number for fBoolAllinOne
        if( fBoolAllinOne && fTelescope == 0 ) i_toShow = 3;
        else if( fBoolAllinOne ) i_toShow = 1;
        for( unsigned int i = 0; i < i_toShow; i++ )
        {
            fTextEvent[i]->SetNDC( true );
            if( !fPlotPaper ) fTextEvent[i]->DrawLatex( i_TextX, i_TextY-=i_TextdY, fTextEvent[i]->GetTitle() );
        }
    }
// big letters for plotpaper options
    if( fPlotPaper && fTelescope == fData->getTeltoAna()[0] )
    {
        ...
        if( fCurrentTimeSlice >= 0 ) sprintf( iText, "%s FADC %d", iText, fCurrentTimeSlice );
...
// get local trigger list
    if( fBoolAllinOne )
    {
        ...
        {
            if( fData->getReader()->hasLocalTrigger( t ) ) sprintf( iText, "%s %d", iText, t+1 );
        }
    }
    else sprintf( iText, "Max channel %d", int( fData->getReader()->getMaxChannels() ) );
    fTextEvent[1]->SetTitle( iText );
    ...
    int i_numtrig = 0;
    int i_numtrig2 = 0;
    for( unsigned int i = 0; i < fData->getDead().size(); i++ ) if( fData->getDead()[i] ) i_numtrig++;
    for( unsigned int i = 0; i < fData->getDead(true).size(); i++ ) if( fData->getDead(true)[i] ) i_numtrig2++;
...
// show only event number for fBoolAllinOne
    if( fBoolAllinOne && fTelescope == 0 ) i_toShow = 2;
    else if( fBoolAllinOne ) i_toShow = 1;
    for( unsigned int i = 0; i < i_toShow; i++ )
    {
        fTextEvent[i]->SetNDC( true );
...
// draw results at bottom of canvas
    if( fData->getImageParameters()->ntubes > 0 && !fBoolAllinOne )
    {
        ...
        if( !fPlotPaper ) fTextEvent[fTextEvent.size()-2]->DrawLatex( i_TextX, 0.01, fTextEvent[fTextEvent.size()-2]->GetTitle() );
        if( fData->getRunParameter()->fImageLL )
        {
            fTextEvent[fTextEvent.size()-1]->SetNDC( true );
            if( !fPlotPaper ) fTextEvent[fTextEvent.size()-1]->DrawLatex( i_TextX, 0.04, fTextEvent[fTextEvent.size()-1]->GetTitle() );
        }
    }
...
void VCamera::setPMTColorScheme( vector<unsigned int> v_value, bool i_select, double zmin, double zmax, string i_axisTitle, bool i_scale, bool i_DrawDead, bool iLowGain )
{
    ...
    for( unsigned int i = 0; i < v_value.size(); i++ ) it[i] = (double)(v_value[i]+0.5);

    setPMTColorScheme( it, i_select, zmin, zmax, i_axisTitle, i_scale, i_DrawDead, iLowGain );
}


void VCamera::setPMTColorScheme( valarray<unsigned int> v_value, bool i_select, double zmin, double zmax, string i_axisTitle, bool i_scale, bool i_DrawDead, bool iLowGain )
{
    valarray<double> it(v_value.size());
    for( unsigned int i = 0; i < v_value.size(); i++ ) it[i] = (double)(v_value[i]+0.5);
...
// timing plot
    if( i_select )
    {
        ...
        for( unsigned int i = 0; i < v_value.size(); i++ )
        {
            ...
            {
                if( v_value[i] > i_max && v_value[i] < 64. ) i_max = v_value[i];
                if( v_value[i] < i_min && v_value[i] > 0. ) i_min = v_value[i];
            }
        }
// get maxima from input parameters (zmax=0.) or from above
        if( zmax == 0. ) wlmax = i_max * 1.005;
        else             wlmax = zmax;
...
// if all values equals zero set a range of +-0.1
    if( wlmax < wlmin ) { wlmax = 10.; wlmin = 0.; }
...
// plot axis on the right side
    for( int i = 0; i < fndivz; i++ )
    {
        ...
        w2 = wlmax;
        if (i < fndivz-1) w2 = wlmin + wls / fndivz * ( i + 1 );
...
// hit channels
        if( iDrawChannel )
        {
            if( fData->getImage()[i] ) scaler = 1.;
            else if( fData->getBorder()[i] ) scaler = 0.6;
            else
            {
...
//     else           scaler = 0.03;
            ...
            if( fData->getDeadRecovered(iLowGain)[i] ) fgraphTubesEntry[i]->SetFillStyle( 3013 );
        }
// dead channels
        else if( !fBoolAllinOne )
        {
...
{
    if( fDebug ) cout << "VCamera::setPMTColorOnOff" << endl;
    for( unsigned int i = 0; i < v_value.size(); i++ )
    {
        ...
        if( fBoolAllinOne )
{
    fAnaEllipse->SetLineColor( fTelescopeEllipseColor );
        ...
        {
            if( fData->getImageParameters()->Fitstat < 0. ) fAnaEllipse->SetLineColor( 7 );
            else                                            fAnaEllipse->SetLineColor( 5 );
        }
        fAnaEllipse->Draw();
        ...
        if( fBoolAllinOne )
{
   fAnaEllipse1->SetLineColor( fTelescopeEllipseColor );
        ...
        {
            if( fData->getImageParameters()->Fitstat < 0. ) fAnaEllipse1->SetLineColor( 7 );
            else                                            fAnaEllipse1->SetLineColor( 5 );
        }
...
// draw a line from the center through the long axis of the ellipse
            ...
            if( fBoolAllinOne )
    {
       fEllipseLine->SetLineColor( fTelescopeEllipseColor );
            ...
            else                 fEllipseLine->SetLineStyle( 1 );
...
{
    if( fData->getShowerParameters()->fShower_Weight_DISP[iMethod][d] > 0.5 ) fAnaShowerDir->SetMarkerColor( 4 );
    else                                                                      fAnaShowerDir->SetMarkerColor( 3 );
                                }
                                fAnaShowerDir->SetMarkerSize( 2. );
                                fAnaShowerDir->SetMarkerStyle( 34 );
                                fAnaShowerDir->Draw();
                            }
                        }
                    }
...
void VCamera::showSelectedChannel( int i_channel, bool i_delete )
{
    fCanvas->SetEditable( true );
    fCanvas->cd();
    if( i_delete ) hideSelectedChannel();
...
void VCamera::hideSelectedChannel()
{
    if( fTubeSelected >= 0 && fTubeSelected < (int)fgraphTubes.size() )
    {
        fgraphTubes[fTubeSelected]->SetLineWidth( 1 );
        fgraphTubes[fTubeSelected]->SetLineColor( 15 );
        fgraphTubes[fTubeSelected]->Draw();
    }
    fTubeSelected = -1;
}
...
{
    if( fDebug ) cout << "VCamera::rescaleSums " << v_value.size() << "\t" << fData->getReader()->getFullHitVec().size() << endl;
    double imax = -999999;
    double imin = 999999.;
...
// maximum/minimum
        for( unsigned i = 0; i < i_v_valueSize; i++ )
        {
            ...
            {
                if( abs(v_value[i]) > imax ) imax = abs(v_value[i]);
                if( abs(v_value[i]) < imin ) imin = abs(v_value[i]);
            }
        }
    }
    else imax = fScaleMax;
    ...
    if( iOffset )
    {
        for( unsigned i = 0; i < i_v_valueSize; i++ ) v_value[i] -= imin;
        imax -= imin;
    }
// rescale to maximum = 1.
    if( imax != 0. ) { for( unsigned i = 0; i < i_v_valueSize; i++ ) v_value[i] /= imax; }
    return v_value;
}
...
//   for( unsigned int i = 0; i < i_val.size(); i++ ) if( !fData->getDead()[i] ) { max = i_val[i]; break; }
    for( unsigned int i = 0; i < iSize; i++ ) if( !fData->getDead()[i] && i_val[i] > max ) max = i_val[i];
    return max;
}
...
//   for( unsigned int i = 0; i < i_val.size(); i++ ) if( !fData->getDead()[i] ) { min = i_val[i]; break; }
    for( unsigned int i = 0; i < iSize; i++ ) if( !fData->getDead()[i] && i_val[i] < min ) min = i_val[i];
    return min;
}


void VCamera::getMinMax( valarray<double>& i_val, double& imin, double& imax )
{
    unsigned int iSize = i_val.size();
    if( iSize == 0 )
    {
    ...
    for( unsigned int i = 0; i < iSize; i++ )
    {
        ...
        {
            if( i_val[i] < min ) min = i_val[i];
            if( i_val[i] > max ) max = i_val[i];
        }
    }
    imin = min;
    imax = max;
