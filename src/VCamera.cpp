	...
	if( iEventNumber != 0 )                       /* first time, there is no content in the tubes */
	{
		switch( fcameraModus )
		{
			case C_CHARGE:
				if( fPlotColor )
				{
					setPMTColorScheme( fData->getSums(), false,  100., 0., "charge [d.c.]", true );
				}
				else
				{
			...
			case C_TIMING:
				if( fPlotColor )
				{
					setPMTColorScheme( fData->getSums(), false,  0., fScaleMax, "charge [d.c]", true );
				}
				else
				{
					setPMTColorForChargeTiming();
			...
			case C_PEDMEAN:
				if( !bFixScale )
				{
					setPMTColorScheme( fData->getPeds(), false, 100., 0., "charge", false );
				}
				else
				{
					setPMTColorScheme( fData->getPeds(), false, 10., 25., "charge", false );
				}
				break;
			case C_PEDVAR:
			{
				if( fData->getRunParameter()->fsourcetype != 7 || fData->getReader()->hasFADCTrace() )
				{
					...
					if( !bFixScale )
					{
						setPMTColorScheme( i_pedvars, false, 100., 0., "charge", false );
					}
					else
					{
						setPMTColorScheme( i_pedvars, false, 5., 20., "charge", false );
					}
				}
			}
			break;
			case C_GAINS:
				if( !bFixScale )
				{
					setPMTColorScheme( fData->getGains(), false, 100., 0., "gain", false );
				}
				else
				{
					setPMTColorScheme( fData->getGains(), false, 0.8, 1.2, "gain", false );
				}
				break;
			case C_GAINVARS:
				if( !bFixScale )
				{
					setPMTColorScheme( fData->getGainvars(), false, 100., 0., "gain", false );
				}
				else
				{
					setPMTColorScheme( fData->getGainvars(), false, 0.8, 1.2, "gain", false );
				}
				break;
			case C_TOFF:
			...
			case C_PEDMEANLOW:
				if( !bFixScale )
				{
					setPMTColorScheme( fData->getPeds( true ), false, 100., 0., "charge", false );
				}
				else
				{
					setPMTColorScheme( fData->getPeds( true ), false, 10., 25., "charge", false );
				}
				break;
			case C_PEDVARLOW:
			{
			...
			case C_GAINSLOW:
				if( !bFixScale )
				{
					setPMTColorScheme( fData->getGains( true ), false, 100., 0., "gain", false );
				}
				else
				{
					setPMTColorScheme( fData->getGains( true ), false, 0.8, 1.2, "gain", false );
				}
				break;
			case C_GAINVARSLOW:
				if( !bFixScale )
				{
					setPMTColorScheme( fData->getGainvars( true ), false, 100., 0., "gain", false );
				}
				else
				{
					setPMTColorScheme( fData->getGainvars( true ), false, 0.8, 1.2, "gain", false );
				}
				break;
			case C_TOFFLOW:
				...
				if( iMethod < fData->getShowerParameters()->fShower_Xoff_DISP.size() )
				{
					...
					for( unsigned int d = d_min; d < d_max; d++ )
					{
						if( fData->getShowerParameters()->fShower_Xoff_DISP[iMethod][d] > -90. )
						{
							...
							if( fBoolAllinOne )
							{
								fAnaShowerDir->SetMarkerColor( d + 1 );
							}
							else
							{
