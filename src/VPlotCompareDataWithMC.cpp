...
VPlotCompareDataWithMC::VPlotCompareDataWithMC( string iFile )
{
   ...
   fDataFileName = iFile;
   fDataFile = 0;
   if( fDataFileName.size() > 0 ) openDataFile( fDataFileName );
...
void VPlotCompareDataWithMC::help()
{
  ...
  cout << "mscw/mscl energy dependent:      msc_plots()  " << endl << endl;
  cout << "mwr/mlr energy dependent:        mwr_plots()  " << endl << endl;
...
void VPlotCompareDataWithMC::drawMatchingTests( TH1D* h1, TH1D* h2, double x_min, double x_max )
{
   ...
   if( x_min < -998. ) x_min = h1->GetXaxis()->GetXmin();
   if( x_max < -998. ) x_max = h1->GetXaxis()->GetXmax();
...
// scale to same contents (integral)
  if( bContents == 1 )
  {
      ...
      if( xmax <  9998 ) i_max = h_sims->GetXaxis()->FindBin( xmax );
      ...
      if( xmin > -9998 ) i_min = h_diff->GetXaxis()->FindBin( xmin );
      if( xmax <  9998 ) i_max = h_diff->GetXaxis()->FindBin( xmax );
      for( int i = i_min; i <= i_max; i++ )
	{
	  if( h_diff->GetBinContent( i ) > 0 ) z += h_diff->GetBinContent( i );
	}
...
// scale to peak (three bins around maximum)
  else if( bContents == 3 )
  {
      ...
      if( imaxbin < h_sims->GetNbinsX() )  s_sims += h_sims->GetBinContent( imaxbin + 1 );

      imaxbin = h_diff->GetMaximumBin();
      z = h_diff->GetBinContent( imaxbin );
      if( imaxbin > 1 ) z += h_diff->GetBinContent( imaxbin - 1 );
...
void VPlotCompareDataWithMC::setHistogramAtt( TH2D* his, double imin )
{
  ...
  if( imin > -900. ) his->SetMinimum( imin );
  his->SetStats( 0 );
...
void VPlotCompareDataWithMC::plotRelativePlot( TH1D *h1, TH1D *h2, double xmin, double xmax )
{
  ...
  if( xmin < -998. ) xmin = h1->GetXaxis()->GetXmin();
  if( xmax < -998. ) xmax = h1->GetXaxis()->GetXmax();
...
// get the scaling between simulations and data
  ...
  if( !hHistogram_SIMS || !hHistogram_DIFF ) return;
...
*/
void VPlotCompareDataWithMC::msc_vs_energy_plots( int iRebin, double xmin, double xmax )
{
  if( !fDataFile ) return;
 ...
 *
 */
void VPlotCompareDataWithMC::plot_energyDependentDistributions( string iVariable, int iRebin, double x_min, double x_max, string iPlot )
{
  if( !fDataFile ) return;
 ...
 *
 */
void VPlotCompareDataWithMC::stereo_parameter( int msc_rebin )
{
  if( !fDataFile ) return;
...
void VPlotCompareDataWithMC::core_plots( int iRebin, int iScaling )
{
  ...
  TCanvas *cSCoreRel = new TCanvas( "cSCoreRel", "sims/diff (core positions)", 10, 110, 600, 330 );
...
*/
void VPlotCompareDataWithMC::distance_plots()
{
  if( !fDataFile ) return;
  ...
  for( int i = 0; i < 100; i++ ) cdistsim[i] = 0;
  if( fNTel > 100 ) 
  {
  ...
  for( unsigned int i = 0; i < fNTel; i++ )
    {
      ...
      setHistogramAtt( hR_sims[i], 2, 1, 1, 20, 1 );
      ...
      setHistogramAtt( hR_diff[i], 1, 1, 1, 21, 1 );

      sprintf( hname, "hR%d_ON", i+1 );
      hR_on[i] = (TH1D*)fDataFile->Get( hname );
      setHistogramAtt( hR_on[i], 3, 1, 1, 20, 1 );

      sprintf( hname, "hR%d_OFF", i+1 );
      hR_off[i] = (TH1D*)fDataFile->Get( hname );
      setHistogramAtt( hR_off[i], 4, 1, 1, 21, 1 );
   
      sprintf( hname, "R%d", i+1 );
      getScaling( s_sims, s_diff, hname, 1 );
      if( hR_sims[i]->GetEntries() > 0 ) hR_sims[i]->Scale( s_sims );
      if( hR_diff[i]->GetEntries() > 0 ) hR_diff[i]->Scale( s_diff );
...
void VPlotCompareDataWithMC::single_telescope( int telid )
{
   if( telid > 0 )
   {
       single_telescope( telid, "SIMSDIFF" );
       single_telescope( telid, "REL" );
   }
   else
   {
      for( unsigned int i = 0; i < fNTel; i++ )
...
void VPlotCompareDataWithMC::single_telescope( int telid, string iPlot, bool iOneCanvas, 
...
// relative histograms
      ...
      if( fPlotPoster ) setHistogramAtt( hsims, 2, 3, 2, 20, 1 );
      setHistogramAtt( hdiff, 1, 0.5, 1, 21, 1 );
      if( fPlotPoster ) setHistogramAtt( hdiff, 1, 3, 2, 21, 1 );
      setHistogramAtt( hon, 3, 1, 1, 20, 1 );
      if( fPlotPoster ) setHistogramAtt( hon, 2, 3, 2, 20, 1 );
      setHistogramAtt( hoff, 4, 1, 1, 21, 1 );
      if( fPlotPoster ) setHistogramAtt( hoff, 1, 3, 2, 21, 1 );
      setHistogramAtt( hrel, 9, 1, 1, 20, 1 );
      if( fPlotPoster ) setHistogramAtt( hrel, 1, 3, 2, 21, 1 );
...
// relative plots
      ...
      if( !fPlotPoster && iPlot != "REL" ) iL->Draw();
...
void VPlotCompareDataWithMC::msc_plots( char *offFile, char *helium, char *proton, double xmin, double xmax, string ivar )
{
  ...
  if( hMSC_sims->GetEntries() > 0 ) hMSC_sims->Scale( s_sims );
  if( hMSC_diff->GetEntries() > 0 ) hMSC_diff->Scale( s_diff * 1.15 );
  if( hMSC_off->GetMaximum() > 0 ) hMSC_off->Scale( hMSC_diff->GetMaximum() / hMSC_off->GetMaximum() / 2. );

  cMSCsim->cd();
  hMSC_sims->SetMaximum( hMSC_sims->GetMaximum() * 1.3 );
  if( ivar == "MSCW" ) hMSC_sims->SetXTitle( "mean scaled width" );
  else                 hMSC_sims->SetXTitle( "mean scaled length" );
  hMSC_sims->Draw( "e" );
  ...
  if( offFile )
      ...
      if( !fOff->IsZombie() )
	{
	  ...
	  if( ivar == "MSCW" ) sprintf( hname, "hmscw_off" );
	  else if( ivar == "MSCL" ) sprintf( hname, "hmscl_off" );
	  ...
	  if( hMSC_off_ana->GetMaximum() > 0 ) hMSC_off_ana->Scale( hMSC_diff->GetMaximum() / hMSC_off_ana->GetMaximum() / 2. );
  ...
  if( helium && proton )
      ...
      if( !fP->IsZombie() )
	{
      ...
      if( hCR->GetMaximum() > 0. ) hCR->Scale( hMSC_diff->GetMaximum() / hCR->GetMaximum() / 2. * 1.05 );
      hCR->Draw( "same" );
    }
...
*/
void VPlotCompareDataWithMC::mwr_vs_energy_plots( int iRebin, double xmin, double xmax )
{
  if( !fDataFile ) return;
