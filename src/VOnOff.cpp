...
// set names and titles
    ...
    itemp = his->GetTitle();
    if( itemp.rfind( "(on)" ) < itemp.size() ) itemp.replace( itemp.rfind( "(on)" ), 4, ititle );
    else itemp += ititle;
    his->SetTitle( itemp.c_str() );
    if( ytitle.size() > 0 ) his->GetYaxis()->SetTitle( ytitle.c_str() );
}
...
// fill the corresponding lists
        if( itemp.find( "herec" ) == 0 || itemp.find( "hLinerec" ) == 0 ) hListEnergyHistograms->Add( hTemp );
        else if( itemp.find( "hrf" ) == 0 ) hListRandomForestHistograms->Add( hTemp );
        else hListStereoParameterHistograms->Add( hTemp );
        hList->Add( hTemp );
    }
    hList->Add( hPList );
}
...
{
    if( fDebug ) cout << "VOnOff::doQfactors" << endl;
    hQList->Clear();
...
// qfactor on low bound cut
            ...
            for( int i = i_nbins; i > 0; i-- )
            {
                ...
                if( i_sumon + i_sumoff > 0 ) hTemp->SetBinContent( i, VStatistics::calcSignificance(i_sumon,i_sumoff,i_norm) );
...
{
    if( fDebug ) cout << "VOnOff::do2DSignificance" << endl;
    ...
    for( int j = 1; j <= hmap_stereo_sig->GetNbinsX(); j++ )
    {
        ...
        for( int k = 1; k <= hmap_stereo_sig->GetNbinsY(); k++ )
        {
            ...
            else                hmap_stereo_sig->SetBinContent( j, k, -9999. );
...
TH2D *VOnOff::do2DSignificance( TH2D *ion, TH2D *ioff, double i_norm )
{
    ...
    for( int j = 0; j < hmap_stereo_sig->GetNbinsX(); j++ )
    {
        for( int k = 0; k < hmap_stereo_sig->GetNbinsY(); k++ )
        {
            double on  = ion->GetBinContent(j,k);
            double off = ioff->GetBinContent(j,k);
            if ( (on+off) > 0.) hmap_stereo_sig->SetBinContent( j, k, VStatistics::calcSignificance( on, off, i_norm ) );
            else                hmap_stereo_sig->SetBinContent( j, k, 0. );
            if( hmap_stereo_sig->GetBinContent( j, k ) > i_sigmax )
            {
                i_sigmax = hmap_stereo_sig->GetBinContent( j, k );
                i_sigon = on;
...
// write all sky plots into sky histogram directory
    ...
    if( hListSkyHistograms ) hListSkyHistograms->Write();
    if( hSig ) hSig->Write();
    if( hSigUC ) hSigUC->Write();
...
// write all stereo parameter histograms
    ...
    if( hListStereoParameterHistograms ) hListStereoParameterHistograms->Write();
...
// write all energy histograms
    ...
    if( hListEnergyHistograms ) hListEnergyHistograms->Write();
...
// write all random forest histograms
    ...
    if( hListRandomForestHistograms ) hListRandomForestHistograms->Write();

// write all quality factor histograms
    iDir->cd();
    wDir = (TDirectory*)iDir->Get( "qualityFactorHistograms" );
    if( !wDir ) iDir->mkdir( "qualityFactorHistograms" )->cd();
    else wDir->cd();
    if( hQList ) hQList->Write();
...
void VOnOff::fill1DSignificanceHistogram( double rmax )
{
    ...
    for( int j = 0; j < hmap_stereo_sig->GetNbinsX(); j++ )
    {
        for( int k = 0; k < hmap_stereo_sig->GetNbinsY(); k++ )
        {
            ...
            ir  = sqrt( ir );
            if( ir < rmax && hmap_stereo_sig->GetBinContent( j, k ) != 0. ) h1Dsig->Fill( hmap_stereo_sig->GetBinContent( j, k ) );
        }
    }
}
...
// do nothing for uncorrelated plots
    string itemp;
    itemp = h->GetName();
    if( itemp.find( "UC" ) < itemp.size() ) return;
...
{
   if( TMath::Abs( fMaxSigma ) < 1.e-10 ) return 0.;
...
{
   if( TMath::Abs( fMaxSigmaX ) < 1.e-10 ) return 0.;
...
{
   if( TMath::Abs( fMaxSigmaY ) < 1.e-10 ) return 0.;
