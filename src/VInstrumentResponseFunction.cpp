...
{
   if( fSpectralWeight ) fSpectralWeight->setMCParameter( iMCIndex, iMin, iMax );
}
...
bool VInstrumentResponseFunction::initialize( string iName, string iType, unsigned int iNTel, double iMCMaxCoreRadius, double iZe, int iNoise, double iPedvars, double iXoff, double iYoff )
{
   ...
   for( unsigned int i = 0; i < fVSpectralIndex.size(); i++ )
   {
      ...
      for( unsigned int j = 0; j < fVMinAz.size(); j++ )
      {
	     ...
	     if( !i_irf.back()->initialize( hname, iType, iNTel, iMCMaxCoreRadius ) ) return false;
...
// confine MC az to -180., 180.
		 if( fData->MCaz > 180. ) fData->MCaz -= 360.;
// expect bin like [135,-135]
		 if( fVMinAz[i_az] > fVMaxAz[i_az] )
		 {
		     if( fData->MCaz < fVMinAz[i_az] && fData->MCaz > fVMaxAz[i_az] ) continue;
		 }
...
// fill histograms
	       ...
	       {
	          if( fIRFData[s][i_az] ) fIRFData[s][i_az]->fill( i_weight );
               }
            }
        }
    }
...
// fill resolution graphs
   ...
   for( unsigned int i = 0; i < fIRFData.size(); i++ )
   {
      ...
      {
          if( fIRFData[i][j] ) fIRFData[i][j]->terminate( fContainmentProbability, fContainmentProbabilityError );
      }
   }
...
void VInstrumentResponseFunction::setHistogrambinning( int iN, double iMin, double iMax )
{
   for( unsigned int i = 0; i < fIRFData.size(); i++ )
   {
      for( unsigned int j = 0; j < fIRFData[i].size(); j++ )
      {
          if( fIRFData[i][j] ) fIRFData[i][j]->setHistogrambinning( iN, iMin, iMax );
      }
   }
}
...
void VInstrumentResponseFunction::setDataTree( CData *iData )
{
   ...
   for( unsigned int i = 0; i < fIRFData.size(); i++ )
   {
      for( unsigned int j = 0; j < fIRFData[i].size(); j++ )
      {
          if( fIRFData[i][j] ) fIRFData[i][j]->setDataTree( fData );
      }
   }
}
...
void VInstrumentResponseFunction::setCuts( VGammaHadronCuts* iCuts ) 
{
   ...
   if( fAnaCuts )
   {
	 ...
	 for( unsigned int j = 0; j < fIRFData[i].size(); j++ )
	 {
	     if( fIRFData[i][j] ) fIRFData[i][j]->setArrayCentre( fAnaCuts->getArrayCentre_X(), fAnaCuts->getArrayCentre_Y() );
	 }
      }
   }
}
