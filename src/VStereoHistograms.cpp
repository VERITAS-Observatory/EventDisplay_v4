...
//! setup unrotated sky map
    ...
    hxyoff_stereo = new TH2D( i_key, i_name, xbin, xmin, xmax, ybin, ymin, ymax );
    hxyoff_stereo->SetXTitle("X-position on Camera (#circ)");
    hxyoff_stereo->SetYTitle("Y-position on Camera (#circ)");
...
// this histogram must have the same binning definition as hmap_stereo !!!!
    ...
    if( hisList ) hisList->Add( hmap_alphaNorm );
...
//! setup energy histogram (logarithmic energy axis)
    sprintf(i_key,"herec_%s", fHisSuffix.c_str());
    sprintf( i_name, "energy spectrum (%s)", fHisSuffix.c_str());
    herec = new TH1D( i_key, i_name, i_ebin, i_emin, i_emax );
    herec->SetXTitle( "log_{10} energy [TeV]" );
    herec->SetYTitle( "dN/dE [cm^{-2} s^{-1} TeV^{-1}]" );
    hisList->Add( herec );
    hListParameterHistograms->Add( herec );
    ...
    sprintf(i_key,"herecRaw_%s", fHisSuffix.c_str());
    sprintf( i_name, "differential unweighted energy spectrum (%s)", fHisSuffix.c_str());
    herecRaw = new TH1D( i_key, i_name, i_ebin, i_emin, i_emax );
    herecRaw->SetXTitle( "log_{10} energy [TeV]" );
    herecRaw->SetYTitle( "dN/dE [TeV^{-1}]" );
    hisList->Add( herecRaw );
    ...
    herecEffectiveArea->SetXTitle( "log_{10} energy [TeV]" );
    herecEffectiveArea->SetYTitle( "effective area [m^{2}]" );
    hisList->Add( herecEffectiveArea );
...
//! setup energy histogram (linear energy axis)
    sprintf(i_key,"hLinerec_%s", fHisSuffix.c_str());
    sprintf( i_name, "energy spectrum (%s)", fHisSuffix.c_str());
    hLinerec = new TH1D( i_key, i_name, i_Linebin, i_Linemin, i_Linemax );
    hLinerec->SetXTitle( "energy [TeV]" );
    hLinerec->SetYTitle( "dN/dE [cm^{-2} s^{-1} TeV^{-1}]" );
    hisList->Add( hLinerec );
    hListParameterHistograms->Add( hLinerec );
    ...
    hLinerecRaw->SetXTitle( "energy [TeV]" );
    hLinerecRaw->SetYTitle( "dN/dE [TeV^{-1}]" );
    hisList->Add( hLinerecRaw );
    hListParameterHistograms->Add( hLinerecRaw );
    ...
    hLinerecEffectiveArea->SetXTitle( "log_{10} energy [TeV]" );
    hLinerecEffectiveArea->SetYTitle( "effective area [m^{2}]" );
    hisList->Add( hLinerecEffectiveArea );
...
void VStereoHistograms::setAlphaOff( TH2D *ioff, bool bUncorrelated )
{
    if( ioff )
    {
        if( !bUncorrelated ) hmap_alpha_off   = ioff;
        else                 hmap_alpha_offUC = ioff;
    }
}
...
// write all sky plots into sky histogram directory
    ...
    if( !wDir ) iDir->mkdir( "skyHistograms" )->cd();
    else wDir->cd();
    hListSkyMaps->Write();
...
// write all stereo parameter histograms
    ...
    if( !wDir ) iDir->mkdir( "stereoParameterHistograms" )->cd();
    else wDir->cd();
...
// write all energy histograms
    ...
    if( !wDir ) iDir->mkdir( "energyHistograms" )->cd();
    else wDir->cd();
...
// write all random forest histograms
    ...
    if( !wDir ) iDir->mkdir( "randomForestHistograms" )->cd();
    else wDir->cd();
...
// write all rate histograms
    ...
    if( !wDir ) iDir->mkdir( "rawRateHistograms" )->cd();
    else wDir->cd();
    hisRateList->Write();
    hisRateList->Delete();
...
bool VStereoHistograms::readParameterHistograms()
{
    ...
    if( !readHistograms( hListStereoParameterHistograms, "stereoParameterHistograms" ) ) return false;
    iDir->cd();
    if( !readHistograms( hListRandomForestParameterHistograms, "randomForestHistograms" ) ) return false;
    iDir->cd();
    if( !readHistograms( hListEnergyHistograms, "energyHistograms" ) ) return false;
    iDir->cd();
    if( !readHistograms( hisRateList, "rawRateHistograms" ) ) return false;
    iDir->cd();
...
// get only on/off histograms
        ...
        bool bRead = false;
        if( bIsOn && iTemp.find( "_on" ) < iTemp.size() ) bRead = true;
        if( !bIsOn && iTemp.find( "_off" ) < iTemp.size() ) bRead = true;

        if( bRead )
        {
            for( iter = hListNameofParameterHistograms.begin(); iter != hListNameofParameterHistograms.end(); iter++ )
            {
                if( iTemp.find( iter->first ) < iTemp.size() ) iter->second = (TH1*)ho;
            }
...
// write all sky plots into sky histogram directory
    ...
    if( !wDir ) wDir = (TDirectory*)gDirectory->Get( "stereo/skyHistograms" );
    if( !wDir )
    {
...
// get only on and off histograms
        ...
        bool bFill = false;
        if( bIsOn && iTemp.find( "_on" ) < iTemp.size()  ) bFill = true;
        if( !bIsOn && iTemp.find( "_off" ) < iTemp.size() ) bFill = true;
        if( iTemp.find( "hmap_alphaUC_off" ) < iTemp.size() ) bFill = true;
        if( iTemp.find( "hmap_alpha_off" ) < iTemp.size() ) bFill = true;
        ...
        if( bFill )
        {
            if( iTemp.find( "UC" ) < iTemp.size() )
            {
                ...
                {
                    hListSkyMapsUC->Add( ho );
                    if( iTemp.find( "hmap_stereoUC" ) < iTemp.size() ) hmap_stereoUC = (TH2D*)ho;
                    if( iTemp.find( "hmap_alphaNormUC" ) < iTemp.size() ) hmap_alphaNormUC = (TH2D*)ho;
                    if( iTemp.find( "hmap_alphaUC" ) < iTemp.size() ) hmap_alphaUC = (TH2D*)ho;
                }
            }
            else
            {
                ...
                {
                    hListSkyMaps->Add( ho );
                    if( iTemp.find( "hmap_stereo" ) < iTemp.size() ) hmap_stereo = (TH2D*)ho;
                    else if( iTemp.find( "hmap_alphaNorm" ) < iTemp.size() ) hmap_alphaNorm = (TH2D*)ho;
                    else if( iTemp.find( "hmap_alpha" ) < iTemp.size() ) hmap_alpha = (TH2D*)ho;
                    else if( iTemp.find( "hxyoff_stereo" ) < iTemp.size() ) hxyoff_stereo = (TH2D*)ho;
                }
            }
        }
        else ho->Delete();
...
{
    if( !hisRateList ) return;

    if( !hrate_1sec || !hrate_10sec || !hrate_1min ) return;
...
void VStereoHistograms::writeObjects( string iFile, string iDirectory, TObject *g )
{
    ...
    if( iDirectory.size() > 0 )
    {
        ...
        if( !wDir ) iDir->mkdir( iDirectory.c_str() )->cd();
        else        wDir->cd();
    }
    wDir = gDirectory;
...
//   cout << "\t\t copying objects from file " << iFile << " to directory " << iDirectory << endl;
// open input file
