...
void            VTimeMask::initialise( Int_t run_number, Double_t run_start, Double_t run_end, string file_name )
{
    ...
    if( !file_name.empty() ) mask_file = file_name;
    ...
    if ( run_end - run_start > secs_day )   problem( "run duration greater than one day" );
    else if ( run_end < run_start )         problem( "end time before start time" );
    else                                    duration_seconds = UInt_t( ceil( run_end - run_start ) );
...
// Check sanity of user input (oh, those crazy users!)
                ...
                if ( mask_width < 1 ) continue;   // Mask width should be finite
                                                  // Mask mode should represent a boolean
                if ( mask_mode != 0 && mask_mode != 1 ) continue;
...
//printMask(100);
    ...
    if ( set_success )  cout <<"\t -> mask is open for "<< getEffectiveDuration() <<" secs."<< endl;
    else        cout <<"\t -> default mask is open for entire duration of run."<< endl;
...
// Let's start at the beginning
    if ( mask.empty() ) start_time = now;

    Int_t instant =  Int_t( floor( now - start_time ) );
    if ( instant < 0 ) return kFALSE;
...
{
    UInt_t count=0;
    for ( UInt_t t=0; t < mask.size(); t++ ) count += checked.at( t );
    return count;
}
...
{
    UInt_t count=0;
    for ( UInt_t t=0; t < mask.size(); t++ ) count += accepted.at( t );
    return count;
}
...
{
    UInt_t count=0;
    for ( UInt_t t=0; t < mask.size(); t++ ) count += counted.at( t );
    return count;
}
...
// Accumulate weighted sum and weighted count
    for ( UInt_t t=0; t < mask.size(); t++ )
    {
        if ( mask.at(t) ) sum   += ( t + 0.5 );   // Weight by center of mask bin
    }
...
void            VTimeMask::printMeanTime( Bool_t event_statistics, ostream &terminal ) const
{
    ...
    Double_t    event_res       = 0.;       if ( event_statistics ) event_res       = getMeanTime_Events();
    Double_t    accepted_res    = 0.;       if ( event_statistics ) accepted_res    = getMeanTime_Accepted();
...
//Display on STDOUT or STDERR
    ...
    if ( event_statistics ) terminal        <<  event_str.str() << endl;
    if ( set_success ) terminal             <<  open_str.str() << endl;
    if ( event_statistics && set_success ) terminal     <<  accepted_str.str() << endl;
...
{
    if ( interval_seconds == 0 ) interval_seconds = 100;
...
// so under normal circumstances you can expect up to two events outside the mask domain.
    ...
    if ( set_success ) terminal << open_str.str()       << endl;
    if ( set_success ) terminal << openFrac_str.str()       << endl;

    if ( event_statistics )
    {
        terminal    << checked_str.str()            << endl;
        if ( set_success ) terminal << accepted_str.str()       << endl;
        if ( set_success ) terminal << acceptedFrac_str.str()   << endl;
    }
...
void            VTimeMask::writeObjects() const
{
    ...
    if( !wDir ) iDir->mkdir( "timeMask" )->cd();
    else        wDir->cd();
