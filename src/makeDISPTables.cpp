...
// run timing
    ...
    if( !readInputParameter( argv[1] ) ) exit( 0 );
...
// loop over all files
    for( unsigned int i = 0; i < f_ze.size(); i++ )
    {
		...
		if( iName.find( "__ZENITHANGLE__" ) != string::npos )  iName.replace( iName.find( "__ZENITHANGLE__" ), 15, f_ze[i] );
                if( iName.find( "__WOBBLEOFFSET__" ) != string::npos ) iName.replace( iName.find( "__WOBBLEOFFSET__" ), 16, f_woff[j] );
                if( iName.find( "__NOISELEVEL__" ) != string::npos )   iName.replace( iName.find( "__NOISELEVEL__" ), 14, f_noise[k] );
// hardwired file name (!)
