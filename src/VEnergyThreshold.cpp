...
{
    if( fDebug ) cout << "VEnergyThreshold::openEffectiveAreaFile " << iname << endl;
...
{
    if( fDebug ) cout << "VEnergyThreshold::calculateEnergyThreshold " << fTreeEth << endl;
    ...
    if( !fTreeEth )
    {
        if( !setUpThresholdTree() ) return false;
    }

    if( nentries < 0 ) nentries = fEffArea->fChain->GetEntries();
    if( nentries > fEffArea->fChain->GetEntries() ) nentries = fEffArea->fChain->GetEntries();
    for( int i = 0; i < nentries; i++ )
    {
        fEffArea->GetEntry( i );

        if( i % 10000 == 0 && i > 0 ) cout << "\t now at entry " << i << endl;
        ...
        if( hLin ) feth = getEnergyThreshold( hLin, true, bFit );
...
double VEnergyThreshold::getEnergy_maxSystematic( TObject *h, double iSys )
{
    ...
    if( strcmp( "TProfile", h->ClassName() ) == 0  )
    {
        ...
        {
            if( p->GetBinError( i ) < 0.001 ) continue;
        ...
        if( y1 < iSys )
        {
            ...
            if( i > 1 )
            {
                ...
                if( x1 - x2 != 0. )
                {
                    a = (y1-y2)/(x1-x2);
                    b = y1 - a*x1;
                    if( a != 0. ) return TMath::Power( 10., (iSys-b)/a );
                    else          return 0.;
                }
            }
            else
...
// weighed bin of three largest bins
    if( !bFit )
    {
        ...
        {
            if( bLogEnergyAxis ) feth += h->GetBinContent( tt ) * TMath::Power( 10., h->GetXaxis()->GetBinCenter( tt ) );
            else                 feth += h->GetBinContent( tt ) * h->GetXaxis()->GetBinCenter( tt );
            iTotWeight += h->GetBinContent( tt );
        }
        if( iTotWeight > 0. ) feth /= iTotWeight;
    }
// fit region around maximum by pol5
	...
	if( bLogEnergyAxis ) feth = TMath::Power( 10., i_f.GetMaximumX() );
	else                 feth = i_f.GetMaximumX();
    }
...
{
    if( fDebug ) cout << "VEnergyThreshold::setUpThresholdTree  " << fTreeEth << endl;
    if( fTreeEth ) return true;
...
// no file with energy thresholds given; return fixed value
    if( fEnergyThresholdFileName.size() <= 0 ) return fEnergyThresholdFixedValue;

// no run data given, return 0 energy threshold
    if( iRunData == 0 ) return 0.001;

// open file with energy thresholds
    if( !fEnergyThresholdFile && !openEnergyThresholdFile() ) return 0.001;
...
{
    if( fEnergyThresholdFileName.size() <= 0 ) return false;
...
// energy threshold vs azimuth angle
    ...
    for( unsigned int i = 0; i < iDraw.size(); i++ )
    {
        ...
        if( bPlot ) fTreeEth->Draw( iDraw[i].c_str(), iCut[i].c_str(), plot_option.c_str() );
        else        fTreeEth->Draw( iDraw[i].c_str(), iCut[i].c_str(), (plot_option + " same" ).c_str() );
...
// search for last plotted graph in TPad
        ...
        while( lnk )
        {
            ...
            if( strcmp( obj->ClassName(), "TGraph" ) == 0 ) g = (TGraph*)obj;
            lnk = lnk->Next();
        }
        if( g )
        {
