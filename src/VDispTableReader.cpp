...
{
   if( fHisto_binning.find( iVariableName ) == fHisto_binning.end() ) return false;	
   if( fHisto_min.find( iVariableName ) == fHisto_min.end() )         return false;	
   if( fHisto_max.find( iVariableName ) == fHisto_max.end() )         return false;	
...
// test variable name
    ...
    if( !iFound )
    {
       ...
       for( unsigned int i = 0; i < fHisto_ListOfVariables.size(); i++ ) cout << "\t" << fHisto_ListOfVariables[i] << endl;
       return false;
    }
...
// draw errors
            ...
            for( int i = 1; i <= h2D_DispTableError->GetNbinsX(); i++ )
            {
                ...
                {
                    if(  h2D_DispTableError->GetBinContent( i, j ) > 0. ) h2D_DispTableError->SetBinContent( i, j, h2D_DispTableError->GetBinError( i, j )/ h2D_DispTableError->GetBinContent( i, j ) );
                }
            }
	...
	else cout << "h2D_DispMissTable not found" << endl;
    }
    return true;
}
...
//////////////////////////////////////////////////////////////////////////////////////////////
    if( !iRead )
    {
        if( !isHistoBinningSet( "WidthOverLength" ) ) return false;
        if( !isHistoBinningSet( "ScaleLength" ) )     return false;
        if( !isHistoBinningSet( "ScaleWidth" ) )      return false;
        if( !isHistoBinningSet( "Size" ) )            return false;
...
// list of histograms
        ...
        if( hisList ) hisList->Add( h2D_DispTable );
        ...
        if( hisList ) hisList->Add( h2D_DispPhiTable );
        ...
        if( hisList ) hisList->Add( h2D_DispMissTable );
        ...
        if( hisList ) hisList->Add( h2D_DispTableN );
        ...
        if( hisList ) hisList->Add( h3D_DispTable );
        ...
        if( hisList ) hisList->Add( h3D_DispPhiTable );
        ...
        if( hisList ) hisList->Add( h3D_DispMissTable );
        ...
        if( hisList ) hisList->Add( h3D_DispTableN );
        ...
        if( hisList ) fData->Branch( hisList, 32000, 0 );
    }
//////////////////////////////////////////////////////////////////////////////////////////////
//
...
// fill data vectors
	    ...
	    fTreeEntry_Map.clear();
	       ...
	       {
	          if( TMath::Abs( f_ze[j] - ze ) < 1.e-2 ) i_found_ze = j;
               }
	       if( i_found_ze < 0 )
	       {
	          f_ze.push_back( ze );
		  i_found_ze = (int)f_ze.size()-1;
	       ...
	       {
	          if( TMath::Abs( f_az_min[j] - az_min ) < 1.e-2 ) i_found_az = j;
               }
	       if( i_found_az < 0 )
	       {
		  ...
		  i_found_az = (int)f_az_min.size()-1;
	       ...
	       {
	          if( TMath::Abs( f_woff[j] - woff ) < 1.e-2 ) i_found_woff = j;
               }
	       if( i_found_woff < 0 )
	       {
		  ...
		  i_found_woff = (int)f_woff.size()-1;
	       ...
	       {
	          if( TMath::Abs( f_noise[j] - pedvar ) < 5.e-1 ) i_found_noise = j;   // is 0.5 too large??
               }
	       if( i_found_noise < 0 )
	       {
...
{
    if( h2D_DispTable )  h2D_DispTable->Reset();
    if( h2D_DispPhiTable )  h2D_DispPhiTable->Reset();
    if( h2D_DispMissTable )  h2D_DispMissTable->Reset();
    if( h2D_DispTableN ) h2D_DispTableN->Reset();
}
...
{
    if( !iH2D || !iH2DN || !iH2DPhi || !iH2DMiss ) return false;
    if( !iH3D || !iH3DN || !iH3DPhi || !iH3DMiss ) return false;

    if( !h2D_DispTable )     return false;
    if( !h2D_DispPhiTable )  return false;
    if( !h2D_DispTableN )    return false;
    if( !h2D_DispMissTable ) return false;
    if( !h3D_DispTable )     return false;
    if( !h3D_DispPhiTable )  return false;
    if( !h3D_DispTableN )    return false;
    if( !h3D_DispMissTable ) return false;
...
{
    f_ze.clear();
    for( unsigned int i = 0; i < i_ze.size(); i++ ) f_ze.push_back( atof( i_ze[i].c_str() ) );
    f_az_min = i_az_min;
    f_az_max = i_az_max;
    f_woff.clear();
    for( unsigned int i = 0; i < i_woff.size(); i++ ) f_woff.push_back( atof( i_woff[i].c_str() ) );
    f_noise.clear();
    for( unsigned int i = 0; i < i_noise.size(); i++ ) f_noise.push_back( atof( i_noise[i].c_str() ) );
}
...
void VDispTableReader::print( bool bDetailed )
{
	...
	for( unsigned int i = 0; i < f_ze.size(); i++ ) cout << "ze vector " << f_ze[i] << endl;
	for( unsigned int i = 0; i < f_az_min.size(); i++ ) cout << "az vector " << f_az_min[i] << "\t" << f_az_max[i] << endl;
	for( unsigned int i = 0; i < f_woff.size(); i++ ) cout << "woff vector " << f_woff[i] << endl;
        for( unsigned int i = 0; i < f_noise.size(); i++ ) cout << "noise vector " << f_noise[i] << endl;
    }
}
...
// otherwise like [-45,45]
    ...
    {
        if( i == 0 && ( az > f_az_min[0] || az < f_az_max[0] ) ) return 0;
        else if( az > f_az_min[i] && az < f_az_max[i] ) return i;
    }
    return 0;
}
...
{
   if( fTreeEntry_Map.find( iID ) != fTreeEntry_Map.end() ) return fTreeEntry_Map[iID];
...
// get closest lower or upper value
    else
    {
       ...
       unsigned int i_ze_bin_low = 0;
       getIndexBoundary( &i_ze_bin_up, &i_ze_bin_low, f_ze, iZe );
       if( iZe_Inter == 1 )      i_ze_bin = i_ze_bin_low;
       else if( iZe_Inter == 2 ) i_ze_bin = i_ze_bin_up;
    }
