...
void VCameraRead::resetNeighbourLists( bool bMaxN )
{
	if( fDebug )
	{
		cout << "VCameraRead::resetNeighbourLists() " << fMaxNeighbour << endl;
	}
	vector< int > i_pix;
	vector< unsigned int > i_pixN;
	vector< vector< int > > i_N;
			...
			for( unsigned int k = 0; k < j; k++ )
			{
				...
				if( itemp < 2.1 * sqrt( 2. )*iTubeDistance_min )
				{
					if( getAnaPixel( i )[k] > 0 )
					{
						...
						if( fNeighbour[i][j].size() > fMaxNeighbour )
						{
							fMaxNeighbour = fNeighbour[i][j].size();
						}
					}
					if( getAnaPixel( i )[j] > 0 )
					{
						...
						if( fNeighbour[i][k].size() > fMaxNeighbour )
						{
							fMaxNeighbour = fNeighbour[i][j].size();
						}
					}
...
{
    VSQLTextFileReader a( string(iDBTextDirectory +"/"+run_number+".camerarotation") );
    if( !a.isGood() )
    {
        return false;
    }
