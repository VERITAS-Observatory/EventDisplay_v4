...
{
    if( iDataSet >= fData.size() ) return false;
...
// add a eventdisplay style root file
          if( gE0 && gE0->GetN() > 0 )
	  {
	     ...
	     {
	        gE0->GetPoint( i, x, y );
		if( emin_lin > 0. && log10( emin_lin ) > x ) continue;
		if( emax_lin > 0. && log10( emax_lin ) < x ) continue;
// accept only 4 sigma points (preliminary)
                if( gE0->GetErrorY( i ) > 0. && y/gE0->GetErrorY( i ) < 4. ) continue;
		...
		z++;
             ...
             if( bDebug ) fData.back()->gData->Print();
          }
// add a CTA production style root file
	  else if( AngRes )
	  {
	     ...
	     {
		if( emin_lin > 0. && log10( emin_lin ) > AngRes->GetXaxis()->GetBinCenter( i ) ) continue;
		if( emax_lin > 0. && log10( emax_lin ) < AngRes->GetXaxis()->GetBinCenter( i ) ) continue;
...
// assume that everything else is a text file
   else
   {
	  ...
	  {
	     g.GetPoint( i, x, y );
	     if( bEnergyAxis_linear_GeV ) x = log10( x ) - 3.;
	     if( bResolutionAxis_arcmin ) y /= 60.;
...
void VStereoReconstruction::plotLegend()
{
   ...
   TLegend *iLAng = 0;
   if( fData.size() < 3 )  iLAng = new TLegend( 0.5, 0.7, 0.94, 0.94 );
   else                    iLAng = new TLegend( 0.6, 0.6, 0.85, 0.85 );
...
{
    if( bClearExistingDataSet ) fData.clear();
...
// open text file
    ...
    while( getline( is, is_line ) )
    {
        if( is_line.size() > 0 )
        {
            ...
            is_stream >> temp;
// check set number
            if( (unsigned int)(atoi( temp.c_str() )) != iSet ) continue;
