...
// telescope number
            ...
            if( t_temp >= 0 && t_temp != (int)fTelID ) continue;
// high or low gain values
            is_stream >> iTemp;
            if( iTemp == "HIGHGAIN" && fLowGain ) continue;
            if( iTemp == "LOWGAIN" && !fLowGain ) continue;
...
// test pedestal range
    ...
    {
        if( fDebug ) cout << "testPedestals (1): " << ichannel << " " << fDEAD_ped_min << " " << iPeds << endl;
        return 1;
    }
    if( iPeds > fDEAD_ped_max )
    {
        if( fDebug ) cout << "testPedestals (1): " << ichannel << " " << fDEAD_ped_max << " " << iPeds << endl;
        return 1;
    }
...
{
    if( fIsMC || frunmode == 2 || frunmode == 5 ) return 0;

    if( iGain < fDEAD_gain_min )
    {
        if( fDebug ) cout << "testGains (5): " << ichannel << " " << fDEAD_gain_min << " " << iGain << endl;
        return 5;
    }
    if( iGain > fDEAD_gain_max )
    {
        if( fDebug ) cout << "testGains: " << ichannel << " " << fDEAD_gain_max << " " << iGain << endl;
        return 5;
    }
...
unsigned int VDeadChannelFinder::testGainVariations( unsigned int ichannel, double iGainVar )
{
    ...
    {
        if( fDebug ) cout << "testGainVariations (6): " << ichannel << " " << fDEAD_gainvar_min << " " << iGainVar << endl;
        return 6;
    }
...
unsigned int VDeadChannelFinder::testGainDev( unsigned int ichannel, double iGain, double iGainVar, bool iDefault )
{
    ...
    {
        if( fDebug ) cout << "testGainDev (7): " << ichannel << " " << iGain << " " << iGainVar << endl;
        return 7;
    }
...
unsigned int VDeadChannelFinder::testTimeOffsets( unsigned int ichannel, double iT )
{
    ...
    {
        if( fDebug ) cout << "testTimeOffsets (8): " << ichannel << " " << fDEAD_toffset_max << " " << iT << endl;
        return 8;
    }
