...
bool VAnalysisUtilities::openFile( string iname, int irun, bool iStereo, bool iPrint )
{
    ...
    if( iStereo )
    {
       char dname[200];
       bool b_cd = false;
       if( irun == 0 ) b_cd = fAnasumDataFile->cd( "total/stereo" );
       else if( irun < 0 )
       {
       ...
       if( t ) readTargetCoordinatesFromtRunSummary( t, irun );
    }
    else
    {
        char hdir[200];
	if( irun == 0 ) fAnasumDataFile->cd( "total" );
	else if( irun < 0 )
	{
	    sprintf( hdir, "total_%d", -1*irun );
	    if( !fAnasumDataFile->cd( hdir) ) return 0;
	}
	else
	{
    ...
    if( iPrint ) cout << "file open: " << iname << " (" << irun << ")" << endl;
...
bool VAnalysisUtilities::readTargetCoordinatesFromtRunSummary( TTree* t, int ion )
{
   ...
   if( t->GetBranch( "SkyMapCentreRAJ2000" ) ) t->SetBranchAddress( "SkyMapCentreRAJ2000", &fSkyMapCentreRAJ2000 );
   else fSkyMapCentreRAJ2000 = 0.;
   if( t->GetBranch( "SkyMapCentreDecJ2000" ) ) t->SetBranchAddress( "SkyMapCentreDecJ2000", &fSkyMapCentreDecJ2000 );
   else fSkyMapCentreDecJ2000 = -99.;
   if( t->GetBranch( "TargetShiftWest" ) ) t->SetBranchAddress( "TargetShiftWest", &fTargetShiftWest );
   else fTargetShiftWest = 0.;
   if( t->GetBranch( "TargetShiftNorth" ) ) t->SetBranchAddress( "TargetShiftNorth", &fTargetShiftNorth );
   else fTargetShiftNorth = 0.;
...
// order matters!                                                                                                                                                               
       ...
       {
	   if( TMath::Abs( fTargetRAJ2000 ) < 1.e-8  && TMath::Abs( fTargetDecJ2000 ) < 1.e-8 ) fSkyMapCentreDecJ2000 = fTargetDec;
	   else fSkyMapCentreDecJ2000 = fTargetDecJ2000;
       }
       if( TMath::Abs( fSkyMapCentreRAJ2000 ) < 1.e-8 )
       {
	   if( TMath::Abs( fTargetRAJ2000 ) < 1.e-8  && TMath::Abs( fTargetDecJ2000 ) < 1.e-8 ) fSkyMapCentreRAJ2000 = fTargetRA;
	   else fSkyMapCentreRAJ2000 = fTargetRAJ2000;
       }
       if( iRun == ion || ion <= 0 ) break;
   }
...
{
    if( fAnasumDataFile ) fAnasumDataFile->Close();
    else                  return false;
...
{
    if( !fAnasumDataFile ) return 0;

    char hname[200];
    if( iTot > 0 ) sprintf( hname, "total_%d/stereo", iTot );
    else if( iTot < 0 ) sprintf( hname, "total_%d/stereo", -1*iTot );
...
TGraph* VAnalysisUtilities::calcCumulativeSig(int iTot)
{
...
// check if this run is in runlist
            ...
            if( !bFoundRun ) continue;

            i_temp.runnumber = c->runOn;
            i_temp.MJD = c->MJDOn;
            if( i_temp.runnumber > 0 &&  i_temp.MJD > fRunList_MJD_max ) fRunList_MJD_max = i_temp.MJD;
            if( i_temp.runnumber > 0 &&  i_temp.MJD < fRunList_MJD_min ) fRunList_MJD_min = i_temp.MJD;
            i_temp.tOn = c->tOn;
...
TObject* VAnalysisUtilities::getHistogram( string hisname, int runnumber, string dirname )
{
    ...
    char dx[600];
    if( runnumber > 1 ) sprintf( dx, "run_%d/stereo/%s", runnumber, dirname.c_str() );
    else
    {
        if( runnumber == 0 )      sprintf( dx, "total/stereo/%s", dirname.c_str() );
        else if( runnumber == 1 ) sprintf( dx, "total_%d/stereo/%s", runnumber, dirname.c_str() );
        else                      sprintf( dx, "total_%d/stereo/%s", -1*runnumber, dirname.c_str() );
    }
