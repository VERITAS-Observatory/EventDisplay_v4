...
vector< TGraph* > VSensitivityCalculator::getCrabSpectrum( vector< double > i_fCrabFlux, bool bInt, string bUnit, bool bReset )
{
    if( fDebug )
    {
        cout << "VSensitivityCalculator::getCrabSpectrum " << i_fCrabFlux.size() << endl;
    }
        ...
        double iEnergyMin_TeV_lin, double iEnergyMax_TeV_lin )
{
    ...
    if( dE_Log10 > 0. )
    {
        cout << "calculating or reading differential sensitivity" << endl;
    }
    else
    {
        cout << "calculating or reading integral sensitivity" << endl;
    }
    cout << "==============================================" << endl;
    ...
    for( unsigned int i = 0; i < fPlottingCrabFlux_CU.size(); i++ )
    {
        if( i < i_fFunCrabFlux.size() && i_fFunCrabFlux[i] )
        {
            ...
            if( bUnit != "CU" )
            {
                ...
                {
                    sprintf( htitle, "%d Crab", ( int )( fPlottingCrabFlux_CU[i] ) );
                }
...
vector< VDifferentialFlux > VSensitivityCalculator::getDifferentialFluxVectorfromMC( double dE_Log10, double& iNorm )
{
    ...
    {
        if(!getMonteCarlo_EffectiveArea((*i_MCData_iterator ).second, dE_Log10 ) )
        {
            return a;
        }
    }
        ...
        double iE_low, double iE_up )
{
    ...
    if( iMCPara.hWeightedRate )
    {
        return getMonteCarloRateFromWeightedRateHistogram( iE_low, iE_up, iRateError, iMCPara.hWeightedRate );
    }
        ...
        double iE_low, double iE_up )
{
    ...
    if( iWeightedRate )
    {
        return getMonteCarloRateFromWeightedRateHistogram( iE_low, iE_up, iRateError, iWeightedRate );
    }
