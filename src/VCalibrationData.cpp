...
double VCalibrationData::getLowGainMultiplier_Sum( int iSumWindow, int jSumWindow )
{
	...
	if( fLowGainMultiplier_Sum.count( temp ) == 0 )
	{
		return 0;
	}
	else
	{
		return fLowGainMultiplier_Sum.at( temp );
	}
}
/*
...
double VCalibrationData::getLowGainSumCorrection( int iSumWindow, int jSumWindow, bool HiLo )
{
	...
	if( getLowGainMultiplier_Sum( iSumWindow, jSumWindow ) == 0 || getLowGainMultiplier_Trace() == 0 )
	{
		return 1.0;
	}
