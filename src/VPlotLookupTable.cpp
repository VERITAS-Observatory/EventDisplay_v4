...
void VPlotLookupTable::plot2DHistogram( TH2F *h, unsigned int iSetID, string ititle, int iCanvasX, double i_min, double i_max, bool iZLog )
{
   ...
   if( fLookupTableData[iSetID]->fLookupTable == "energyER" ) h->SetAxisRange( fLogEnergyAxis_min, fLogEnergyAxis_max, "X" );
   else                                                       h->SetAxisRange( fLogSizeAxis_min, fLogSizeAxis_max, "X" );
   h->SetAxisRange( fDistanceAxis_min, fDistanceAxis_max, "Y" );

   if( i_min > -998. ) h->SetMinimum( i_min );
   if( i_max > -998. ) h->SetMaximum( i_max );
...
TH2F* VPlotLookupTable::divide2DHistograms( TH2F *h1, TH2F *h2, char *hname )
{
    ...
    for( int i = 1; i <= hMM->GetNbinsX(); i++ )
    {
	  ...
	  {
	     hMM->SetBinContent( i, j, h1->GetBinContent( i, j ) / h2->GetBinContent( i, j ) );
	  }
	  else hMM->SetBinContent( i, j, 0. );
       }
   }
...
// median
   ...
   if( hMM_median ) hMM_median->SetZTitle( hname );
...
// mean
   ...
   if( hMM_mean ) hMM_mean->SetZTitle( hname );
...
{
// add a new data set
   if( !checkTableName( iTable ) ) return false;
...
bool VPlotLookupTable::checkTableName( string iTableName )
{
   ...
   {
      if( iTableName == fListOfTableNames[i] ) return true;
   }

   cout << "VPlotLookupTable::checkTableName unkown table name: " << iTableName << endl;
   cout << "\t allowed are: ";
   for( unsigned int i = 0; i < fListOfTableNames.size(); i++ ) cout << "\t" << fListOfTableNames[i];
   cout << endl;
   cout << endl;
   return false;
