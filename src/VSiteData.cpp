...
bool VSiteData::checkIntegrity()
{
    ...
    if( iS != fObservationTime_s.size() ) return false;
    if( iS != fCameraOffset_deg.size() ) return false;
    if( iS != fArray.size() ) return false;
    if( iS != fSiteFile_exists.size() ) return false;
    if( iS != fSiteFile_Emin.size() ) return false;
    if( iS != fSiteFile_Emax.size() ) return false;
    if( iS != fPlottingColor.size() ) return false;
    if( iS != fPlottingLineStyle.size() ) return false;
    if( iS != fPlottingFillStyle.size() ) return false;
    if( iS != fLegend.size() ) return false;
    if( iS != fGraphSensitivity.size() ) return false;
...
// ignore lines with '#' in the beginning
	 ...
	 {
	     if( iTemp == iListOfSites[i] ) bFound = true;
         }
	 if( !bFound ) iListOfSites.push_back( iTemp );
       }
   }
// no more sites
   if( iSiteCounter >= iListOfSites.size() ) return false;
...
// ignore lines with '#' in the beginning
	 ...
	 fSiteName = iTemp;
      ...
      if( !is_stream.eof() ) is_stream >> fSite_B_dB;
      if( !is_stream.eof() )
      {
         is_stream >> iTemp;
	 ...
	 fPlottingLineStyle.push_back( atoi( iTemp.c_str() ) );
      ...
      else fPlottingLineStyle.push_back( 1 );
      if( !is_stream.eof() )
      {
         is_stream >> iTemp;
	 fPlottingFillStyle.push_back( atoi( iTemp.c_str() ) );
      }
      else fPlottingFillStyle.push_back( 1 );
      if( !is_stream.eof() )
      {
...
// fill up remaining vectors
  ...
  if( fDebug ) cout << "VSiteData::addDataSet: integrity: " << checkIntegrity() << endl;
...
// add observing time to legend
  ...
  for( unsigned int i = 0; i < fObservationTime_s.size(); i++ )
  {
     ...
     {
	if( fObservationTime_s[i] < 1800. ) sprintf( hname, "%s, %s", fLegend[i].c_str(), fArray[i].c_str() );
	else                                sprintf( hname, "%s, %s", fLegend[i].c_str(), fArray[i].c_str() );
	fLegend[i] = hname;
     }
  }
...
// check that file exists
  for( unsigned int i = 0; i < fSiteFileName.size(); i++ )
  {
      ...
      if( iF.IsZombie() ) fSiteFile_exists[i] = false;
