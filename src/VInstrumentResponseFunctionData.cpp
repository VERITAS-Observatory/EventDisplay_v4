...
bool VInstrumentResponseFunctionData::initialize( string iName, string iType, unsigned int iNTel, double iMCMaxCoreRadius )
{
   ...
   if( fType_numeric < 0 ) return false;
   fType = iType;

   fName = iName;
   fNTel = iNTel;
   fMCMaxCoreRadius = iMCMaxCoreRadius;
   if( fMCMaxCoreRadius < 1.e-2 ) fMCMaxCoreRadius = 500.;
...
int VInstrumentResponseFunctionData::testResponseFunctionType( string iType )
{
    ...
    {
       if( iType == fListofResponseFunctionTypes[i] ) return (int)i;
    }
...
// define temporary histogram and fill with projection
      if( iHistoName.size() > 0 ) sprintf( iname, "%s_%d", iHistoName.c_str(), i );
      else 
      {
...
// get total number of events in histogram
      for( int j = 1; j <= iTemp->GetNbinsX(); j++ ) iTotSum += iTemp->GetBinContent( j );
      if( iTotSum  > 0. )
      {
	 double iTempSum = 0.;
...
// require at least 20 events to calculate a good RMS
	       if( iTemp->GetEntries() > 20. )
	       {
               ...
               else                           vResE.push_back( 0. );
	       break;
            }
          }
       }
       if( iHistoName.size() > 0 ) hList->Add( iTemp );
       else if( iTemp ) delete iTemp;
    }
