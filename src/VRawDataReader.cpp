...
{
    if( fDebug ) cout << "VRawDataReader::~VRawDataReader()" << endl;
/*   delete fFileRead;
   delete fEvent; */
}
...
{
    if( fDebug ) cout << "bool VRawDataReader::getNextEvent()" << endl;
    bool iStatus = true;
    for( unsigned int i = 0; i < fNTel; i++ )
    {
        try
        {
            ...
            if( fDebug ) cout << "bool VRawDataReader::getNextEvent() status: " << iStatus << endl;
        }
        catch(...)
        {
...
//	 fEventParser[i]->setEvent(fEvent[i]);
    ...
    if( fDebug ) cout << "bool VRawDataReader::getNextEvent() (end)" << endl;
