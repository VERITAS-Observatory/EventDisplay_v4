...
{
    if( getDebugFlag() ) cout << "VDST::VDST()" << endl;
...
void VDST::fill()
{
    if( getDebugFlag() ) cout << "VDST::fill()" << endl;
    if( fDST_tree == 0 ) return;
// init base analyzer
    if( !fDSTini )
    {
...
// pointing
	   ...
	   else if( i < getPointing().size() && getPointing()[i] ) fDSTpointElevation[i]   = getPointing()[i]->getTelElevation();
        }
    }
    fDSTLTrig = i_localTrigger.to_ulong();
// fill DST tree only for triggered events
    if( fMC && getRunParameter()->fIsMC == 2 && fDSTLTrig == 0 ) return;
...
// ntel is always to total number of telescopes in the DST file
   ...
   for( unsigned int i = 0; i < fDSTntel_data; i++ ) fDSTtel_data[i] = i;
...
// fill pulse timing level
	...
	{
	   for( unsigned int t = 0; t < fRunPar->fpulsetiminglevels.size(); t++ ) fDSTpulsetiminglevels[i][t] = fRunPar->fpulsetiminglevels[t];
        }
...
// fill pulse timing
                    if( fRunPar->fpulsetiminglevels.size() < getDSTpulsetiminglevelsN() )
		    {
		       ...
		       for( unsigned int t = 0; t < fRunPar->fpulsetiminglevels.size(); t++ ) fDSTpulsetiming[i][t][j] = t_PulseTimingTemp[t];
                    }
                    double i_max = 0.;
                    int maxpos = 0;
                    fTraceHandler->getTraceMax( corrfirst, corrfirst+getSumWindow(), i_max, maxpos );
		    ...
		    if( fRunPar->fpulsetiminglevels.size() < getDSTpulsetiminglevelsN() )
		    { 
		       for( unsigned int t = 0; t < fRunPar->fpulsetiminglevels.size(); t++ ) fDSTpulsetiming[i][t][j] = (float)getPulseTiming()[t][j];
                    }
                ...
                if( getTraceFit() > -1 )
                {
                    fDSTChi2[i][j] = (float)getTraceFitChi2()[j];
                    fDSTRT[i][j] = (float)getTraceFitRiseTime()[j];
                    fDSTFT[i][j] = (float)getTraceFitFallTime()[j];
                    fDSTRTpar[i][j] = (float)getTraceFitRiseTimeParameter()[j];
                    fDSTFTpar[i][j] = (float)getTraceFitFallTimeParameter()[j];
                    fDSTTraceNorm[i][j] = (float)getTraceFitNorm()[j];
                }
            }
            else
            {
		...
		if( fRunPar->fpulsetiminglevels.size() < getDSTpulsetiminglevelsN() )
		{
		   for( unsigned int t = 0; t < fRunPar->fpulsetiminglevels.size(); t++ ) fDSTpulsetiming[i][t][j] = 0.;
                }
                fDSTMax[i][j] = 0;
                fDSTRawMax[i][j] = 0;
                fDSTHiLo[i][j] = 0;
...
// only write events with more than NN ntubes to disk (for one telescope only)
    if( getNTel() == 1 && intubes < getRunParameter()->fdstminntubes ) return;
    fDST_tree->Fill();
}
...
void VDST::terminate()
{
    if( getDebugFlag() ) cout << "VDST::terminate()" << endl;
...
// write pulse shape histograms
            if( fDSTfile->mkdir( "meanPulses" )->cd() )
            {
                ...
                {
                    setTelID( i );
                    if( getMeanPulseHistograms() ) getMeanPulseHistograms()->Write();
                }
            }
// write pulse sum histograms
            if( fDSTfile->mkdir( "pulseSums" )->cd() )
            {
                ...
                {
                    setTelID( i );
                    if( getIntegratedChargeHistograms() ) getIntegratedChargeHistograms()->Write();
                }
            }
