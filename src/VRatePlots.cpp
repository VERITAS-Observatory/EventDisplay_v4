...
VRatePlots::VRatePlots( VAnaSumRunParameter* iPar, map< int, double > itime )
{
    ...
    vector< int > iruns;
    for( unsigned int i = 0; i < iPar->fRunList.size(); i++ )
    {
        iruns.push_back( iPar->fRunList[i].fRunOn );
    }
    fRuns = iruns;
...
{
    int i_point = 0;
    for( i_point = 0; i_point < ( int )fRuns.size(); i_point++ ) if( irun == fRuns[i_point] )
        {
            break;
        }
    ...
    if( fTime.find( irun ) != fTime.end() )
    {
        ...
        if( isig > 0 )
        {
            hRate_run->SetPointError( i_point, 0., irate / isig ) ;
        }
        else
        {
            hRate_run->SetPointError( i_point, 0., 0. );
        }
        hRate_time->SetPoint( i_point, fTime[irun], irate );
        if( isig > 0 )
        {
            hRate_time->SetPointError( i_point, 0., irate / isig );
        }
        else
        {
            hRate_time->SetPointError( i_point, 0., 0. );
        }
    }
...
void VRatePlots::fill( vector< double > itime, vector< double > ion, vector< double > ioff, vector< double > isig, vector< double > irate )
{
    ...
    for( unsigned int i = 0; i < itime.size(); i++ )
    {
        if( ion[i] > 0 && ioff[i] > 0 && isig[i] > 0 && irate[i] > 0 )
        {
            ...
            if( isig[i] > 0. )
            {
                hRate_time->SetPointError( z, 0., irate[i] / isig[i] );
            }
            else
            {
                hRate_time->SetPointError( z, 0., 0. );
            }
            z++;
        }
    }
