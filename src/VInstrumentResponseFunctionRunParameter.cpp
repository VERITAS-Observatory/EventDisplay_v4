...
// MONTECARLOENERGYRANGE <MC min energy [TeV]> <MC max energy [TeV]> <MC spectral index>
            else if( temp == "MONTECARLOENERGYRANGE" )
            {
                ...
                if( !is_stream.eof() ) is_stream >> fMCEnergy_min;
                if( !is_stream.eof() ) is_stream >> fMCEnergy_max;
                if( !is_stream.eof() ) is_stream >> fMCEnergy_index;
            }
// fill MC histograms
...
// energy reconstruction method 
            ...
            {
                if( !is_stream.eof() ) is_stream >> fEnergyReconstructionMethod;
            }
// number of bins on log10 energy axis 
            else if ( temp == "ENERGYAXISBINS" )
            {
                if( !is_stream.eof() ) is_stream >> fEnergyAxisBins_log10;
            }
// energy reconstruction quality
            else if( temp == "ENERGYRECONSTRUCTIONQUALITY" )
            {
                if( !is_stream.eof() ) is_stream >> fIgnoreEnergyReconstructionQuality;
            }
// number of azimuth bins
            else if( temp == "AZIMUTHBINS" )
            {
                if( !is_stream.eof() ) is_stream >> fAzimuthBins;
            }
...
// telescope type dependent cuts
            ...
            {
                if( !is_stream.eof() ) is_stream >> fCutFileName;
            }
...
// read scatter radius
                if( !is_stream.eof() ) is_stream >> fCoreScatterRadius;
                if( !is_stream.eof() ) is_stream >> fCoreScatterMode;
            }
...
// define  spectral index bins
    ...
    for( unsigned int i = 0; i < fNSpectralIndex; i++ ) fSpectralIndex.push_back( fSpectralIndexMin + (double)i * fSpectralIndexStep );
...
VMonteCarloRunHeader* VInstrumentResponseFunctionRunParameter::readMCRunHeader()
{
   TChain c( "data" );
   TFile *iF = 0;
   if( c.Add( fdatafile.c_str() ) ) iF = c.GetFile();
...
void VInstrumentResponseFunctionRunParameter::print()
{
    ...
    for( unsigned int i = 0; i < fAzMin.size(); i++ ) cout << " " << i << " [" << fAzMin[i] << "," << fAzMax[i] << "]";
    cout << endl;
