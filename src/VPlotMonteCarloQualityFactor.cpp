...
void VPlotMonteCarloQualityFactor::resetHistograms()
{
   ...
   for( iData = fData.begin(); iData != fData.end(); iData++ )
   {
      if( (*iData).second->hSignal ) (*iData).second->hSignal->Reset();
      if( (*iData).second->hBackground ) (*iData).second->hBackground->Reset();
      if( (*iData).second->hQFactors_UpperCut ) (*iData).second->hQFactors_UpperCut->Reset();
      if( (*iData).second->hQFactors_LowerCut ) (*iData).second->hQFactors_LowerCut->Reset();
  }
...
bool VPlotMonteCarloQualityFactor::setDataChain( string iChain, bool bSignal )
{
    ...
    if( nFil == 0 )
    {
       ...
       if( bSignal ) cout << "signal chain";
       else          cout << "background chain";
       cout << endl;
       return false;
    }
    if( bSignal ) fSignalChain     = new CData( iC );
    else          fBackgroundChain = new CData( iC );
...
void VPlotMonteCarloQualityFactor::calculateQfactors()
{
   ...
   for( iData = fData.begin(); iData != fData.end(); iData++ )
   {
      ...
      double iSTot = 0.;
      for( int i = 1; i <= hS->GetNbinsX(); i++ ) iSTot += hS->GetBinContent( i );
      double iBTot = 0.;
      for( int i = 1; i <= hB->GetNbinsX(); i++ ) iBTot += hB->GetBinContent( i );
...
// calculate quality factor (upper)
	 ...
	 if( iB > 0. && iBTot > 0. && iSTot > 0. ) hQU->SetBinContent( i, (iS/iSTot) / sqrt( iB / iBTot ) );
	 else                                      hQU->SetBinContent( i, 0. );
      }
// calculate quality factor (lower)
	 ...
	 if( iB > 0. && iBTot > 0. && iSTot > 0. ) hQL->SetBinContent( i, (iS/iSTot) / sqrt( iB / iBTot ) );
	 else                                      hQL->SetBinContent( i, 0. );
      }
// normalise distributions to number of entries
// normalise distributions to number of entries
      if( iSTot > 0. ) hS->Scale( 1./iSTot );
      if( iBTot > 0. ) hB->Scale( 1./iBTot );
...
void VPlotMonteCarloQualityFactor::fill( int iMaxNevents, CData *c, bool bSignal )
{
   ...
   vector< TH1D* > iHis;
   if( bSignal ) iHis.push_back( fData["MSCW"]->hSignal );
   else          iHis.push_back( fData["MSCW"]->hBackground );
   if( bSignal ) iHis.push_back( fData["MSCL"]->hSignal );
   else          iHis.push_back( fData["MSCL"]->hBackground );
   if( bSignal ) iHis.push_back( fData["dE"]->hSignal );
   else          iHis.push_back( fData["dE"]->hBackground );
   if( bSignal ) iHis.push_back( fData["EChi2"]->hSignal );
   else          iHis.push_back( fData["EChi2"]->hBackground );
   if( bSignal ) iHis.push_back( fData["EmissionHeight"]->hSignal );
   else          iHis.push_back( fData["EmissionHeight"]->hBackground );
   if( bSignal ) iHis.push_back( fData["EmissionHeightChi2"]->hSignal );
   else          iHis.push_back( fData["EmissionHeightChi2"]->hBackground );

   int iNevents = c->fChain->GetEntries();
   if( iMaxNevents > 0 && iMaxNevents < iNevents ) iNevents = iMaxNevents;
   cout << "Filling ";
   if( bSignal ) cout << " (signal): ";
   ...
   for( int i = 0; i < iNevents; i++ )
   {
      ...
      if( c->Erec <= 0. ) continue;
      if( log10( c->Erec ) < fEnergy_min ) continue;
      if( log10( c->Erec ) > fEnergy_max ) continue;

// quality cuts
      if( c->Chi2 < 0 ) continue;
      if( c->MSCW < -5. ) continue;
      if( c->MSCW > 20. ) continue;
      if( c->MSCL < -5. ) continue;
      if( c->MSCL > 20. ) continue;
...
// apply basic shape cut
      if( c->MSCW < fMSCW_min ) continue;
      if( c->MSCW > fMSCW_max ) continue;
      if( c->MSCL < fMSCL_min ) continue;
      if( c->MSCL > fMSCL_max ) continue;

      iHis[2]->Fill( c->dE );
      if( c->EChi2 > 0. ) iHis[3]->Fill( log10( c->EChi2 ) );
...
// upper q factor
      ...
      if( (*iData).second->hQFactors_UpperCut->GetMaximum() > (*iData).second->hQFactors_LowerCut->GetMaximum() ) (*iData).second->hQFactors_UpperCut->SetMaximum( (*iData).second->hQFactors_UpperCut->GetMaximum() * 1.5 );
      else  (*iData).second->hQFactors_UpperCut->SetMaximum( (*iData).second->hQFactors_LowerCut->GetMaximum() * 1.5 );
