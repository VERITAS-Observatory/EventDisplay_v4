...
// check if this telescope should be analyzed
        bool bAna = false;
        for( unsigned int t = 0; t < fTeltoAna.size(); t++ ) if( fTeltoAna[t] == i ) bAna = true;
...
//  eventdisplay should analyse a subset only)
            if( iTelGrisu.find( i ) != iTelGrisu.end() ) sprintf( hname, "%s.T%d", i_sourcefile.c_str(), iTelGrisu[i]+1 );
            else                                         sprintf( hname, "%s.T%d", i_sourcefile.c_str(), i+1 );
            cout << "VMultipleGrIsuReader::init sourcefile for telescope " << i+1 << ": " << hname << endl;
            fSourceFileName.push_back( hname );
        }
        else fSourceFileName.push_back( "" );
    }
...
// randomize all values
    if( fNoiseFileReader->getPeds().size() == g->getPeds().size() && g->getPedvars().size() == fNoiseFileReader->getPedvars().size() && g->getPedvarsAllSumWindows().size() == fNoiseFileReader->getPedvarsAllSumWindows().size() && g->getPedRMS().size() == fNoiseFileReader->getPedRMS().size() )
    {
        for( unsigned int i = 0; i < fNoiseFileReader->getPeds().size(); i++ )
        {
            ...
            for( unsigned int w = 0; w < g->getPedvarsAllSumWindows().size(); w++ ) g->getPedvarsAllSumWindows()[w][i] = fNoiseFileReader->getPedvarsAllSumWindows()[w][f];
...
{
    if( iT < fNFiles && iT < fReader.size()  ) return true;
...
{
    if( fReader.size() > 0 && fReader[fTelescopeID] ) return fReader[fTelescopeID];
...
{
    if( getReader() ) return getReader()->getDataFormat();
...
{
    if( fSourceFileName.size() > 0 ) return fSourceFileName[fTelescopeID];
...
{
    if( getReader() ) return getReader()->getChannelHitIndex( t );
...
{
    if( getReader() ) return getReader()->getEventNumber();
...
{
    if( getReader() ) return getReader()->getFullHitVec();
...
{
    if( getReader() ) return getReader()->getFullTrigVec();
...
{
    if( getReader() ) return getReader()->getNumberofFullTrigger();
...
{
    if( getReader() ) return getReader()->getFullAnaVec();
...
{
    if( getReader() ) return getReader()->getEventType();
...
{
    if( getReader() ) return getReader()->getATEventType();
...
{
    if( getReader() ) return getReader()->getHitID( t );
...
{
    if( getReader() ) return getReader()->getHiLo( i );
...
{
    if( getReader() ) return getReader()->getMaxChannels();
...
{
    if( checkTelescopeID( iTel ) && fReader[iTel] ) return fReader[iTel]->getNoiseVec( 0, iHitID );
...
{
    if( getReader() ) return getReader()->getNumChannelsHit();
...
{
    if( getReader() ) return getReader()->getNumSamples();
...
{
    if( checkTelescopeID( iTel ) && fReader[iTel] ) return fReader[iTel]->getPedHisto( 0, ichannel );
...
{
    if( getReader() ) return getReader()->getPeds();
...
{
    if( getReader() ) return getReader()->getPedvars();
...
{
    if( getReader() ) return getReader()->getPedvarsAllSumWindows();
...
{
    if( getReader() ) return getReader()->getPedRMS();
...
{
    if( getReader() ) return getReader()->getSamplesVec();
...
{
    if( getReader() ) getReader()->selectHitChan( t );
}
...
void VMultipleGrIsuReader::setPedestalEventMode( int i_Nped )
{
    ...
    {
        if( fReader[i] && fSelectedTelescope[i] ) fReader[i]->setPedestalEventMode( i_Nped );
    }
}
...
void VMultipleGrIsuReader::setRandomDead( int iNC, int iNB )
{
    for( unsigned int i = 0; i < fReader.size(); i++ )
    {
        if( fReader[i] && fSelectedTelescope[i] ) fReader[i]->setRandomDead( iNC, iNB );
    }
}
...
void VMultipleGrIsuReader::setSumWindow( vector< int > i_sum )
{
    for( unsigned int i = 0; i < fReader.size(); i++ )
    {
        if( fReader[i] && fSelectedTelescope[i] ) fReader[i]->setSumWindow( i_sum );
    }
}
...
void VMultipleGrIsuReader::setSumWindow( unsigned int iTelID, int isw )
{
    for( unsigned int i = 0; i < fReader.size(); i++ )
    {
        if( fReader[i] && fSelectedTelescope[i] ) fReader[i]->setSumWindow( iTelID, isw );
    }
}
...
void VMultipleGrIsuReader::setTelNumberOffset( int iOff )
{
    for( unsigned int i = 0; i < fReader.size(); i++ )
    {
        if( fReader[i] && fSelectedTelescope[i] ) fReader[i]->setTelNumberOffset( iOff );
    }
}
...
void VMultipleGrIsuReader::setTraceFile( string i_Trf )
{
    for( unsigned int i = 0; i < fReader.size(); i++ )
    {
        if( fReader[i] && fSelectedTelescope[i] ) fReader[i]->setTraceFile( i_Trf );
    }
}
...
void VMultipleGrIsuReader::setTrigger( vector<bool> iImage, vector<bool> iBorder )
{
    for( unsigned int i = 0; i < fReader.size(); i++ )
    {
        if( fReader[i] && fSelectedTelescope[i] ) fReader[i]->setTrigger( iImage, iBorder );
    }
}
...
{
    if( checkTelescopeID( iTel ) && fReader[iTel] ) return fReader[iTel]->getLocalTriggerTime( 0 );
...
{
    if( checkTelescopeID( iTel ) && fReader[iTel] ) return fReader[iTel]->getLocalDelayedTriggerTime( 0 );
...
{
    if( getReader() ) return getReader()->getNTelLocalTrigger();
...
// require at least two telescopes with local trigger
    ...
    for( unsigned int i = 0; i < fReader.size(); i++ )
    {
        if( fReader[i] && fReader[i]->hasLocalTrigger( 0 ) ) ntrig++;
        if( ntrig > 1 ) return true;
    }
...
{
    if( checkTelescopeID( iTel ) && fReader[iTel] ) return fReader[iTel]->hasLocalTrigger( 0 );
...
{
    if( getReader() ) return getReader()->getXimpactrot();
...
{
    if( getReader() ) return getReader()->getYimpactrot();
...
{
    if( getReader() ) return getReader()->setDefaultPed( iD );
}
...
// get next event from each file
        bool iC = false;
        if( fReader[i] && fSelectedTelescope[i] ) iC = fReader[i]->getNextEvent();
        iB = (iC && iB );
/////////////////////////////////////////////////////////////////
// set the local trigger for this telescope
        if( fReader[i] && fSelectedTelescope[i] ) fLocalTrigger[i] = fReader[i]->getLocalTrigger()[0];
        else                                      fLocalTrigger[i] = 0;
/////////////////////////////////////////////////////////////////
...
bool VMultipleGrIsuReader::getNextPedestalEvent()
{
    ...
    {
        bool iC = false;
        if( fReader[i] && fSelectedTelescope[i] ) iC = fReader[i]->getNextPedestalEvent();
        iB = (iC && iB );
    }
    return iB;
}
...
bool VMultipleGrIsuReader::getNextShowerEvent()
{
    ...
    {
        bool iC = false;
        if( fReader[i] && fSelectedTelescope[i] ) iC = fReader[i]->getNextShowerEvent();
        iB = (iC && iB );
    }
    return iB;
}
...
{
    if( getReader() ) return getReader()->getMC_primary();
...
{
    if( getReader() ) return getReader()->getMC_energy();
...
{
    if( getReader() ) return getReader()->getMC_X();
...
{
    if( getReader() ) return getReader()->getMC_Y();
...
{
    if( getReader() ) return getReader()->getMC_Xcos();
...
{
    if( getReader() ) return getReader()->getMC_Ycos();
...
{
    if( getReader() ) return getReader()->getMC_Ze();
...
{
    if( getReader() ) return getReader()->getMC_Az();
...
{
    if( getReader() ) return getReader()->getMC_Xoffset();
...
{
    if( getReader() ) return getReader()->getMC_Yoffset();
