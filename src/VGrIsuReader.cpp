...
// read some camera infos
    for( unsigned int i = 0; i < fNTel; i++ ) readCamera( i );
...
// open the source file
        ...
        if( is ) cout << "VGrIsuReader::VGrIsuReader() opened: " << fSourceFileName << endl;
    }
    else
    {
// open the pedestal file (shouldn't be zipped)
        if( fExternalPedFile.size() > 0 ) is_ped.open( fExternalPedFile.c_str() );
// or use file with simulations data (should be opened second -> unzipped)
        else                              is_ped.open( fSourceFileName.c_str() );
        ...
        if( is_ped ) cout << "reading pedestals from pedestal lines in file ";
        if( fExternalPedFile.size() > 0 ) cout << fExternalPedFile << endl;
        else                              cout << fSourceFileName << endl;
    }
}
...
// reset everything
        ...
        for( uint16_t s = 0; s < fNumSamples[i]+1; s++ ) vv_dtemp.push_back( v_dtemp );
...
//      numbering in fPeds is MC numbering
    ...
    {
        for( unsigned int i = 0; i < fNTel; i++ ) readPedsfromLibrary( i );
    }
// case B:
...
{
    if( fDebug ) cout << "VGrIsuReader::readPedsfromLibrary()" << endl;
...
// first set all pedestals to zero (tree contains only non-zero pedestals)
    for( unsigned int j = 0; j < fPeds[i].size(); j++ )     fPeds[i][j] = 0.;
    for( unsigned int j = 0; j < fPedvars[i].size(); j++ )  fPedvars[i][j] = 0.;
...
{
    if( fDebug ) cout << "VGrIsuReader::readPedsfromPlines" << endl;
    if( TMath::Abs( fFADCScale - 1. ) > 0.01 ) cout << "VGrIsuReader::readPedsfromPlines: scale fadc traces by " << fFADCScale << endl;

    if( is_ped.is_open() ) is_ped.close();
// open the pedestal file (shouldn't be zipped)
    if( fExternalPedFile.size() > 0 ) is_ped.open( fExternalPedFile.c_str() );
// or use file with simulations data (should be opened second -> unzipped)
    else                              is_ped.open( fSourceFileName.c_str() );
...
// calculate pedestal variations for different summation windows
                if( i_channel < fMaxChannels[i_telID] )
                {
                    ...
                    if( i_val_Size > 0 ) meanRMS2 /= (double)i_val_Size;
...
// calculate mean and mean2 (E[x**2])
                        ...
                        if( w != 0 && (int)w == fSumWindow[i_telID] ) fPeds[i_telID][i_channel] = mean / (double)w;
// "normal" rms
                        if( w != 0 ) meanRMS2 = meanRMS2 - mean*mean/w/w;
                        if( meanRMS2 >= 0. && (int)w == fSumWindow[i_telID] ) fPedRMS[i_telID][i_channel] = sqrt( meanRMS2 );
                        rms = mean2 - mean*mean;
                        if( rms > 0 ) rms = sqrt( rms );
                        fVPedvars[i_telID][w][i_channel] = rms;
                        if( (int)w == fSumWindow[i_telID] ) fPedvars[i_telID][i_channel] = rms;
                    }
                }
            }
        }
...
{
    if( fDebug ) cout << "VGrIsuReader::getNextEvent " << fPedestalMode << endl;

    bool iSuccess = false;
    if( fPedestalMode ) iSuccess = getNextPedestalEvent();
    else                iSuccess = getNextShowerEvent();

    if( !iSuccess ) setEventStatus( 999 );

    if( fDebug ) cout << "VGrIsuReader::getNextEvent, event status: " << getEventStatus() << endl;
...
{
    if( fDebug ) cout << "VGrIsuReader::getNextPedestalEvent()" << endl;
// increment event number
    fEventNumber++;
// is maximal number of pedestal events reached?
    if( fEventNumber > fNPedestalEvents ) return false;
...
{
    if( fDebug ) cout << "bool VGrIsuReader::getNextShowerEvent()" << endl;
...
// read triggered pixels from GrIsu file
    ...
    while( !is.eof() )
    {
        ...
        is_line = iSRecord;
// skip pedestal lines
        if( is_line == "P" ) continue;
//////////////////////////////////////////////////////
// MC DATA (SHOWER LINE)
//////////////////////////////////////////////////////
        else if( is_line == "S" )
        {
            if( fDebug ) cout << "bool VGrIsuReader::getNextShowerEvent(), S line" << endl;
            if( i_eventFound )
            {
                is.putback( iSRecord );
                break;
...
// direction of telescope plane, shower direction maybe offset
            ...
            is >> fMC_Ycos;
            is >> is_Temp;
            if( is_Temp == "nan" ) fMC_Xoffset = 0.;
            else                   fMC_Xoffset = atof( is_Temp.c_str() );
            is >> is_Temp;
            if( is_Temp == "nan" ) fMC_Yoffset = 0.;
            else                   fMC_Yoffset = atof( is_Temp.c_str() );;
...
//////////////////////////////////////////////////////
        ...
        {
            if( fDebug ) cout << "bool VGrIsuReader::getNextShowerEvent(), W line (" << fNTel << " telescopes)" << endl;
            float i_T = 0.;
            is >> is_Temp;
            is >> i_T;
...
// read the local triggers
                ...
                {
                    is >> i_T;
                    if( int(i_T+0.5) ) fLocalTrigger[i] = true;
                    else               fLocalTrigger[i] = false;
                }
...
// multiple raw data reader, one telescope only
            ...
            {
                is >> i_T;
                if( int(i_T+0.5) ) fLocalTrigger[0] = true;
                else               fLocalTrigger[0] = false;
...
// read the grisu cores
            ...
            {
                cout << "\t" << fArrayTrigger << "\t";
                for( unsigned int t = 0; t < fLocalTrigger.size(); t++ ) cout << fLocalTrigger[t] << "\t";
                for( unsigned int t = 0; t < fLocalTriggerTime.size(); t++ ) cout << fLocalTriggerTime[t] << "\t";
                for( unsigned int t = 0; t < fLocalDelayedTriggerTime.size(); t++ ) cout << fLocalDelayedTriggerTime[t] << "\t";
...
// fill sample vector
                if( ftempSampleFL[i] > fDetectorGeo->getLowGainThreshold()[i_telID]) fSamplesVec[i_telID][i_channel][i] = fDetectorGeo->getLowGainThreshold()[i_telID];
                else                   fSamplesVec[i_telID][i_channel][i] = (uint8_t)ftempSampleFL[i];
            }
        }
    }
...
//        - take background from trace library
        if( fTraceFileName.size() > 0 ) fillBackgroundfromTraceLibrary();
//        - take background generated by grisu
        else fillBackgroundfromPlines();
    }
...
{
    if( hit < fMaxChannels[fTelescopeID] ) return std::make_pair( true, hit );
...
{
    if( i < fMaxChannels[fTelescopeID] ) return i;
    return 0;
}
...
{
    if( hit < fMaxChannels[fTelescopeID] ) fSelectedHitChan[fTelescopeID] = hit;
    else fSelectedHitChan[fTelescopeID] = 0;
}


bool VGrIsuReader::setTelescopeID(unsigned int i_tel )
{
    if( i_tel < fNTel ) fTelescopeID = i_tel;
    else return false;
...
{
    if( fDebug ) cout << "bool VGrIsuReader::readCamera( int iTel ) " << iTel << endl;
...
{
    if( fDebug ) cout << "bool VGrIsuReader::readPixelMix( iTel ) " << iTel << endl;
...
{
    if( fDebug ) cout << "void VGrIsuReader::openTraceLibrary()" << endl;
    if( fTraceFileName.size() > 0 )
    {
...
{
    if( fDebug ) cout << "void VGrIsuReader::fillBackgroundfromTraceLibrary()" << endl;
    int i_traceValue = 0;
    double i_newPed = 0.;
    int i_newPedN = 0;
...
{
    bool bAdd = false;
    if( fExternalPedFile.size() > 0 || fMultiGrIsuReader ) bAdd = true;
...
// add background to existing trace
                else if( bAdd )
                {
                    ...
                    for( unsigned int j = 0; j < fNumSamples[h]; j++ )
                    {
                        ...
                        if( iTemp > fDetectorGeo->getLowGainThreshold()[h]) fSamplesVec[h][i][j] = fDetectorGeo->getLowGainThreshold()[h];
                        else                                                fSamplesVec[h][i][j] += (uint8_t)(fGrIsuPeds[h][i][nstart+j]-fdefaultPed);
                    }
                }
            }
        }
...
void VGrIsuReader::setTrigger( vector<bool> iImage, vector<bool> iBorder )
{
    ...
    for( unsigned int i = 0; i < fFullTrigVec[fTelescopeID].size(); i++ )
    {
        ...
        else fFullTrigVec[fTelescopeID][i] = false;
    }
}
...
{
    if( fDebug ) cout << "VGrIsuReader::hasLocalTrigger" << endl;
    if( iTel < getLocalTrigger().size() ) return getLocalTrigger()[iTel];
    return false;
}
...
{
    if( fDebug ) cout << "void VGrIsuReader::setTraceFile " << i_Trf << endl;
    fTraceFileName = i_Trf;
    for( unsigned int i = 0; i < fNTel; i++ ) readPixelMix( i );
    if( i_Trf.size() > 0 )
    {
...
TH1F* VGrIsuReader::getPedHisto( unsigned int itel, unsigned int ichannel )
{
    if( itel < fhPeds.size() )
    {
        if( ichannel < fhPeds[itel].size() ) return fhPeds[itel][ichannel];
    }
    return 0;
}
...
void VGrIsuReader::setRandomDead( int iNC, int iNB )
{
    ...
    if( fMCNdead )       cout << "VGrIsuReader: setting " << fMCNdead << " channels randomly dead" << endl;
    if( fMCNdeadboard )  cout << "VGrIsuReader: setting " << fMCNdeadboard << "boards randomly dead" << endl;
...
// don't care about if anything is already dead
                ...
                {
                    if( iPix + j < fFullAnaVec[iTel].size() ) fFullAnaVec[iTel][iPix+j] = -1;
                    fFullAnaVec[iTel][iPix] = -1;
                }
                iN++;
...
// set stream to beginning of file
    ...
    while( getline( is, is_line ) )
    {
        if( is_line.size() > 0 )
        {
            ...
            {
                if( is_line.find( "." ) < is_line.size() ) iV = atoi( is_line.substr( 1, is_line.find( "." ) ).c_str() ) * 100;
                if( is_line.rfind( "." ) < is_line.size() ) iV += atoi( is_line.substr( is_line.find( "." )+1, is_line.rfind( "." )-is_line.find( "." )-1 ).c_str() ) * 10;
                if( is_line.rfind( "." ) < is_line.size() ) iV += atoi( is_line.substr( is_line.rfind( "." )+1, is_line.size() ).c_str() );
                return iV;
            }
        }
    }
...
uint8_t VGrIsuReader::getNoiseSample( unsigned int iTel, uint32_t iHitID, unsigned int iSample, bool iNewTrace )
{
    ...
    {
        if( iNewTrace ) fNoiseTraceStart = fRandomGen->Integer( fGrIsuPedsN[iTel][iHitID] - fNumSamples[iTel] );
...
{
    if( iTel < fGrIsuPeds.size() ) return fGrIsuPeds[iTel];
...
{
    if( iTel < fGrIsuPeds.size() && iChannel < fGrIsuPeds[iTel].size() ) return fGrIsuPeds[iTel][iChannel];
