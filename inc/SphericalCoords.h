...
namespace SEphem
{
    ...
    class SphericalCoords
    {
        public:
            ...
            m_theta(theta), m_phi(phi) { rationalize(); }
...
// ----------------------------------------------------------------------
            ...
            double thetaRad() const { return m_theta.rad(); }
            double thetaDeg() const { return m_theta.deg(); }
            double thetaRot() const { return m_theta.rot(); }
            double phiRad() const { return m_phi.rad(); }
            double phiDeg() const { return m_phi.deg(); }
            double phiRot() const { return m_phi.rot(); }
            double phiHrs() const { return m_phi.hrs(); }

            Angle latitude() const { return m_theta.coAngle(); }
            Angle longitude() const { return phi(); }

            double latitudeRad() const { return m_theta.coAngleRadPM180(); }
            double latitudeDeg() const { return m_theta.coAngleDegPM180(); }
            double latitudeRot() const { return m_theta.coAngleRotPM180(); }
            double longitudeRad() const { return phiRad(); }
            double longitudeDeg() const { return phiDeg(); }
            double longitudeRot() const { return phiRot(); }

            bool isPole() const { return((m_theta==0)||(m_theta==Angle::sc_Pi)); }

            double z() const { return cos(m_theta); }
            double x() const { return sin(m_theta)*cos(m_phi); }
            double y() const { return sin(m_theta)*sin(m_phi); }
...
// ----------------------------------------------------------------------
            ...
            void setDeg(double theta, double phi)
                { m_theta=Angle::frDeg(theta); m_phi=Angle::frDeg(phi); rationalize(); }
            void setTheta(const Angle& theta) { set(theta,m_phi); }
            void setPhi(const Angle& phi) { set(m_theta,phi); }
...
// -------------------------------------------------------------------
        ...
        SphericalCoords::directionTo(const SphericalCoords& c) const
    {
        double sindphi;
        double cosdphi;
        if(isPole())sindphi=sin(c.m_phi),cosdphi=cos(c.m_phi);
        else sindphi=sin(c.m_phi-m_phi),cosdphi=cos(c.m_phi-m_phi);
        double sinth1=sin(c.m_theta);
        double costh1=cos(c.m_theta);
        double sinth2=sin(m_theta);
        double costh2=cos(m_theta);
