...
Int_t Ctelconfig::Cut(Long64_t entry)
{
...
double Ctelconfig::getArrayMaxSize()
{
   if( fChain )
   {
	 ...
	 if( TMath::Sqrt( (TelX-iXc)*(TelX-iXc) + (TelY-iYc)*(TelY-iYc) ) > iMax ) iMax = TMath::Sqrt( (TelX-iXc)*(TelX-iXc) + (TelY-iYc)*(TelY-iYc) );
      }
