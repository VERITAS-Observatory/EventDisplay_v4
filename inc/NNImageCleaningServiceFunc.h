...
TGraph* ReadIPRGraph( TString fname, TString graphname )
{
	...
	if( !fptr )
	{
		cout << "[ReadIPRGraph]:: Can't find file" << endl;
		if( getchar() == 'q' )
		{
			return NULL;
		}
...
float ReadConvFactorsHist( TString fname, TString hname, int bin )
{
	...
	if( !fptr )
	{
		cout << "[ReadConvFactorsHist]:: Can't find file" << endl;
		if( getchar() == 'q' )
		{
			return 0.;
		}
...
Double_t EquiRate( Double_t* x, Double_t* par ) // returns coinctime in ns !!!
{
	...
	Double_t valIPR = gIPR->Eval( xx, spl, "" );
	if( valIPR < 100 || xx >= ChargeMax )
	{
		valIPR = 100;    //Hz
	}
...
Double_t EquiRateMST( Double_t* x, Double_t* par ) // returns coinctime in ns !!!
{
	...
	Double_t valIPR = gIPR2->Eval( xx, spl, "" );
	if( valIPR < 100 || xx >= ChargeMax )
	{
		valIPR = 100;    //Hz
	}
...
Double_t EquiRateSCSST( Double_t* x, Double_t* par ) // returns coinctime in ns !!!
{
	...
	Double_t valIPR = gIPR3->Eval( xx, spl, "" );
	if( valIPR < 100 || xx >= ChargeMax )
	{
		valIPR = 100;    //Hz
	}
...
Double_t EquiRateSST( Double_t* x, Double_t* par ) // returns coinctime in ns !!!
{
	...
	Double_t valIPR = gIPR4->Eval( xx, spl, "" );
	if( valIPR < 100 || xx >= ChargeMax )
	{
		valIPR = 100;    //Hz
	}
...
Double_t EquiRateBound( Double_t* x, Double_t* par ) // returns coinctime in ns !!!
{
	...
	Double_t valIPRref = gIPR->Eval( par[1], spl, "" );
	if( valIPRref < 100 || par[1] >= ChargeMax )
	{
		valIPRref = 100;
	}
...
Double_t EquiRateBoundMST( Double_t* x, Double_t* par ) // returns coinctime in ns !!!
{
	...
	Double_t valIPRref = gIPR2->Eval( par[1], spl, "" );
	if( valIPRref < 100 || par[1] >= ChargeMax )
	{
		valIPRref = 100;
	}
...
Double_t EquiRateBoundSCSST( Double_t* x, Double_t* par ) // returns coinctime in ns !!!
{
	...
	Double_t valIPRref = gIPR3->Eval( par[1], spl, "" );
	if( valIPRref < 100 || par[1] >= ChargeMax )
	{
		valIPRref = 100;
	}
...
Double_t EquiRateBoundSST( Double_t* x, Double_t* par ) // returns coinctime in ns !!!
{
	...
	Double_t valIPRref = gIPR4->Eval( par[1], spl, "" );
	if( valIPRref < 100 || par[1] >= ChargeMax )
	{
		valIPRref = 100;
	}
...
{
	TF1* f1 = NULL;
	if( teltype < 1 || teltype > 4 )
	{
		f1 = new TF1( funcname, EquiRate, xlow, xup, 3 );   //LST setting for all types
	}
	if( teltype == 1 )
	{
		f1 = new TF1( funcname, EquiRate, xlow, xup, 3 );
	}
	if( teltype == 2 )
	{
		f1 = new TF1( funcname, EquiRateMST, xlow, xup, 3 );
	}
	if( teltype == 3 )
	{
		f1 = new TF1( funcname, EquiRateSCSST, xlow, xup, 3 );
	}
	if( teltype == 4 )
	{
		f1 = new TF1( funcname, EquiRateSST, xlow, xup, 3 );
	}
...
TF1* RateContourBound( int teltype, TString funcname, float fRate, float refThresh, float fCombFactor, float xlow, float xup )
{
	...
	if( teltype < 1 || teltype > 4 )
	{
		f1 = new TF1( funcname, EquiRateBound, xlow, xup, 3 );   //LST setting for all types
	}
	if( teltype == 1 )
	{
		f1 = new TF1( funcname, EquiRateBound, xlow, xup, 3 );
	}
	if( teltype == 2 )
	{
		f1 = new TF1( funcname, EquiRateBoundMST, xlow, xup, 3 );
	}
	if( teltype == 3 )
	{
		f1 = new TF1( funcname, EquiRateBoundSCSST, xlow, xup, 3 );
	}
	if( teltype == 4 )
	{
		f1 = new TF1( funcname, EquiRateBoundSST, xlow, xup, 3 );
	}
