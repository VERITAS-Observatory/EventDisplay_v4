...
class VEvndispData
{
	...
	public:
		...
		double              getBorderThresh()
		{
			if( getImageCleaningParameter() )
			{
				return getImageCleaningParameter()->fborderthresh;
			}
			else
			{
				return 0.;
		...
		double              getBrightNonImageThresh()
		{
			if( getImageCleaningParameter() )
			{
				return getImageCleaningParameter()->fbrightnonimagetresh;
			}
			else
			{
				return 0.;
		...
		VImageCleaningRunParameter* getImageCleaningParameter()
		{
			if( fTelID < getRunParameter()->fImageCleaningParameters.size() )
			{
				return getRunParameter()->fImageCleaningParameters[fTelID];
			}
			else
			{
				return 0;
		...
		double              getImageThresh()
		{
			if( getImageCleaningParameter() )
			{
				return getImageCleaningParameter()->fimagethresh;
			}
			else
			{
				return 0.;
		...
		vector<int>&        getLowGainDefaultSumWindows()
		{
		...
		double              getLowGainMultiplier_Sum( int iWindow, int jWindow )
		{
		...
		double              getTraceFit()
		{
			return fRunPar->ftracefit;
		}
		valarray<double>&   getTraceFitChi2()
		{
			return fAnaData[fTelID]->fChi2;
		}
		valarray<double>&   getTraceFitFallTime()
		{
			return fAnaData[fTelID]->fFallTime;
		}
		valarray<double>&   getTraceFitFallTimeParameter()
		{
			return fAnaData[fTelID]->fFallTimePar;
		}
		valarray<double>&   getTraceFitNorm()
		{
			return fAnaData[fTelID]->fTraceNorm;
		}
		valarray<double>&   getTraceFitRiseTime()
		{
			return fAnaData[fTelID]->fRiseTime;
		}
		valarray<double>&   getTraceFitRiseTimeParameter()
		{
			return fAnaData[fTelID]->fRiseTimePar;
		}
		...
		VFitTraceHandler*   getFitTraceHandler()
		{
		...
		{
			return fYGraph[fTelID];
		}
		TGraphErrors*       getRGraph()
		{
			return fRGraph[fTelID];
		}
		...
		void                setAnaData()
		{
			fAnaData[fTelID]->initialize( fDetectorGeo->getNChannels( fTelID ), getReader()->getMaxChannels(), ( getTraceFit() > -1. ), getDebugFlag(), getRunParameter()->fMCNdeadSeed, getNSamples(), getRunParameter()->fpulsetiminglevels.size(), getRunParameter()->fpulsetiming_tzero_index, getRunParameter()->fpulsetiming_width_index );
		}
		...
		void                setZeroSuppressed( bool iZ )
		{
			fAnaData[fTelID]->fZeroSuppressed.assign( fDetectorGeo->getNChannels( fTelID ), iZ );
		}
		void                setZeroSuppressed( unsigned int iChannel, bool iZ )
		{
		...
		double           getTimeCutPixel()
		{
			if( getImageCleaningParameter() )
			{
				return getImageCleaningParameter()->ftimecutpixel;    //HP
			}
			else
			{
				return 0.;
		...
		double           getTimeCutCluster()
		{
			if( getImageCleaningParameter() )
			{
				return getImageCleaningParameter()->ftimecutcluster;    //HP
			}
			else
			{
				return 0.;
		...
		int              getMinNumPixelsInCluster()
		{
			if( getImageCleaningParameter() )
			{
				return getImageCleaningParameter()->fminpixelcluster;    //HP
			}
			else
			{
				return 0;
		...
		int              getNumLoops()
		{
			if( getImageCleaningParameter() )
			{
				return getImageCleaningParameter()->floops;    //HP
			}
			else
			{
				return 0;
		...
		void                setTraceChi2( unsigned int iChannel, double iS )
		{
			fAnaData[fTelID]->fChi2[iChannel] = iS;
		}
		void                setTraceChi2( double iV )
		{
			fAnaData[fTelID]->fChi2 = iV;
		}
		void                setTraceFallTime( unsigned int iChannel, double iS )
		{
			fAnaData[fTelID]->fFallTime[iChannel] = iS;
		}
		void                setTraceFallTime( double iV )
		{
			fAnaData[fTelID]->fFallTime = iV;
		}
		void                setTraceFallTimeParameter( unsigned int iChannel, double iS )
		{
			fAnaData[fTelID]->fFallTimePar[iChannel] = iS;
		}
		void                setTraceFallTimeParameter( double iV )
		{
			fAnaData[fTelID]->fFallTimePar = iV;
		}
		...
		void                setTraceNorm( unsigned int iChannel, double iS )
		{
			fAnaData[fTelID]->fTraceNorm[iChannel] = iS;
		}
		void                setTraceNorm( double iV )
		{
			fAnaData[fTelID]->fTraceNorm = iV;
		}
		void                setTraceRiseTime( unsigned int iChannel, double iS )
		{
			fAnaData[fTelID]->fRiseTime[iChannel] = iS;
		}
		void                setTraceRiseTime( double iV )
		{
			fAnaData[fTelID]->fRiseTime = iV;
		}
		void                setTraceRiseTimeParameter( unsigned int iChannel, double iS )
		{
			fAnaData[fTelID]->fRiseTimePar[iChannel] = iS;
		}
		void                setTraceRiseTimeParameter( double iV )
		{
			fAnaData[fTelID]->fRiseTimePar = iV;
		}
		...
		void                setXGraph( TGraphErrors* igraph )
		{
			fXGraph[fTelID] = igraph;
		}
		void                setYGraph( TGraphErrors* igraph )
		{
			fYGraph[fTelID] = igraph;
		}
		void                setRGraph( TGraphErrors* igraph )
		{
			fRGraph[fTelID] = igraph;
		}
