...
class VMinimizerFn
{
  ...
  public:
    ...
    unsigned int getNParam() const { return fNParam; }
...
class VMinimizer
{
  ...
  public:
    ...
    virtual ~VMinimizer() { }
    //// Get Methods
    virtual unsigned getNParam() const { return fParam.size(); }          //! number of parameters.  
    virtual unsigned getNFit() const { return fNFit; }                    //! number of fitted parameters.  
    virtual const vector<double>& getParam() const { return fParam; }     //! vector of parameters.  
    virtual double getParam(unsigned ip) const { return fParam[ip]; }     //! parameter value.  
    virtual const vector<double>& getError() const { return fError; }     //! vector of parameter errors.  
    virtual double getError(unsigned ip) const { return fError[ip]; }     //! parameter error.  
    bool getIsFree(unsigned ip ) const { return fFit[ip]; }               //! free parameters.  
    virtual const vector< vector< double > >& getCov() const { return fCov; }           //! vectors of parameter covariance
    virtual double getCov(unsigned ip1, unsigned ip2) const { return fCov[ip1][ip2]; }  //! covariance of two parameters.  
    virtual double getRho(unsigned ip1, unsigned ip2) const { return getCov(ip1,ip2)/(getError(ip1)*getError(ip2)); }  //! correlation coefficient of two parameters.  
    virtual double getFnVal() const { return fFnVal; }      //! value of object function.      
    const VMinimizerFn& getFn() const { return fFn; }       //! object of function.      
    bool getConverged() const { return fConverged; }        //! fit converged or not
    double getHiBound(unsigned ip) const { return fHiBound[ip]; }      //! high bound
    bool getHasHiBound(unsigned ip) const { return fHasHiBound[ip]; }  //! has high bound
    double getLoBound(unsigned ip) const { return fLoBound[ip]; }      //! low bound
    bool getHasLoBound(unsigned ip) const { return fHasLoBound[ip]; }  //! has low bound
    //// Set Methods
    void setParam(const vector<double>& param) { fParam = param; }
    void setParam(unsigned ip, double val) { fParam[ip] = val; }
    void setError(unsigned ip, double err) { fError[ip] = err; }
    void setCovariance(const vector< vector< double > >& cov) { fCov = cov; for(unsigned int iparm=0; iparm<getNParam(); iparm++) setError(iparm,sqrt(fCov[iparm][iparm])); }
    virtual void setLoBound(unsigned ip, double val) { fHasLoBound[ip] = true; fLoBound[ip] = val; }  //! Set a lower bound on a parameter.
    virtual void setHiBound(unsigned ip, double val) { fHasHiBound[ip] = true; fHiBound[ip] = val; }  //! Set an upper bound on a parameter.
    void setFnVal(double val) { fFnVal = val; }
    void setConverged(bool converged) { fConverged = converged; }
    //// other functions
    void freeze(unsigned ip) { if( ip < fFit.size() ) fFit[ip] = false; fNFit = 0; for(unsigned ifit = 0; ifit < fFit.size(); ifit++) if(fFit[ifit]) fNFit++; }
...
class VSVD
{
 public:
  ...
  private:
    ...
    double SIGN(const double a, const double b) { return (b>=0.0)?fabs(a):-fabs(a); }
    int                        fNRow;
    int                        fNCol;
