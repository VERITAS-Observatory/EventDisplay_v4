...
class VFluxCalculation : public TObject
{
	...
	public:
		...
		vector< double > getFluxError()
		{
			return fRunFluxE;
		}
		vector< double > getFluxConstant()
		{
			return fRunFluxConstant;
		}
		vector< double > getFluxConstantError()
		{
			return fRunFluxConstantE;
		}
		vector< double > getMJD()
		{
			return fRunMJD;
		}
		double           getMJD( int iRun );
		vector< double > getRunList()
		{
			return fRunList;
		}
		vector< double > getTOn()
		{
			return fRunTOn;
		}
		...
		TGraphAsymmErrors* getRXTEvsMJD()
		{
			if( fRXTE )
			{
				return fRXTE->gFluxMJD;
			}
			else
			{
				return 0;
		...
		TGraphAsymmErrors* getRXTEvsPhase()
		{
			if( fRXTE )
			{
				return fRXTE->gFluxPhase;
			}
			else
			{
				return 0;
		...
		unsigned int  loadRunList( int iRunMin = -1, int iRunMax = -1, unsigned int iTot = 1, double iMJDMin = -99., double iMJDMax = -99. );
		void          printResults();
