...
// raweventparser
        virtual bool                hasAT() { return false; }
        std::pair< bool, uint32_t > getChannelHitIndex (uint32_t i ) { if( fEvent[fTelID] ) return fEvent[fTelID]->getChannelHitIndex( i ); else return fDummyPair; }
        uint32_t                    getRunNumber()                  { return 0; }
        uint32_t                    getEventNumber()                { if( fEvent[fTelID] ) return fEvent[fTelID]->getEventNumber(); else return fEventNumber; }
        uint8_t                     getNewEventType();
        uint8_t                     getNewEventType( unsigned int itel );
        uint8_t                     getEventType();
        uint8_t                     getATEventType() { return getEventType(); }
        bool                        getHitBit( uint32_t i ) { if( fEvent[fTelID] ) return fEvent[fTelID]->getHitBit( i ); else return true; }
        std::vector< bool >         getFullHitVec()                 { if( fEvent[fTelID] ) return fEvent[fTelID]->getFullHitVec(); else return fDummyBoolV;  }
        std::vector< bool >         getFullTrigVec()                { if( fEvent[fTelID] ) return fEvent[fTelID]->getFullTrigVec(); else return fDummyBoolV; }
        int                         getNumberofFullTrigger();
        uint32_t                    getGPS0()                       { if( fEvent[fTelID] ) return fEvent[fTelID]->getGPSTime()[0]; else return 0; }
        uint32_t                    getGPS1()                       { if( fEvent[fTelID] ) return fEvent[fTelID]->getGPSTime()[1]; else return 0; }
        uint32_t                    getGPS2()                       { if( fEvent[fTelID] ) return fEvent[fTelID]->getGPSTime()[2]; else return 0; }
        uint32_t                    getGPS3()                       { if( fEvent[fTelID] ) return fEvent[fTelID]->getGPSTime()[3]; else return 0; }
        uint32_t                    getGPS4()                       { if( fEvent[fTelID] ) return fEvent[fTelID]->getGPSTime()[4]; else return 0; }
        uint16_t                    getGPSYear();
        virtual uint16_t            getATGPSYear() { return getGPSYear(); }
        uint16_t                    getMaxChannels();
        vector< int >               getNIncompleteEvents() { return fNIncompleteEvent; }
        uint16_t                    getNumChannelsHit()             { if( fEvent[fTelID] ) return fEvent[fTelID]->getNumChannelsHit(); else return 0; }
        uint16_t                    getNumSamples();
        unsigned int                getNTel() { return 0; }
...
// rawfile
        ...
        bool                       isMC();
        int                        getMC_primary() { return getSMC_primary(); }
        float                      getMC_energy() { return getSMC_energy(); }
        float                      getMC_X() { return getSMC_X(); }
        float                      getMC_Y() { return getSMC_Y(); }
        float                      getMC_Xcos() { return getSMC_Xcos(); }
        float                      getMC_Ycos() { return getSMC_Ycos(); }
        float                      getMC_Ze() { return getSMC_Ze(); }
        float                      getMC_Az() { return getSMC_Az(); }
...
//      bool                       hasLocalTrigger( unsigned int iTel );
	...
	VMonteCarloRunHeader* getMonteCarloHeader() { return fMonteCarloHeader; }
};
#endif
