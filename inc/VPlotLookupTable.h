...
class VPlotLookupTable : public VPlotUtilities, public VHistogramUtilities
{
   ...
   void    plot2DHistogram( TH2F *h, unsigned int iSetID, char* htitle, int iCanvasX, double i_min = -999., double i_max = -999. );
  ...
  ~VPlotLookupTable() {}
   ...
   void  setPlottingDistanceAxis( double imin = 0., double imax = 600. ) { fDistanceAxis_min = imin; fDistanceAxis_max = imax; }
   void  setPlottingLogEnergyAxis( double imin = -2., double imax = 2.5 ) { fLogEnergyAxis_min = imin; fLogEnergyAxis_max = imax; }
   void  setPlottingLogSizeAxis( double imin = 1.0, double imax = 7. ) { fLogSizeAxis_min = imin; fLogSizeAxis_max = imax; }
