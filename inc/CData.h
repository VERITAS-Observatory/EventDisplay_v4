...
// data is in a chain -> get first tree and version number from it
    else if( tree->IsA() == TChain::Class() )
    {
        ...
        {
            string itemp = "";
            if( tree->GetTree() ) itemp = tree->GetTree()->GetTitle();
            if( itemp.find( "VERSION" ) < itemp.size() )
            {
                fVersion = atoi( itemp.substr( itemp.find( "VERSION" )+7, itemp.size() ).c_str() );
            }
...
// test if this is a MC file
    if( tree->GetBranchStatus( "MCe0" ) ) fMC = true;
// test if second max branch exists
    if( tree->GetBranchStatus( "SizeSecondMax" ) ) fBOOLSecondMax = true;
    if( tree->GetBranchStatus( "theta2_All" ) ) fBOOLtheta2_All = true;
...
// MC tree
    if( fMC )
    {
	if( fVersion >7 ) fChain->SetBranchAddress("MCprimary", &MCprimary );
	else              MCprimary = -99;
...
//AMC 09102009
    if( fBOOLSecondMax ) fChain->SetBranchAddress("SizeSecondMax",&SizeSecondMax);
    else                 SizeSecondMax = 0.;
//AMC 09102009
    ...
    {
        for(unsigned int dex = 0; dex < 25; dex++) theta2_All[dex] = 99.0;
    }
	...
	{
	    for( int i = 0; i < VDST_MAXTELESCOPES; i++ ) size2[i] = 0.;
        }
    ...
    if( !fShort ) fChain->SetBranchAddress("NMSCW",&NMSCW);
    else          NMSCW = 0;
...
// Get branch pointers
    ...
    if( fBOOLSecondMax )  b_SizeSecondMax = fChain->GetBranch("SizeSecondMax");
    else                  b_SizeSecondMax = 0;

    if( fBOOLtheta2_All )  b_theta2_All = fChain->GetBranch("theta2_All");
    else                  b_theta2_All = 0;
