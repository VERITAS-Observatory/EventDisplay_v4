...
//        VVirtualDataReader() throw (VFileException);
        ...
        virtual vector< int >               getNIncompleteEvents() { return b; }
        virtual unsigned int                getNumTelescopes() { return 0; }
        virtual string                      getSourceFileName() { return fSourceFileName; }
        virtual unsigned int                getTelescopeID() { return 0; }
        ...
        virtual bool                        setTelescopeID(unsigned int a) { return true; }
// raweventparser
        virtual std::pair< bool, uint32_t > getChannelHitIndex (uint32_t) = 0;
        virtual uint32_t                    getEventNumber() = 0;
        virtual uint8_t                     getNewEventType() { return 0; }
	...
	virtual VMonteCarloRunHeader*       getMonteCarloHeader() = 0;
        ...
        virtual TH1F*                       getPedHisto( unsigned int a, unsigned int b ) { return 0; }
        virtual valarray<double>&           getPeds() { return v; }
        virtual valarray<double>&           getPedvars() { return v; }
        virtual vector< valarray<double> >& getPedvarsAllSumWindows() { return vv; }
        virtual valarray<double>&           getPedRMS() { return v; }
...
// rawfile
        ...
        virtual bool                        getPrevEvent() { return true; }
#ifndef NOVBF
        virtual VArrayTrigger*              getArrayTrigger() { return 0; }
#else
        virtual VDummyArrayTrigger*         getArrayTrigger() { return 0; }
#endif

// telescopes pointing
        virtual std::vector< double >       getTelElevation() { return d; }
        virtual std::vector< double >       getTelAzimuth() { return d; }
...
// MC
        ...
        bool                       isDST() { if( getDataFormatNum() == 4 ) return true; else return false; }
        bool                       isPEFormat() { if( getDataFormatNum() == 6  ) return true; else return false; }
        bool                       isVBFMC() { if( getDataFormatNum() == 2 ) return true; else return false; }
        virtual TTree*                     getMCTree() { return 0; }
                                                  //!< MC primary type
