...
class VModelLnL : public VMinimizerFn
{
	...
	public:
		...
		void createLnLTable( double pixel_var, double pixel_nsb ); //TEST
		
		void val( const vector<double>& a, double& lnl, vector<double>& beta, vector< vector< double > >& alpha ) const; // CHECK: does this need to be virtual ?? (total of telescope log-likelihoods providing input to the minimizer)
