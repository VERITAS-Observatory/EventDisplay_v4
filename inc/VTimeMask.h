...
// Create internal time format: double VAnaUtils::getUTC(int i_mjd, double i_seconds) returns <MJD>.<fraction of day>
        Double_t        exactMJD_secs( Int_t MJD, Double_t secsMid ) const { return secs_day*VSkyCoordinatesUtilities::getUTC( MJD, secsMid ); }
// Return integer MJD portion of internal time
        Int_t           timeMJD( Double_t internal_time ) const { return Int_t( floor( internal_time/secs_day ) ); }
// Return seconds since UTC midnight
        Double_t        time_secsMid( Double_t internal_time ) const { return internal_time - Double_t( secs_day )*timeMJD( internal_time ); }
...
// Is the event allowed to pass the mask?
        ...
        Bool_t      checkAgainstMask( Double_t eventUTC ) { return checkMaskNow( secs_day*eventUTC ); }

// Count events passing gamma and direction cuts
        void        countOn( Double_t eventUTC ) { counted.at( Int_t( floor( secs_day*eventUTC - start_time ) ) )++; }

        Bool_t      getMaskStatus() const         // Has the mask encountered any errors; is it still active?
        {
            return !override;
        }
        Int_t       getRunID() const { return run_id; }
        Int_t       getMaskStartMJD() const { return timeMJD( start_time ); }
        Double_t    getMaskStartTime() const { return time_secsMid( start_time ); }
        Double_t    getMaskStartUTC() const { return start_time / secs_day; }
        ...
        void        resetEventTotals() { checked.assign( mask.size(), 0); accepted.assign( mask.size(), 0); }
...
// Various takes on the mean time of the run; all in <MJD>.<fraction of day>
        Double_t    getMeanUTC_Run() const {return ( start_time + getMeanTime_Run() ) / secs_day;}
        Double_t    getMeanUTC_Mask() const {return ( start_time + getMeanTime_Mask() ) / secs_day;}
        Double_t    getMeanUTC_Events() const {return ( start_time + getMeanTime_Events() ) / secs_day;}
        Double_t    getMeanUTC_Accepted() const {return ( start_time + getMeanTime_Accepted() ) / secs_day;}
