...
class VDispTableAnalyzer
{
	...
	float getDispPhi() { return f_disp_Phi; }
	float getDispPhiE() { return f_disp_PhiE; }
	float getDispMiss() { return f_disp_Miss; }
	float getXcoordinate_disp( unsigned int i ) { if( i < x_disp.size() ) return x_disp[i]; else return -9999.; }
	float getYcoordinate_disp( unsigned int i ) { if( i < y_disp.size() ) return y_disp[i]; else return -9999.; }
