...
// coordinate transformer
	...
	unsigned int getCameraCentreTubeIndex() { if( fTelID < fCameraCentreTubeIndex.size() ) return fCameraCentreTubeIndex[fTelID]; 
        ...
        int                  getFADCRange()  { return fFADCRange; }
        vector<float>&       getFocalLength()     //! get focal length [m]
        {
            return fMirFocalLength;
        }
	vector< unsigned int>& getNMirrors() { return fNMirrors; } 
	vector< float >&     getMirrorArea() { return fMirrorArea; }
        vector<float>&       getFieldofView()  { return fCameraFieldofView; }
        unsigned int         getGrIsuVersion()  { return fGrIsuVersion; }
        bool                 isLowGainSet() { return fLowGainIsSet; }
        vector<float>&       getCameraScaleFactor() { return fCameraScaleFactor; }
	...
	float                getLengthOfSampleTimeSlice( unsigned int iTelID ) { if( iTelID < fSample_time_slice.size() ) return fSample_time_slice[iTelID]; 
        ...
        map< unsigned int, unsigned int > getTelIDGrisu() { return fTelIDGrisu; }
        map< unsigned int, unsigned int > getTelID_matrix() { return getTelIDGrisu(); }
        vector<float>        getTelRadius()       //!< return vector of radius of telescope dishes
        {
            return fTelRad;
        }
        vector<ULong64_t>&     getTelType()  { return fTelType; }
	vector<ULong64_t>      getTelType_list();
        ...
        vector<float>&       getX_MM( unsigned int iTel )  { return fXTubeMM[iTel]; }
        ...
        vector<float>&       getY_MM( unsigned int iTel )  { return fYTubeMM[iTel]; }
