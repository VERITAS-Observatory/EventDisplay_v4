...
// disp table scaling
	...
	double scaleLengthParameter( double iLength ) { return scaleParameter( iLength, fLengthScaleParameter ); }
	double scaleWidthParameter( double iWidth ) { return scaleParameter( iWidth, fWidthScaleParameter ); }
	...
	bool setHistoBinning( string iVariableName, int iBin, double iMin, double iMax );
        void setNoise( unsigned int k, float iNoise ) { if( k < f_noise.size() ) f_noise[k] = iNoise; }
	void setWidthLengthScalingParameters( double iWS = 0.08, double iLS = 0.21 ) { fWidthScaleParameter = iWS; fLengthScaleParameter = iLS; }
        void terminate();
