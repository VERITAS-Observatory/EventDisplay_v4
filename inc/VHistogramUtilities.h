...
class VHistogramUtilities
{
   ...
   bool             get_Graph_from_Histogram( TH1F* h, TGraphErrors* g, bool bIgnoreErrors = false, double iMinBinContent = 0. );
   bool             get_Graph_from_Histogram( TH1F* h, TGraphAsymmErrors* g, bool bIgnoreErrors = false, bool bLinXaxis = false, double iCutUnrealisticErrors = -1. );
   TGraphErrors*    get_Profile_from_TH2D( TH2D *iH, TGraphErrors*g = 0, string iM = "median", int i_rebin = 2, double iXaxisValue = -10., double iMinusValue = 0. );
   ...
   ClassDefNV( VHistogramUtilities, 2 );
};
