...
// plotting variables
   ...
   ~VStereoReconstruction() {}
    ...
    void      plotLegend();
    void      setDebug( bool iB = false ) { bDebug = iB; }
    void      setName( string a = "VStereoReconstruction" ) { fName = a; }
    bool      setPlottingVariable( string iVar = "direction" );
    TCanvas*  getPlottingCanvas()                                 { return fPlottingCanvas; }
    void      setPlottingCanvasSize( int iX = 600, int iY = 600 ) { fPlottingCanvasSizeX = iX; fPlottingCanvasSizeY = iY; }
    void      setPlottingEnergyRangeLinear( double xmin = 0.01, double xmax = 200. ) { fPlottingMinEnergy = xmin; fPlottingMaxEnergy = xmax; }
    void      setPlottingEnergyRangeLog( double xmin = -2.0, double xmax = 2.0 ) { fPlottingMinEnergy = TMath::Power( 10., xmin ); fPlottingMaxEnergy = TMath::Power( 10., xmax ); }

    void      setPlottingLogEnergyAxis( bool iB = true )              { fPlottingLogEnergyAxis = iB; }
    void      setPlottingYaxis( double iMin = 0., double iMax = 0.2 ) { fPlottingYaxisMin = iMin; fPlottingYaxisMax = iMax; }
