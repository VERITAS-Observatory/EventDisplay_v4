...
class VModel3DFn
{
 private:
  ...
  double dot(double ax, double ay, double az, double bx, double by, double bz) const { return (ax*bx) + (ay*by) + (az*bz); }
 ...
 public:
  ...
  ////////
  ///void calcTESTFn( unsigned int iTel, unsigned int iPix, const double ipX, const double ipY, const double ipZ, double& val) const; //TEST
