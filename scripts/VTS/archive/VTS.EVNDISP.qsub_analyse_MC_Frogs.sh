...
set SFILN=NFINFIL
set NEVENTS=EVENTSEVENTS
##set ITER=ITERITER

@ ITER = $SGE_TASK_ID - 1

set NUMBER=(`expr $ZEW + $ITER`)
...
# output directory
##############################################################################################
set ODIR=$YDIR/analysis_d20130804_ATM"$ATMO"_"$TTA"_NOISE"$NOISE"/
mkdir -p $ODIR
...
FROGS="-frogs /lustre/fs5/group/cta/users/gahughes/VERITAS/analysis/EVDv400/V5_FLWO/newtemplate/mscw_ATM21_d20130804/gamma_"$ZEW"deg_750m_w"$WOB"_ID0_ana1234_NOISE"$NOISE"_1_"$ITER".root -frogid 0"

##############################################################################################
# run eventdisplay 
##############################################################################################

$EVNDISPSYS/bin/evndisp -runnumber=$SRUN -nevents=$NEVENTS -firstevent=$FIRSTEVENT -sourcefile $XFIL -deadchannelfile $DEAD -arraycuts $ACUT -outputfile $ODIR/$RRR.root -teltoana=$TTA $MCOPT $PEDOPT -calibrationdirectory $ODIR >& $ODIR/$RRR.log

####$EVNDISPSYS/bin/evndisp -runnumber=$SRUN -nevents=$NEVENTS -firstevent=$FIRSTEVENT -sourcefile $XFIL -deadchannelfile $DEAD -arraycuts $ACUT $FROGS -outputfile $ODIR/$RRR.root -teltoana=$TTA $MCOPT $PEDOPT -calibrationdirectory $ODIR >& $ODIR/$RRR.log
