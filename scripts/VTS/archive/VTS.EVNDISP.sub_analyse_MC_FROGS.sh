...
then
  echo
  echo "./VTS.EVNDISP.sub_evndisp_MC_GRISU_VBF.sh <ze> <array=V4/V5/V6> <particle=1/14/402> <ATMOSPHERE=21/22> <recid>" 
  echo 
...
###############################################################################################################
	 ...
	 ODIR=$VERITAS_USER_DATA_DIR"/analysis/EVDv400/"$ARRAY"_FLWO/gamma_"$ZEW"deg_750m/wobble_$WOB/"
	 mkdir -p $ODIR
     fi 
     if [ $PART = "14" ]
     then
