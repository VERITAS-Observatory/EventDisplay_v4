...
if [[ "$ISPIPEFILE" =~ ^/dev/pts/[0-9]{1,2} ]] ; then # its a terminal (not a pipe)
	if ! [ $# -eq 1 ] ; then
		echo "Prints the run numbers that had data recorded for all 4 telescopes."
...
while read -r RUNID TELECONFIG ; do
	if [[ "$RUNID" =~ ^[0-9]+$ ]] ; then
		
		#echo "::$RUNID $TELECONFIG"
		# decode the date tag
...
done < <($MYSQL -e "USE VERITAS ; SELECT run_id, config_mask FROM tblRun_Info WHERE $SUB")
